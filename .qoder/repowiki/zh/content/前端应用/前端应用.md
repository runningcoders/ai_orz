...
- [frontend/src/pages/user/mod.rs](file://frontend/src/pages/user/mod.rs)
...
- 新增进程管理页面和工具调用Tab功能
- 优化HTTP工具创建表单和验证逻辑
- **新增**：用户偏好编辑功能，支持Markdown格式的偏好书写和渲染
- **新增**：个人资料页「我的偏好」区块，使用统一的MarkdownRenderer组件进行展示
...
- 前端构建配置、开发环境与部署流程
- **新增**：进程管理、工具调用监控等高级功能
- **新增**：用户偏好编辑功能，支持Markdown格式的个性化设置
...
- **新增**：用户偏好编辑组件：UserProfile 支持 Markdown 格式的偏好书写和渲染。
- **新增**：Markdown 渲染组件：MarkdownRenderer 提供安全的 Markdown 转 HTML 渲染，支持 Mermaid 图表。
...
## 架构总览
前端以 Dioxus 为渲染引擎，通过 Router 驱动页面；页面组件调用 API 客户端进行数据交互；全局状态（认证、Toast、主题）通过 Context 共享；样式由 Tailwind + DaisyUI 提供，并通过自定义 CSS 增强视觉效果。**后端提供SPA回退服务，确保所有路由都能正确加载index.html**。构建时通过 build.rs 生成格式化的配置代码，确保前后端配置的一致性。**新增的用户偏好编辑功能通过 MarkdownRenderer 组件实现安全的 Markdown 渲染**。
...
P->>A : 调用 get_current_user_info()
A->>BC : 获取格式化配置
BC-->>A : FrontendConfig{api_base_url}
A->>B : GET /api/v1/user/me
B-->>A : ApiResponse{data.preferences}
A-->>P : 返回用户信息
P->>MR : 渲染 preferences 字段
...
- [frontend/build.rs:30-48](file://frontend/build.rs#L30-L48)
- [frontend/src/pages/user/profile.rs:27-42](file://frontend/src/pages/user/profile.rs#L27-L42)
...
- **新增**：用户资料页面：`/user/profile` 路由指向 UserProfile 组件，支持个人信息编辑和用户偏好管理。
...
- **新增**：用户信息API：get_current_user_info 和 update_current_user 支持用户偏好字段的读写。
...
- **新增**：Markdown 渲染依赖 pulldown-cmark，用于安全的 Markdown 转换。
...
- **新增**：Markdown 渲染优化：使用 use_memo 缓存渲染结果，避免重复解析；延迟扫描 Mermaid 代码块。
...
- **新增**：Markdown 渲染异常：确认 pulldown-cmark 依赖正确安装，检查 Mermaid 渲染函数是否在 window 对象上可用。
- **新增**：用户偏好保存失败：检查 update_current_user API 调用是否正确，确认 preferences 字段格式有效。
...
## 结论
AI Orz 前端采用 Dioxus 构建，具备清晰的分层结构与可维护的组件体系。通过统一的路由、状态管理与 API 客户端，实现了良好的可扩展性与一致性。**新增的SPA深链接处理机制确保了所有路由都能正确加载，完善的移动端适配提升了用户体验，前端配置格式改进提供了更一致的开发体验，用户偏好编辑功能丰富了用户的个性化设置能力**。主题系统与响应式设计进一步增强了应用的可用性。建议后续继续完善组件文档与测试覆盖，持续优化性能与可访问性。
...
- **新增**：Markdown 样式：`.markdown-body` 和 `.markdown-compact` 类提供统一的 Markdown 渲染样式。
...
- **新增**：Markdown 渲染：使用 MarkdownRenderer 组件安全地渲染 Markdown 内容。
- **新增**：用户偏好编辑：在 UserProfile 组件中使用 preferences 信号管理偏好内容。
