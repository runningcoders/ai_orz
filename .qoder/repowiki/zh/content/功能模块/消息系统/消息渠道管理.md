...
## 更新摘要
**所做更改**
- 新增凭证引用机制章节，详细说明lark_credential_id字段的作用和实现
- 更新渠道配置结构，反映凭证与渠道的解耦设计
- 新增用户级凭证库说明，解释identity_credentials字段的使用
- 更新凭证解析流程，展示从渠道引用到实际凭证的完整链路
- 增强API文档，包含凭证相关的请求参数和响应字段
- 更新故障排查指南，涵盖凭证相关问题的诊断方法
...
## 简介
本文件面向"消息渠道管理"能力，覆盖多渠道（HTTP、WebSocket、SSE、邮件等）的创建、配置、管理与监控。**重要更新**：现已支持基于凭证的引用机制，实现了凭证与渠道的解耦设计，提升了飞书渠道的安全性和可维护性。重点说明：
- 渠道类型与状态模型
- DAL 统一分发与 DAO 独立实现
- **新增**：凭证引用机制与用户级凭证库
- 连接测试、错误聚合与失败隔离
...
## 架构总览
DAL 通过纯 match 将消息分发到具体渠道 DAO，失败不阻断其他渠道；同时记录成功/失败并更新渠道最后推送时间与错误信息。SSE 作为内置推送通道，与外部渠道并行工作。**新增**：凭证引用机制实现了渠道配置与实际凭证的解耦，提升了安全性和可维护性。
...
MC-->>DAL : [已启用渠道列表]
loop 遍历渠道
DAL->>U : 解析凭证引用 (lark_credential_id)
U-->>DAL : 返回解密后的凭证
...
### 凭证引用机制
**新增**：凭证引用机制实现了渠道配置与实际凭证的解耦设计：

1. **凭证存储**：用户级凭证存储在 users.identity_credentials JSON 列中，支持加密存储敏感信息
2. **渠道引用**：渠道配置中的 `lark_credential_id` 字段指向具体的凭证ID
3. **解析流程**：DAL层通过 `resolve_credentials_with_library` 方法将凭证ID解析为实际凭证
4. **安全验证**：创建/更新渠道时通过 `validate_lark_credential_ref` 进行凭证有效性校验

```mermaid
flowchart TD
Start(["渠道操作"]) --> CheckType{"是否飞书渠道?"}
CheckType --> |否| Skip["跳过凭证验证"]
CheckType --> |是| Validate["validate_lark_credential_ref"]
Validate --> Resolve["resolve_lark_credential_ref"]
Resolve --> FindCred["查找凭证ID"]
...
- **新增**：凭证解析：在推送前解析渠道引用的凭证，确保使用正确的认证信息。
...
CheckLark --> |是| ResolveCred["解析凭证引用"]
...
- **更新**：飞书凭证解析：通过 `resolve_lark_credentials` 函数从用户凭证库解析渠道引用的凭证。
...
- **新增**：凭证库依赖：DAL 层依赖用户凭证库进行凭证解析，形成新的依赖关系。
...
- **新增**：凭证解析缓存：同一批渠道扫描中同用户只查一次凭证库，消除 N+1 查询问题。
...
## 结论
消息渠道管理以 DAL 为统一入口，采用纯 match 分发到独立 DAO，实现高内聚低耦合与编译期安全。**重大更新**：新增的凭证引用机制实现了渠道配置与实际凭证的解耦设计，通过用户级凭证库统一管理飞书等外部平台的认证信息，提升了系统的安全性和可维护性。通过连接测试、失败聚合与 SSE 推送，形成完整的渠道生命周期管理能力。配合 stats 监控与集成测试，保障多渠道稳定运行与持续演进。
