...
B --> C["FsReadToolFactory<br/>读取文件"]
B --> D["FsWriteToolFactory<br/>写入文件"]
B --> E["HttpFetchToolFactory<br/>HTTPS 抓取"]
B --> F["ShellExecToolFactory<br/>命令执行"]
...
## 架构总览
下图展示了从请求到工具执行的完整链路：注册表根据协议选择工厂，工厂基于数据库中的 ToolPo 构建具体工具实例，执行时受安全层约束，结果经 AOP 事件落盘为 JSONL 日志。
...
BF --> FR["FsReadToolFactory"]
BF --> FW["FsWriteToolFactory"]
BF --> HF["HttpFetchToolFactory"]
BF --> SE["ShellExecToolFactory"]
...
## 结论
内置工具系统通过工厂模式与注册表实现了高内聚、低耦合的工具生态。fs_read 与 fs_write 提供了安全可控的文件操作能力，配合严格的路径与权限校验，满足多 Agent 协作场景下的数据安全需求。HTTP 抓取与 Shell 执行进一步扩展了外部交互能力，并通过 AOP 与 JSONL 日志保障可观测性。遵循本文档的开发指南与最佳实践，可高效扩展新的内置工具并保持系统稳定与安全。

## 附录：自定义内置工具开发指南
- 步骤概览
  1. 实现 BuiltinToolFactory：提供 create_po() 定义默认元数据（id/name/description/parameters_schema/config/control_mode），以及 create(po) 返回 Box<dyn CoreTool>。
