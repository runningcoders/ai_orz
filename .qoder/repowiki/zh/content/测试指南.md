...
## 更新摘要
**所做更改**
- 更新了 CI/CD 管道配置部分，反映并行化作业执行和优化的构建流程
- 添加了 sccache 缓存机制的详细说明
- 更新了覆盖率阈值配置，区分 main 分支和 PR 的不同标准
- 增强了 protobuf 依赖管理的说明
- 改进了测试执行策略和性能优化指导
...
2. **报告生成阶段**：分离 lcov 和 HTML 报告生成
3. **阈值检查**：根据触发事件动态设置 fail-under-lines 阈值
4. **摘要输出**：将覆盖率摘要写入 GitHub Actions Summary
...
## 结论
AI Orz 的测试体系通过分层设计（单元、集成、E2E）、统一的测试基础设施（env/app/assertions/factories）与严格的 CI/CD 质量门禁，确保了代码质量与开发效率。最新的 CI/CD 管道优化包括并行化作业执行、sccache 缓存机制和差异化覆盖率阈值，显著提升了构建速度和测试效率。遵循本指南中的最佳实践，可进一步提升测试的可维护性与稳定性。
