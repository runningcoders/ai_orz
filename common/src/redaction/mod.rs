...
//! ```ignore
//! use common::redact;
//! // 字符串 → Result<String>；DTO / Value / Vec → Result<同类型>
//! // fail-closed：脱敏失败返回 Err，由上层决定处理方式（common::error 已提供 From 转换）
//! Ok(Json(redact!(response)?))
//! ```
//!
//! 本模块是前后端共享库：**内部不做任何日志输出**，仅返回 `Result` / 就地改写，
//! 观测与降级策略完全由调用方决定。
//!
//! 分层
//! ----
...
/// 本函数不可能失败（就地改写，无往返校验）；可能失败的路径在
/// [`redact!`] 宏（JSON 往返）中，以 `Err` 形式暴露。
