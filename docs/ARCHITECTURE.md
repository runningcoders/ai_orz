...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| **P1** | ToolCallResult 产物化引用策略 | ToolCallResult 已完成不复制 request args、inline size bound、基于 call_id/tool_id 的 ToolCallEntry 查询；trace_ref 仍是后续轻量引用协议字段；仅当结果需要用户下载或成为 Project Artifact 时再接入 attachment / artifact |
