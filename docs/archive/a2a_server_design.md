...
> 当前生效方案请参考：[common/src/api/a2a.rs](common/src/api/a2a.rs)（协议实体定义）+ [src/handlers/a2a/](src/handlers/a2a/)（5 个端点 Handler + mapper + JSON-RPC 分发）+ [docs/ARCHITECTURE.md](../ARCHITECTURE.md) §5.1 实体关系（A2A 反向能力即 external_agent）
