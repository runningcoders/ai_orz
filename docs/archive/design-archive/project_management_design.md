...
> 📦 归档标记（2026-08-16）：归档冻结。保留原因：project_management_design 设计文档归档冻结，设计决策已沉淀至 wiki 长文。生效方案：见源码和 wiki 长文。
...
参考 `docs/archive/design-archive/project_design.md`，PO 模型与 DAO 已完成。

### 2. tasks 表（无需修改）

参考 `docs/archive/design-archive/task_design.md`，PO 模型与 DAO 已完成。
