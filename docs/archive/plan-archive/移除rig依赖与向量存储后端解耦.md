...
> 📦 归档标记（2026-08-16）：归档冻结。保留原因：移除rig依赖与向量存储后端解耦 功能已完成并通过验收，文档转为历史快照。生效方案：见源码和 wiki 长文。
...
> - [external_agent_design.md](../archive/design-archive/external_agent_design.md) — 外部 Agent（Cli/Remote）协议设计
> - [vector_search_architecture.md](../archive/design-archive/vector_search_architecture.md) — 向量索引架构与 DAL 向量化约束
