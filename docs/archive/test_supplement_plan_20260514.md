...
> 📦 **归档标记（2026-08-15）**：本文档描述的早期 **Tool 模块 5 项功能 + Memory 模块 6 项功能的测试补齐计划**（关联 commits 05ef2f0 / 1242ea0）已被项目当前 1124 个 100% 通过率的测试套件（DAO/DAL/Domain/Handler/Pkg 完整覆盖）覆盖，对应测试以实际单元测试文件中的 test 函数为准。
> 保留原因：历史审计——追溯 2026-05-14 时点对 Tool/Memory 两模块的测试缺口识别与优先级排序；对照现有测试回看当时的计划项是否已全部落地。
> 当前生效方案请参考：AGENTS 启动流程测试对齐规则（`tests/common/env.rs::init_full_test_env` 严格复刻真实启动顺序）+ DAO/DAL/Domain/Pkg 各模块 `*_test.rs` 实际测试代码 + 覆盖率门槛（PR 38% / main 45% cargo-llvm-cov）
