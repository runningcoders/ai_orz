...
> 📦 **归档标记（2026-08-15）**：本文档描述的早期 **零散遗留事项 scratch pad 记录方式**（无结构化7章、跨多主题杂项混合）已被 [docs/plan/](../../plan/) 目录下按模板B规范化的独立功能计划文档取代。
> 保留原因：历史审计——对照3项已完成事项的提交SHA（0222c0d / 0488c57 / 977a04a）追溯决策上下文；2项未完成待办可作为后续独立 plan 的输入素材。
> 当前生效方案请参考：[docs/plan/Agent搜索查询三场景统一重构.md](../../plan/Agent搜索查询三场景统一重构.md)（含Tool/Skill/Project/Task三场景统一规范），其余后续新增事项请按模板B在 plan/ 下创建独立文档。
