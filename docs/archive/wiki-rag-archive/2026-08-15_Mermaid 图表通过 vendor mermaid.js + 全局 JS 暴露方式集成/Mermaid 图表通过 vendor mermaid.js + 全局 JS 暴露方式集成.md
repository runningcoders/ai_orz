> 📦 归档标记（2026-08-15）：被 [Mermaid 图表采用 vendor mermaid.js 的独立阶段实现](docs/wiki/knowledge/zh/Mermaid 图表采用 vendor mermaid.js 的独立阶段实现/Mermaid 图表采用 vendor mermaid.js 的独立阶段实现.md) 取代。保留原因：历史参考，主卡已吸收本卡独有源码锚点与硬约束。生效方案：主卡真实路径作为唯一 RAG 召回目标。
---
kind: design
