> 📦 归档标记（2026-08-15）：被 [进程列表通过 shell_list LLM 工具双暴露，复用 Agent scope 过滤](docs/wiki/knowledge/zh/进程列表通过 shell_list LLM 工具双暴露，复用 Agent scope 过滤/进程列表通过 shell_list LLM 工具双暴露，复用 Agent scope 过滤.md) 取代。保留原因：历史参考，主卡已吸收本卡独有源码锚点与硬约束。生效方案：主卡真实路径作为唯一 RAG 召回目标。
---
kind: design
