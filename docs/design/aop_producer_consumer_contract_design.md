...
> 状态：**全部落地**（2026-09-16，设计已拍板）。**已落地**：① 删除死代码 `models/event.rs`（§8）；② **Step 1/2/3/4 全部完成**（§7）—— topic 收敛为 `common::enums::EventTopic`、`Producer` 契约改形（DAL-as-Producer）、`EventSink`/`ProducerLoop`/`RetryDecision`/`finish_consumption` 单一收尾出口、删 `ack/nack`/`source`；**P2/P3/P6 三个既往缺陷随之修复**，A2A 拆成「生产者只认领 + `a2a_poll` 消费者执行」；wiki 长文与 RAG 卡全量同步（Step 4）。③ **遗留项收尾（2026-09-16 晚）**：§4.1 `ordered` 入队 wiring（`enqueue_ungated` + publish 按订阅声明选路；**§6.1-2 运行期告警用户拍板不做**——声明是订阅者自己的事，框架不提醒）、per-event 指数退避（`EventRef.not_before`，1s→60s 封顶，见 §4.5/§8）、email 游标从进程级静态收编为注入式 `UidCursorStore`（与微信 `CursorStore` 同构）。无遗留。
...
> ✅ **wiring 已落地（2026-09-16 晚）**：publish 侧按订阅声明选路——`Subscription.ordered == true` → `queue.enqueue`（门闩队列），`false` → 新增的 `EventQueue::enqueue_ungated`（无视 `order_key` 直进 `global_heap`，JSON 里的 `order_key` 保留给监控展示）。**§6.1-2 的运行期告警用户拍板不做**：`ordered` 是订阅者自己的属性，声明错了行为自然不对，框架不替它操心（护栏仍由 §6.1-1 注册期硬校验 + §6.1-3 护栏单测兜底）。测试：`ungated_enqueue_bypasses_order_key_gate`（钉死「未声明 ordered 的事件即使带 order_key 也不进门闩队列」）。
...
2. ~~运行期一次性告警~~：**用户拍板不做（2026-09-16 晚）**——`ordered` 是订阅者自己的属性，声明错了行为自然不对，框架不替它操心。§4.1 wiring 落地后，未声明 `ordered` 的事件走 `enqueue_ungated` 直进堆（无门闩），行为与声明**自洽**，告警失去意义。
...
| 4 ✅ **已落地（2026-09-16）** | 清理：`Producer::register` 遗留调用与 `a2a_polling` 的 domain 直调经 `git grep` 核实**已自然满足**；wiki 长文与 RAG 卡全量手改同步（~38 篇，失效 token 清零，提交 `f300b5c0`） | ✅ `docs-lint`（659 files, 0 violations）+ 全量测试绿（lib 1511 过，唯一挂系既有 lark 并行 DB flake；全部集成 target 绿） |
| 5 ✅ **已落地（2026-09-16 晚）** | 遗留项收尾：① **§4.1 `ordered` 入队 wiring** —— 新增 `EventQueue::enqueue_ungated`，publish 侧按订阅声明选路（声明 ordered → 门闩队列；否则直进堆）；**§6.1-2 运行期告警用户拍板不做**；② **per-event 指数退避** —— `EventRef.not_before` + `retry_backoff_ms`（1s→2s→…→60s 封顶），`dequeue_next` 摘放扫描不阻塞已到期事件；③ **email 游标收编** —— 删进程级静态 `CONFIRMED_UIDS`，改注入式 `UidCursorStore`（`ImapPollRegistry` 创建，`EmailDaoImpl` 经 `cursors()` 共享同一 `Arc`，与微信 `CursorStore` 同构） | ✅ `cargo clippy --workspace --exclude frontend --all-targets -- -D warnings` 绿；`cargo fmt --all -- --check` 绿；`cargo test --workspace --exclude frontend --lib` **1515 过 / 1 挂**（新增 4 条：退避曲线 / 退避不阻塞 / ungated 绕过门闩 / registry-DAO 共享游标；唯一挂系既有 lark 并行 DB flake，单跑 5/5 过）；全部集成 target 绿；`docs-lint` 绿 |
...
3. **`ordered` 未接入入队判定**（§4.1 注）：Step 1 保持「门闩无条件按 `order_key` 生效」的原行为；`Subscription.ordered` 目前只被注册期校验读取。因此 §6.1-2 的运行期告警在 Step 1 **尚未生效**，随 Step 2 的 wiring 一起打开。（→ **已于 2026-09-16 晚收尾落地**，见 §4.1 注；运行期告警用户拍板不做，见 §6.1-2）
...
3. **`ordered` 仍只用于注册期校验**：§6.1-2 的「未声明 `ordered` 却带 order_key → 运行期告警」**尚未接线**（`Registry::register_consumer` 只读 `ordered` 做「Sync + ordered → Err」）。它牵涉入队判定改造、与契约改形不同批 → 保持「门闩无条件按 `order_key` 生效」的原行为（行为等价）。§7.1-3 曾说「随 Step 2 wiring 一起打开」，**此处更正：本步不含该项**。（→ **已于 2026-09-16 晚收尾落地**，见 §4.1 注；告警不做）
...
**与 Step 1 的衔接**：§7.1-2 预告的「Step 2 注册 `MessageDalImpl` 时同步把 `message.created` 订阅改成 `.notify_producer()`」**已兑现**（[message.rs](src/consumer/message.rs#L97-L99)）。**测试清单执行状态**：除「`ordered=false` 且带 order_key → 直接分发」（§6.1-2，见上注 3，未接线——**已于 2026-09-16 晚落地**，对应队列层测试 `ungated_enqueue_bypasses_order_key_gate`）外全部落地；`Discard` 用例只断言「走 ack 路径且仍回调 `on_consumed`」，「同 `order_key` 后继可继续」复用 `EventQueue` 既有 ack 语义、未另加例。
...
| **重试时间策略**（per-event 退避 / `Retry { after }`） | **框架侧 per-event 退避已落地（2026-09-16 晚）**：`EventRef.not_before` + `nack` 时按 `attempt` 指数递增（`retry_backoff_ms`：1s→2s→…→60s 封顶），`dequeue_next` 摘出未到期事件再放回（不阻塞同堆已到期事件）；worker 的 `error_retry_sleep_ms` 保留（nack 后的节奏 sleep）。**`Retry { after: Duration }`（生产者自带时间策略）仍按 YAGNI 延后**；重试**次数**上限维持契约——框架层不做 max_retry，由生产者 `on_failed` 决定（`inbound_retry::decide` 的 `attempt >= 8` 兜底）。测试：`retry_backoff_grows_exponentially_and_caps` / `nack_backoff_defers_event_without_blocking_due_ones`（毫秒级退避注入 `with_backoff_policy`） |
| wiki 里旧 `EventTopic` / 已删 `models/event.rs` 的段落 | **已于 Step 4 全量手改同步（2026-09-16，`f300b5c0`）**：~38 篇失效 token 清零，新概念（DAL-as-Producer / EventSink / ProducerLoop / RetryDecision / finish_consumption / notify_producer / inbound_retry / a2a_poll 拆分）全部写入。注意 wiki 头部「快照不手改」不适用于 agent 增量维护——规范在 `docs/skills/ai-orz-wiki-maintainer.md`，手改 + `docs_lint` 通过是可行路径 |
