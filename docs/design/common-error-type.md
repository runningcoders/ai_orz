...
> 🎯 **本文档定位**：前后端/API/工具调用/运行时统计共享的错误契约设计——ErrorCode 细粒度枚举+ErrorType 粗粒度分类+ErrorField 安全业务上下文
> 状态：定稿（2026-06-26；完整实现重构完成，所有测试通过）
> 查阅场景：新增错误码枚举、排查错误分类与前端展示映射、设计工具调用返回错误格式时打开；具体错误定义直接看 common/src/error/
>
> 关联文档：
