...
> 🎯 **本文档定位**：贯穿请求全生命周期的上下文对象设计——用户身份/组织元数据/基础设施引用、不可变优先与重建替代修改原则
> 状态：定稿（2026-08-03；caller_type 字段与身份封装方法落地完成）
> 查阅场景：新增上下文元数据字段、排查 Builder 重建语义、理解调用方身份/权限封装方法时打开；具体字段定义直接看 common/src/context/
>
> 关联文档：
