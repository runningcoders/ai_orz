...
3. ✅ 实现 `http_fetch` + 完整单元测试（4/4 通过）
4. ✅ 在 `builtin.rs` 汇总注册，启动时自动注册
5. ✅ 编译验证通过
6. ✅ 讨论文件读写关键点和设计决策 ✅ **讨论完成**
7. ✅ 实现 `fs_read` + 单元测试（4/4 通过）
8. ✅ 实现 `fs_write` + 单元测试（5/5 通过）
9. ✅ 在 `builtin.rs` 更新注册
10. ✅ 运行全量后端测试验证 **508/508 全部通过**
11. ✅ 更新文档对齐最终实现
12. ✅ 准备提交代码
