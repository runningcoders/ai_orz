...
- **collector.rs 新增时间序列查询能力**：StatsInterval 枚举新增 `Minutely`（原来只有 Hourly/Daily），时间序列截断函数 `timestamp - (timestamp % 60000)` 按分钟 bucket（⚠️ 不可写成 `(timestamp / 60000) * 60000`：DuckDB 的 `/` 是浮点除，会失去 bucket 语义且返回值落 0）；服务于 Token 消耗看板（finance handler → model_provider domain → DAO → Stats.query_time_series），分钟级三线（tokens_input/output + call_count）聚合。
