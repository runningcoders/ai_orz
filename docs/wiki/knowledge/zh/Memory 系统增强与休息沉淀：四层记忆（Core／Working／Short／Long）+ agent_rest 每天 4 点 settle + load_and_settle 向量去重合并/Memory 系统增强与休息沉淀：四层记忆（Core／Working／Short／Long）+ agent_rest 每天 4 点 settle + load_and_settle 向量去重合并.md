...
| 22 | **收尾归属必须按 topic 反查 producer，禁止按 `source` 字符串分流**（2026-09-16 Step 2/3 落地，覆盖旧约定）：`Consumer::ack` / `nack` 与 `source` 已**整体删除**；`messages.status → Processed` 搬进 `impl Producer for MessageDalImpl::on_consumed`（`→ Pending` 搬进同对象 `on_failed`），由 `EventTopic::MessageCreated → producer` 索引直接决定——原先 `if source != "message.created" { return }` 的硬编码分流随之消失。**为什么必须这样**：只有 `message.created` 的 `event_id` 是 messages 表主键；`agent.settle.requested` 等事件在 messages 表无行，统一按 event_id 去 `UPDATE` 会命中 0 行、静默 `Ok`（把「传进来的 id 一定指向 messages 表」变成没说出口的前提）。归属改由 topic 索引承担后，「哪些事件该翻 messages 状态」成为**注册期事实**，而非运行期字符串比较 | `cargo test --lib pkg::aop`（1 producer : 1 topic 校验）；`tests/integration/agent_settle_queue_test.rs` | [src/service/dal/message.rs](src/service/dal/message.rs) + [registry.rs](src/pkg/aop/core/registry.rs) |
