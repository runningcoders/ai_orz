...
| 6 | **Prompt Token 熔断永不抛错**：即使总 Token 超限（极端场景 50 个技能），也仅裁剪 + 打 log_warn，绝不中断 awaken 流程；Layer1+Layer2 总和必须 ≤ 5K（预算保障永不裁剪）；Layer3 注入按优先级排序后超出部分逐个跳过 | 构造 50 个超长 Prompt 技能后调用 awaken 断言 200 OK 且日志有熔断记录 | [dal/agent.rs build_full_prompt 裁剪循环](src/service/dal/agent.rs) Prompt Builder 组装 |
