...
- **环境变量覆盖**：关键项通过 `std::env::var` 读取，如 `AI_ORZ_BASE_PATH`、`AI_ORZ_LISTEN_ADDR`（监听地址）、`SECRET_KEY`（敏感数据加密密钥）、`JWT_SECRET`、`JWT_EXPIRY_HOURS`、`FRONTEND_DIST_DIR`、`TEST_*` 等测试变量；**首次初始化时环境变量偏好会固化进生成的配置文件**（定向替换对应行、保留模板注释），之后环境变量仅本次运行内存覆盖、不再改写文件；**dev 模式专用变量 `DX_BACKEND_URL`** 用于覆盖 `Dioxus.toml` 中 dx 视角的 proxy backend（前后端分机部署逃生舱）。
...
| `src/config.rs` | 后端配置加载器：检查并创建 `.ai_orz` 目录、不存在则写出默认 TOML、解析 TOML 到 `AppConfig`、应用环境变量覆盖（`AI_ORZ_LISTEN_ADDR`/`SECRET_KEY`）、**首次初始化把环境变量偏好固化进配置文件**（`persist_first_init_env_prefs` 定向替换行、保留注释）、确保日志目录存在，并通过 `OnceLock` 提供全局单例 |
...
| `src/pkg/mod.rs` | JWT 初始化回退链：`JWT_SECRET`/`JWT_EXPIRY_HOURS` 环境变量 → 配置文件 `jwt.secret`/`jwt.default_expiry_hours`（首启可由环境变量固化）→ 内置默认 |
...
3. 若 `.ai_orz/ai_orz.toml` 不存在，写入嵌入的默认配置（并记录 `config_created` 首启标记）。
...
6. **环境变量内存覆盖**：`AI_ORZ_LISTEN_ADDR` 直接覆盖 `server.listen_addr`（与配置字段完全对齐，host:port 校验失败即报错）、`SECRET_KEY` 覆盖 `security.secret_key`。
7. **首次初始化固化**：若本次是首次生成配置文件（config_created），把用户通过 `AI_ORZ_LISTEN_ADDR`/`JWT_SECRET`/`JWT_EXPIRY_HOURS`/`FRONTEND_DIST_DIR`/`SECRET_KEY` 指定的偏好定向写回配置文件（`persist_first_init_env_prefs` 替换对应行、保留模板注释），后续环境变量消失仍生效。
8. 根据 `logging.enable_file_log` 创建日志子目录。
9. 存入 `OnceLock` 全局单例，后续通过 `config::get()` 获取。

### 3.3 环境变量覆盖策略
- 应用级：`AI_ORZ_BASE_PATH` 覆盖数据根目录（每次调用 `base_data_path()` 都读环境变量，支持运行时切换）。
- 监听地址：`AI_ORZ_LISTEN_ADDR` 直接覆盖 `server.listen_addr`（host:port，与配置字段完全对齐，见 `src/config.rs`）。
- 安全密钥：`SECRET_KEY` 覆盖 `security.secret_key`（数据库敏感字段加密，见 `src/config.rs`）。
- JWT：`JWT_SECRET`、`JWT_EXPIRY_HOURS` 回退链为「环境变量 → 配置文件 `jwt.secret`/`default_expiry_hours` → 内置默认」（见 `src/pkg/mod.rs`）。
- 前端静态资源：`FRONTEND_DIST_DIR` 可覆盖 `dist` 目录名（见 `src/lib.rs`）。
- **首启固化**：以上映射到配置字段的环境变量在首次初始化时写回配置文件（`persist_first_init_env_prefs`），防止后续环境变量消失时静默回退默认值。
- 测试：`TEST_LLM_*`、`TEST_EMBEDDING_*`、`DATABASE_URL`、`SQLX_OFFLINE` 等控制可选测试行为。
- 跨模块共享的环境变量名集中在 `common/src/config.rs` 中以 `pub const` 形式声明（如 `BASE_DATA_PATH_ENV`、`LISTEN_ADDR_ENV`、`SECRET_KEY_ENV`），其他模块引用常量而非硬编码字符串。
...
2. **配置文件自动生成**：首次运行自动写出默认 TOML，用户不应手动编辑嵌入模板，应编辑 `.ai_orz/ai_orz.toml`；**首次初始化时设置了 `AI_ORZ_LISTEN_ADDR`/`JWT_SECRET`/`JWT_EXPIRY_HOURS`/`FRONTEND_DIST_DIR`/`SECRET_KEY` 会把值固化进文件**（定向替换保留注释），之后环境变量仅内存覆盖不落盘。
3. **TOML 解析失败即报错**：解析错误统一映射为 `ErrorCode::ConfigInvalid`，不会静默降级。
4. **JWT 密钥必须覆盖**：回退链为 `JWT_SECRET` 环境变量 → 配置文件 `jwt.secret`（首启可由 `JWT_SECRET` 固化）→ 内置默认；生产环境务必覆盖，避免使用内置默认值。
...
8. **secret_key 不可为空**：启动时强制校验并自动补写，属于硬性安全约束；`SECRET_KEY` 环境变量可在首次初始化时固化进 `security.secret_key`；`lark.app_secret`、`lark.encrypt_key`、`lark.verification_token` 等敏感字段建议通过环境变量注入。
