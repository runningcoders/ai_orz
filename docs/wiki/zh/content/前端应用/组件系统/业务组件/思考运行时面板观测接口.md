...
- 【② Plan 落地快照】占位：待 [2026-08-14-policy-engine-and-think-runtime.md](docs/superpowers/plans/2026-08-14-policy-engine-and-think-runtime.md) 由 ai-orz-doc-maintainer 精简到 docs/archive/plan-archive/ 后回填（现参考 superpowers 执行蓝图 §File Structure 表格 + §P3/P4 实施阶段）
- 【④ RAG 原子知识卡】
...
#### ④ RAG 原子知识卡（本次追加 T1+T2）：
- [策略引擎：Policy trait + PolicyGroup 嵌套组合 + policy_set! 宏声明式写法](docs/wiki/knowledge/zh/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法.md) — UserCancelPolicy 读取 cancel_token 与前端 cancel 按钮联动；PolicyMixed hard/soft 命中列表在面板展示
...
**2026-08-16 增量更新**：追加 T1（policy_set! 宏 + PolicyMixed 混合模式）与 T2（runtime 诊断三接口 Agent 可调用工具化）2 张 RAG 卡互引；cite 区补齐对应 design/plan 占位；§2 章节来源行号降级为无行号范围（因 policy_set! 宏改造后 awakening.rs 行号范围与旧值存在偏差，优先降级以免指向错误范围）。
