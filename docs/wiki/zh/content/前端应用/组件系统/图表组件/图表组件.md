...
- 配置选项
  ...
  - value_label：Y 轴数值描述。
- 交互与样式
  - HUD 背景、坐标轴刻度、X 轴时间标签（格式按数据桶宽自适应：日桶 `M-D`、同日内分钟/小时桶 `HH:MM`、跨天小时桶 `M-D HH:MM`；标签个数按画布可用宽度反推）。
  - Y 轴刻度数值按量级进位（K/M/B，字符数 ≤ 5），百万级不再顶破画布左缘。
...
**变更 commit**：3f6872c8（聊天侧栏运行统计）+ 1245f3f8（Token 消耗看板驱动前端多场景消费）

### LineChart 组件扩展（line_chart.rs）
- **多曲线支持**：原来单系列 → 新增 `primary_second_field`（主色第二曲线）+ `secondary_field`（右轴独立曲线，独立刻度）+ `secondary_label` 配置，实现同轴三条折线（input/output tokens 左轴 + call_count 右轴）
- **颜色参数化**：`draw_line`/`draw_points` 函数从原来硬编码 HUD_PRIMARY → 接受 color 参数，每个系列独立配色
- **X 轴时间标签模式**：新增 `TimestampFormat::TimeOfDay`（HH:MM），支撑分钟级时序；原 Date 模式保留用于日/小时粒度
- **图例绘制**：新增 `draw_legend` 函数，底部居中显示各曲线 swatch + 名称

### HudPalette 扩展（hud_palette.rs）
- HUD_SECONDARY(#22d3ee 青蓝) — 冷色系第二曲线（与 HUD_PRIMARY 暖橙形成冷暖对比）
- HUD_TERTIARY(#a78bfa 紫) — 第三曲线（同轴三线场景）

### stats.rs 适配层扩展
- 新增 `UserStatsPanel`（个人信息页用，自带 HudPanel 外壳）
- 新增 `AgentStatsPanelCompact`（聊天侧栏 Agent Tab 用，无外壳 + 320x180 原生 Canvas 尺寸，避免 CSS 缩放导致文字过小）
- `render_time_series_chart_sized(width, height)` 接受动态尺寸参数

### LineChart 消费场景扩展
| 场景 | 尺寸 | 配置 |
|------|------|------|
| 统计仪表盘（原） | 600x200 | 单系列 call_count |
| 个人信息页 | 600x200 | 三线：input/output 左轴 + call_count 右轴 |
| 聊天侧栏 Agent Tab | 320x180 | 三线：唤醒次数 QPS + input/output tokens |

---

### 本文关联的文档
- 🎴 RAG 卡: docs/wiki/knowledge/zh/Canvas HUD 可视化：GraphCanvas 知识图谱 + 图表场景LineDonut + 仪表盘Gauge双版 + HudPalette橙光光晕/Canvas HUD 可视化：GraphCanvas 知识图谱 + 图表场景LineDonut + 仪表盘Gauge双版 + HudPalette橙光光晕.md

---

### 更新摘要（2026-09-11，base ad54411c→HEAD）
**主题**：新增 RingProgress 环形进度组件 + TimeRangePicker 通用时间筛选 + 引用块即时显示
**关键变更**：
1. **RingProgress 通用环形进度组件（新增）**——`frontend/src/components/ring_progress.rs` 纯 Canvas 2D 渲染，复用 HudPalette.draw_glow_stroke 橙色光晕；props: ratio(0.0-1.0) + color；AgentRuntimeState.tokens_used_ratio → 驱动 Agent 上下文 Token 占比展示
2. **TimeRangePicker 通用时间区间筛选组件（新增）**——`frontend/src/components/time_range_picker.rs` props: start/end + 预设快捷按钮（1h/6h/24h/7d/30d）；所有统计看板统一引入（AOP 系统页/ModelProvider Token 时序/用户页统计）
3. **聊天侧栏引用块即时显示**——chat.rs + chat_side_panel.rs 引用块（带 reference_id 的消息气泡上方）即时渲染，宽度严格以本条消息气泡为上限，不再溢出撑破布局
**涉及 RAG 卡**：Canvas HUD 可视化卡 + 统计查询 API 与前端仪表盘卡 + 思考运行时前端观测卡