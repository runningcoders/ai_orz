...
- [frontend/src/components/avatar_bubble.rs](frontend/src/components/avatar_bubble.rs) — 头像信息气泡 + 浮层定位工具（BubbleAnchor/resolve_anchor 等 pub(crate) 开放复用）
- [frontend/src/components/model_provider_bubble.rs](frontend/src/components/model_provider_bubble.rs) — 模型提供商信息气泡（复用 avatar_bubble 定位工具，Agent 页接入）
...
- 【2026-09-18 追加】[模型提供商信息气泡：浮层定位工具复用 + ModelProviderBubble 懒加载卡片](docs/wiki/knowledge/zh/模型提供商信息气泡：浮层定位工具复用%20+%20ModelProviderBubble%20懒加载卡片/模型提供商信息气泡：浮层定位工具复用%20+%20ModelProviderBubble%20懒加载卡片.md) — 浮层信息气泡交互范式（onfocus 触发 + 实测锚点定位 + 懒加载三态缓存）与 Agent 页模型提供商接入
...
- [frontend/src/components/workspace_graph.rs:1-756](frontend/src/components/workspace_graph.rs#L1-L756)
...
#### 2026-09-18 增量：模型提供商信息气泡（ModelProviderBubble）+ 浮层定位工具复用
- **ModelProviderBubble 组件**（frontend/src/components/model_provider_bubble.rs，Ref daed0b60）：Agent 列表页/详情页的模型提供商标识升级为点击弹出信息气泡——文本 chip（tabindex=0）onfocus 触发，浮层展示提供商名称/类型/能力徽章、model_name、描述与最近 24h 精简统计（调用/平均 QPS/输入输出 Token，format_compact_count k/M 进位），卡片底部 Link 跳模型提供商详情页。数据零后端改动：复用 get_model_provider(with_model_call_stats=true)（stats_interval=hourly 小写契约），懒加载三态缓存（None/Some(Err)/Some(Ok)）。
- **avatar_bubble 浮层定位工具 pub(crate) 开放**：BubbleAnchor/resolve_anchor/viewport_size/focused_element/scroll_ancestor_rect/BubbleAlign::dropdown_class 以 pub(crate) 开放给 model_provider_bubble 复用，同一套「向上优先 → 翻向下 → 限高滚动」锚点算法服务头像盒与文本 chip 两种触发形态。
- **resolve_anchor 签名泛化**：原「头像边长 avatar_px」参数假设触发层为正方形，文本 chip 会算错右对齐偏移；收敛为触发盒实测矩形 `box_rect: &web_sys::DomRect` 单参数（总 5 参，规避 clippy too_many_arguments），AvatarSize::px() 随之删除。
...
图表来源
- [frontend/src/components/workspace_graph.rs:98-555](frontend/src/components/workspace_graph.rs#L98-L555)
...
- [frontend/src/components/workspace_graph.rs:10-14](frontend/src/components/workspace_graph.rs#L10-L14)
