...
- [tests/integration/model_provider_embedding_test.rs](tests/integration/model_provider_embedding_test.rs)
...
- [frontend/src/pages/finance/identity_github.rs](frontend/src/pages/finance/identity_github.rs)
...
- [frontend/src/components/model_provider_bubble.rs](frontend/src/components/model_provider_bubble.rs) — Agent 页信息气泡组件，复用本文数据源 API（get_model_provider + model_call_stats）
- [frontend/src/pages/hr/agents.rs](frontend/src/pages/hr/agents.rs) — Agent 列表页信息气泡消费方

### 本文关联的文档

**④ RAG 原子知识卡**：
- 【2026-09-18 追加】[模型提供商信息气泡：浮层定位工具复用 + ModelProviderBubble 懒加载卡片](docs/wiki/knowledge/zh/模型提供商信息气泡：浮层定位工具复用%20+%20ModelProviderBubble%20懒加载卡片/模型提供商信息气泡：浮层定位工具复用%20+%20ModelProviderBubble%20懒加载卡片.md) — Agent 页提供商标识点击弹卡（基础信息 + 最近 24h 统计 + 卡内跳详情页），数据源即本文的 get_model_provider 契约
- [Embedding Provider 生命周期：ModelProviderStatus Disabled(2) + 创建不阻塞策略 + 重建触发条件矩阵](docs/wiki/knowledge/zh/Embedding%20Provider%20生命周期：ModelProviderStatus%20Disabled(2)%20+%20创建不阻塞策略%20+%20重建触发条件矩阵/Embedding%20Provider%20生命周期：ModelProviderStatus%20Disabled(2)%20+%20创建不阻塞策略%20+%20重建触发条件矩阵.md) — 提供商状态机与重建策略（气泡卡中状态/能力徽章的数据来源）
</cite>

## 更新摘要
**变更内容（2026-08-19 增量更新）**
- 新增凭证管理页面引用：identity.rs（身份凭证聚合页）、identity_tavily.rs（Tavily 凭证区块）、identity_github.rs（GitHub 凭证区块）
- 新增 Tavily 集成前端 API 客户端引用：frontend/src/api/tavily_integration.rs
- 新增工具管理页面引用：tools.rs（工具列表）、tool_detail.rs（工具详情）
- 更新 Embedding Provider 切换与向量重建的前端交互描述

**变更内容（2026-09-18 增量，base 005a7e83→daed0b60）**
- Agent 页（agents.rs / agent_detail.rs）模型提供商标识升级为信息气泡：新增组件 frontend/src/components/model_provider_bubble.rs，点击弹出基础信息 + 最近 24h 统计（调用/平均 QPS/Token）浮层卡，卡内跳本文对应的模型提供商详情页
- 数据源零后端改动：复用 get_model_provider 的 with_model_call_stats=true + stats_start_time/stats_end_time + stats_interval=hourly（窗口 24h，interval 值必须小写）
- avatar_bubble.rs 浮层定位工具 pub(crate) 开放：resolve_anchor 泛化为触发盒实测 DomRect 签名（正方形头像盒与长条文本 chip 统一处理），删除 AvatarSize::px()
...
- 凭证管理页面：frontend/src/pages/finance/identity.rs 为身份凭证聚合页，嵌入 identity_tavily.rs（Tavily 凭证区块）与 identity_github.rs（GitHub 凭证区块）。
- 工具管理页面：frontend/src/pages/finance/tools.rs 提供工具列表与管理，tool_detail.rs 提供工具详情。
...
- 前端界面
  - 列表展示、新增表单（名称、类型、模型名、API Key、Base URL、描述、上下文长度）、启用/禁用、调用测试、切换 Embedding Provider 确认弹窗；列表新增 status=2(Disabled) 显示「未启用」badge + 「启用」按钮；新增 Modal 支持 capability 选择（Agent/Embedding），API Key label 动态显示；禁用按钮发 status=2(Disabled) 而非 status=0(软删除)，与删除按钮语义分离；删除确认文案对 Embedding 类型提示语义搜索失效后果。
  - 凭证管理页面：identity.rs 聚合页嵌入 identity_tavily.rs（Tavily API key 凭证区块：列表/创建/编辑/设默认/删除）与 identity_github.rs（GitHub PAT 凭证区块），数据来源 GET /api/v1/finance/identity/tavily/status。
  - 工具管理页面：tools.rs 提供工具列表，tool_detail.rs 提供详情；工具就绪状态 runtime_ready badge 提示 Tavily 凭证是否就绪。
...
- [src/handlers/finance/model_provider/mod.rs:1-25](src/handlers/finance/model_provider/mod.rs#L1-L25)
- [frontend/src/pages/finance/model_providers.rs:1-502](frontend/src/pages/finance/model_providers.rs#L1-L502)
- [frontend/src/pages/finance/identity.rs:1-21](frontend/src/pages/finance/identity.rs#L1-L21)
- [frontend/src/pages/finance/identity_tavily.rs:1-315](frontend/src/pages/finance/identity_tavily.rs#L1-L315)
- [frontend/src/api/tavily_integration.rs:1-55](frontend/src/api/tavily_integration.rs#L1-L55)
- [frontend/src/pages/finance/tools.rs](frontend/src/pages/finance/tools.rs)
- [frontend/src/pages/finance/tool_detail.rs](frontend/src/pages/finance/tool_detail.rs)
...
### 前端界面：列表、新增、测试与切换
- 列表：展示名称、类型、能力、模型、状态与操作按钮；status=2(Disabled) 显示「未启用」badge + 「启用」按钮。
- 新增：创建 Modal 支持 capability 选择（Agent/Embedding），默认 Agent；表单包含名称、类型、模型名、API Key、Base URL、描述、最大上下文长度、推荐上下文长度；API Key label 动态显示（Agent 显示 "API Key"，Embedding 显示 "API Key 或 Token"）；提交后自动触发连接测试提示。
- 测试：弹出对话框输入 Prompt，调用后端接口显示结果。
- 切换：当启用 Embedding Provider 冲突时，弹出确认弹窗，告知将禁用当前、启用新的并重建向量索引（7 类实体顺序重建）。
- 禁用与删除分离：
  - 禁用按钮：发 `UpdateModelProviderStatusRequest { status: 2 }`（Disabled），条目保留在列表，显示「未启用」badge，可再次启用
  - 删除按钮：走 `delete_model_provider` API，Embedding 类型显示警示文案「若它是使用中的向量模型，删除后语义搜索将失效，需重新配置并全量重建向量索引」
- 初始化页（reception.rs）：两步表单，Step 1 基础信息，Step 2 模型配置（对话/向量模型均可选，未配置时跳过并提示后果）；Embedding 创建时跳过连接测试（异步初始化环境可能无网络）。
