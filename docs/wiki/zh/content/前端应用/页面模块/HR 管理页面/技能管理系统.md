...
- [frontend/src/pages/hr/skills.rs](frontend/src/pages/hr/skills.rs) — 技能库列表页（树形 + 分页 + 默认仅 Published + agent tag 分组）
- [frontend/src/api/hr.rs](frontend/src/api/hr.rs) — 前端 HR API 封装
</cite>
...
#### 2026-09-01 增量：Expired 技能独立展示区 + Agent 入职包同步前端视角
- **Expired 技能独立展示区**：Agent 详情页的技能面板拆分为两个 tab——"已安装技能"和"已过期技能"。已安装技能 tab 继续调用 list_for_agent（后端保持 exclude Expired 语义），已过期技能 tab 新增调用 list_expired_agent_skills 拉取 Expired 状态副本。每个 Expired 技能卡片叠加橙光 badge + 恢复按钮，点击后调用 restore_skill API（后端仅允许 Expired→Draft 单向迁移，非 Expired 状态返回 409 Conflict 被前端 toast 提示）。
- **Agent 入职包同步前端感知**：前端 Agent 创建流程不变，但后端 create_agent 内部已从 5 次串行 install_skill_pack 改为 1 次 sync_agent_packs 两阶段同步。这意味着创建新 Agent 后等待的时间略减（单次 Domain 调用内批量 upsert 而非 5 次独立调用），且不会再因某个 tag 的幂等跳过导致整体流程中断。前端无需适配变更，但技能安装状态的刷新时机更可预测。
- **install_to_agent 原地更新的前端可见性**：当用户重新安装同一技能包到已持有的 Agent 时，后端不再创建新副本（避免 UNIQUE 冲突），而是原地更新内容。前端 Skill 详情页下次刷新时自动展示最新 Published 内容，无需额外提示"已重装"。
...
### 本文关联的文档
- 🎴 RAG 卡: docs/wiki/knowledge/zh/Skill 系统增强：5 套 TEMPLATE 预置包 + install_skill_pack 幂等 Tag 分发 + Agent 入职绑定 + Prompt Token 熔断/Skill 系统增强：5 套 TEMPLATE 预置包 + install_skill_pack 幂等 Tag 分发 + Agent 入职绑定 + Prompt Token 熔断.md
- 🎴 RAG 卡: docs/wiki/knowledge/zh/Agent 关联全景与工具技能分组装配：三分组互斥去重 + 专业领域打包复用 + 按需装配/Agent 关联全景与工具技能分组装配：三分组互斥去重 + 专业领域打包复用 + 按需装配.md

---

### 更新摘要（2026-09-10，base d070a076→HEAD）
**主题**：技能库列表页增强——树形 + 分页 + 默认仅 Published + agent tag 分组
**关键变更**：
1. 技能库列表页（frontend/src/pages/hr/skills.rs）从扁平列表升级为分类树形结构——按 category 分组可折叠展开
2. 新增分页支持——后端 search_skills 返回 PagedResult，前端支持 page/page_size 参数
3. 默认过滤仅展示 Published 技能——Draft/Expired 需显式筛选（管理员可切换全部状态）
4. agent tag 分组——技能详情页显示该技能关联的所有 Agent（按 installed_skill_packs tag 匹配）
5. 前端 HR API（frontend/src/api/hr.rs）封装 search_skills / query_skills / list_for_agent 等接口
**涉及 RAG 卡**：Skill 系统增强卡、Agent 关联全景与工具技能分组装配卡