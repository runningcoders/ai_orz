...
- 边样式：
  ...
  - 分组按**展示标签**匹配（`zh_label_from_display`：词表内 → 中文「包含/依赖/导致…」，词表外 → 原文原样）。命中不了词表的自定义关系走中性灰、实线 —— 这是**着色兜底**，不是把类型改成「自定义」
  - 边标签背景框提高可读性
- HUD 主题：
...
### 第六轮（同日）：层级语义对齐 —— 不画端点缺失的边

对齐后的语义：**层级是节点维度的概念**，`traversal_depth = N` 表示「与种子相距 ≤ N 跳的可见节点」，
边不是层级实体，只是连接两个已在结果里的节点的线。因此前端**不再为缺失端点造占位节点**。

| 位置 | 旧行为（问题形态） | 新行为 |
|---|---|---|
| `build_graph_from_results` 关系分支 | 端点不在本批结果时，补一个 `label = "未命名节点"`、`description` / `summary` 全空的占位卡 | 端点缺失（`name_by_id` 查不到）→ **整条边不画**，也不补节点 |
| `node_display_name` | 「未命名节点」既兜底空字段、又承担端点占位 | 只兜底「实体节点本身名称/摘要/正文全空」这一种情况 |
| 边集合 | 可能引用批外节点，画布上出现一屏无内容的卡片 | 每条边两端都有实体节点，卡片一定有名称/正文/hover 内容 |

这是**第二道防线**：后端 `drop_dangling_relations` 已在 `search` / `traverse_knowledge_graph` 两条路径
上单点收敛同一不变式，前端这里保证「即使后端漏了，也画不出假节点」。

**章节来源**
- [frontend/src/pages/hr/knowledge_graph.rs](frontend/src/pages/hr/knowledge_graph.rs)（`build_graph_from_results` / `node_display_name`）
- [src/service/dal/memory.rs](src/service/dal/memory.rs)（`drop_dangling_relations`）

### 第七轮（同日）：蜂巢可见性 —— 中心节点恒在，Agent 选择器退为「起点筛选」

用户拍板的语义：**知识节点是蜂巢共享资产，任何 Agent 都能看到全部知识节点**，
`published` 只是「重要性 / 影响力」标记，不再决定可见性。前端随之调整三点：

| 位置 | 旧行为（问题形态） | 新行为 |
|---|---|---|
| 点击节点展开 | 后端按归属过滤节点、边不受约束 → 中心节点缺席 / 整屏占位卡 | 后端遍历**不施加归属筛选**，**种子（中心节点）恒返回**；本条路径的 `agent_id` 已无约束力 |
| Agent 选择器 | 选/不选决定「能看到哪些节点」（留空 = 只看 `published`） | 退化为**起点筛选**：只影响关键词搜索与推荐起点的选取，**不约束**沿图展开 |
| 详情面板回填 | 中心节点可能不在返回结果里，点击后详情为空 | 遍历结果一定含种子 → `detail_map` 能回填中心节点详情 |

记忆搜索页的「关系」列表同样受益：关键词未命中两端时不再出现只有一半信息的边（DAL 单点闸生效）。

**章节来源**
- [frontend/src/pages/hr/knowledge_graph.rs](frontend/src/pages/hr/knowledge_graph.rs)（`handle_node_click` / Agent 选择器）
- [src/service/dal/memory.rs](src/service/dal/memory.rs)（`traverse_knowledge_graph` / `private_agent_scope`）
