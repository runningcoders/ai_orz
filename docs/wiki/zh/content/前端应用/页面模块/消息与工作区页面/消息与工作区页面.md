...
**主题**：工作台底部横幅重构为 MMORPG 式三段 HUD 布局
**关键变更**：
1. **工作台底部横幅三段布局（frontend/src/pages/workspace.rs）**——将原来简陋的单条底部横幅拆解为「左上状态锚点 / 中间对话入口 / 右下工具提示」三个功能分区，Tailwind `flex justify-between items-end` + HudCallout/HudCard 容器实现，呼应 MMORPG 游戏 UI 的 HUD 风格；
2. **工作台相关同步弹窗 Loading 组件统一**——加载状态改用 DaisyUI `loading loading-spinner` / `loading loading-dots`，不再自定义 spinner；
3. **Agent 列表页同步弹窗（frontend/src/pages/hr/agents.rs）**——预置 Agent 同步进度指示同样统一为 Loading 组件。
