...
- 三线趋势图：分钟级 input/output tokens + call_count，HudPalette 三色（主/次/第三），320x180 紧凑 Canvas 尺寸
- 独立拉取：共享轮询主链路不带 stats 参数（零额外开销），统计仅在 Agent Tab 挂载时触发 get_agent（with_stats=true + with_model_call_stats=true + stats_interval=daily）
...
- **请求参数**：`GetAgentRequest { id, with_stats: Some(true), with_model_call_stats: Some(true), stats_interval: Some("daily") }`
