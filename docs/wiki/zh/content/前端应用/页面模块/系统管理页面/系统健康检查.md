...
- 信道监听运行态：Gauge + 明细表展示两类入站监听——**飞书 WS 连接**（服务端推，per-app `connecting/connected/reconnecting` + 累计重连）与**微信 iLink 长轮询**（客户端拉，per-channel 轮次/累计入站/连续失败/客户端超时/最近成功轮询/已确认游标）。
  - ⚠️ 两者判活口径**不同**：WS 断连会反映到连接阶段；长轮询不会——任务卡死时句柄仍在册、`active_polls` 照样是 1。微信侧靠 `rounds` / `last_poll_at_ms` 是否推进判活（正常约 35s 一轮，>90s 未成功轮询标记「疑似卡死」）。
  - `client_timeouts` 增长而 `last_poll_at_ms` 不动 = 网络 hang；两者都不动 = 循环卡死。

**章节来源**
- [frontend/src/pages/system/health.rs:1-197](frontend/src/pages/system/health.rs#L1-L197)
