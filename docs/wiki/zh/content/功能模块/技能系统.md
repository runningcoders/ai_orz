...
**本文关联三类文档（四类互引闭环）**
- 【① Design 决策快照】
  - [skill_design.md](docs/design/skill_design.md) — skills 表设计 + 文件相对路径管理 + 三状态机（待沉淀/可用/过期）软删除
  - [skill_system_enhancement_design.md](docs/design/skill_system_enhancement_design.md) — tag 过滤 + 技能包唤醒注入机制
- 【② Plan 落地快照】
  - [预置基础技能导入重构.md](docs/plan/预置基础技能导入重构.md) — 5 套 TEMPLATE_* 目录 + default.json 编译期嵌入 include_str!
- 【④ RAG 原子知识卡】
...
- [src/handlers/hr/skill/uninstall_skill_from_agent.rs](src/handlers/hr/skill/uninstall_skill_from_agent.rs)
- [src/handlers/hr/skill/update_skill_file_content.rs](src/handlers/hr/skill/update_skill_file_content.rs)
- [docs/superpowers/plans/2026-07-30-tool-skill-search-install.md:1-1570](docs/superpowers/plans/2026-07-30-tool-skill-search-install.md#L1-L1570)
...
#### ① Design 决策快照
- [skill_system_enhancement_design.md](docs/design/skill_system_enhancement_design.md) — 6 条决策：SQL 层 json_each tag 过滤 / 技能包卸载不删副本 / 安装幂等 / content_hash 重装覆盖 / Token 熔断只注入摘要不注入 skill.md
#### ② Plan 落地快照
- [Agent管理集成测试.md](docs/plan/Agent管理集成测试.md) — Follow-up Skill 向量测试 9 个（CRUD + FTS5 + 向量 + 混合排序 + 删除索引清理）
#### ④ RAG 原子知识卡
