...
- 更新与删除
  - 更新：PUT /api/v1/model-providers/{id}，支持部分字段更新；仅使用中(Normal) Embedding 的 model_name/api_key/base_url 变化触发重建；Disabled 编辑不触发（启用时 switch 全量重建兜底）。
...
- 编辑 Normal Embedding 配置变化 → 触发重建；编辑 Disabled → 不触发
