...
### 本文关联的文档
- 🎴 RAG 卡: docs/wiki/knowledge/zh/Embedding Provider 生命周期：ModelProviderStatus Disabled(2) + 创建不阻塞策略 + 重建触发条件矩阵/Embedding Provider 生命周期：ModelProviderStatus Disabled(2) + 创建不阻塞策略 + 重建触发条件矩阵.md

---

### 更新摘要（2026-09-11，base ad54411c→HEAD）
**主题**：对话模型 context_length 必填 + 初始化/seed 同步补齐
**关键变更**：
1. **DTO 新增 context_length 字段**——common/src/api/model_provider.rs CreateModelProviderRequest 新增 context_length: Option<u32>
2. **创建对话模型时强制校验**——ContextKind::Chat 创建时必须填 context_length（缺失 400 `context_length_required`）；Embedding Provider 可空
3. **更新时可选更新**——update_model_provider 支持 partial update 三态
4. **种子导入 diff 同步校验**——src/service/domain/system/seed/diff.rs 对话模型 context_length 必填
5. **前端模型管理页 + 初始化页**——model_providers.rs + reception.rs 表单新增 context_length 字段输入
6. **Agent 运行时消费**——context_length 字段作为 AgentRuntimeState.context_threshold 数据源，驱动前端 RingProgress 环形进度组件展示上下文 Token 占比
**涉及 RAG 卡**：Embedding Provider 生命周期卡 + 种子配置与系统两阶段初始化卡 + 思考运行时前端观测卡