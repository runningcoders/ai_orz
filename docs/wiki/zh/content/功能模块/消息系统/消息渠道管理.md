...
- 【④ RAG 原子知识卡】
  ...
  - [邮箱渠道全链路：SMTP 出站 + IMAP 入站 + EmailBot 身份凭证类型](docs/wiki/knowledge/zh/邮箱渠道全链路：SMTP%20出站%20+%20IMAP%20入站%20+%20EmailBot%20身份凭证类型/邮箱渠道全链路：SMTP%20出站%20+%20IMAP%20入站%20+%20EmailBot%20身份凭证类型.md) — SMTP 出站 relay/starttls 自动切换 + IMAP 长轮询凭证键控启停 + EmailBot 凭证 AES-256-GCM 加密
- 【③ Wiki 关联长文】
...
EmailBot 是通过普通邮箱账号作为 AI Agent 消息收发代理的渠道：
- **出站**：SMTP（lettre crate）——端口 465 隐式 TLS relay / 其余 STARTTLS starttls_relay；主题从正文派生（MessagePo 无 title 字段，对齐飞书/微信全文推送语义）
- **入站**：IMAP 长轮询（凭证键控——一个 EmailBot 可被 N 个渠道共用，按 credential_id 而非 channel_id 启停）；UID SEARCH UNSEEN + FETCH；IN 列表 400 分块防 999 溢出
- **凭证**：CredentialDetail::EmailBot{email_address, smtp_host, smtp_port, imap_host, imap_port, username, password(加密)}；password 通过 AES-256-GCM 加密存储
- **自动启停**：MessageChannel 创建时若配置了 email_credential_id → EmailDal 自动 start_polling；删除/更新时同步 stop_polling
- **探活阶段一无廉价探针**：SMTP 没有 connect/auth/quit 三阶段（lettre 不单独暴露），test_connection 仅做参数完整性校验；真实连通性由首次 push 运行态观察
- 源码锚点：src/service/dao/email/（smtp.rs + imap.rs）+ src/service/dal/email/ + src/consumer/email_inbound.rs

**章节来源**
- [dao/email/mod.rs:1-47](src/service/dao/email/mod.rs#L1-L47)
- [dal/email.rs:1-80](src/service/dal/email.rs#L1-L80)
...
- 邮箱渠道（EmailBot）专属排查：
  - SMTP 出站失败：检查 smtp_host/smtp_port/username/password 配置是否正确；端口 465 走隐式 TLS relay，其他端口走 STARTTLS starttls_relay，确认与服务器实际配置匹配
  - IMAP 入站无新消息：确认 credential_id 关联的凭证是否启用、IMAP 服务器是否可达；IN 列表 999 以上 UID 会 400 分块拉取，注意日志里是否出现 overflow 警告
  - test_connection 总是通过：SMTP 无廉价探针阶段，探活只做参数完整性校验；真实连通性需观察首次 push 运行态错误日志
  - IMAP 轮询意外停止：检查是否有其他渠道删除/更新导致 stop_polling 被触发（按 credential_id 而非 channel_id 启停，一删则全停）
  - 凭证密码解密失败：EmailBot 的 password 字段走 AES-256-GCM 加密，确认加密密钥版本与 migrate 时一致
...
- [identity_credentials.rs:54-77](common/src/models/identity_credentials.rs#L54-L77)
- [dao/email/imap.rs:1-120](src/service/dao/email/imap.rs#L1-L120)
