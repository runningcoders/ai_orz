...
- 【① Design 决策】[docs/archive/design-archive/agent_loop_engine_design.md](docs/archive/design-archive/agent_loop_engine_design.md) — Agent 自主跟进任务进度 + 补偿巡检 + Owner Agent 调度
- 【① Design 决策】[docs/archive/design-archive/message_interaction_design.md](docs/archive/design-archive/message_interaction_design.md) — 用户-Agent 多角色调度 + 项目上下文路由隔离
- 【② Plan 落地】[docs/archive/plan-archive/聊天MVP.md](docs/archive/plan-archive/聊天MVP.md) — 对话页 3s 轮询 + 双向分页（before/after timestamp）
- 【② Plan 落地】[docs/archive/plan-archive/agent_loop_engine_plan.md](docs/archive/plan-archive/agent_loop_engine_plan.md) — 消息驱动 Agent 唤醒链路 + 事件+定时双链路
...
- [migrations/20260909000004_add_external_key_to_messages.sql](migrations/20260909000004_add_external_key_to_messages.sql) — external_key 话题关联字段新增
- [src/models/message.rs](src/models/message.rs) — MessagePo 新增 external_key 字段
- [src/service/domain/runtime/awakening.rs](src/service/domain/runtime/awakening.rs) — Agent 唤醒注入 reply_to 上下文
- [src/service/dal/lark/impl.rs](src/service/dal/lark/impl.rs) — 飞书 thread_id ↔ external_key 映射
- [src/service/dao/lark/http.rs](src/service/dao/lark/http.rs) — 飞书 API 调用（thread_id/reply_in_thread）</cite>
</cite>
...
**主题**：消息回复链 + external_key 话题关联 + 飞书 thread_id 映射 + 唤醒注入 reply_to 上下文
**关键变更**：
1. reply_to 回复链完善——MessagePo.reply_to_id + root_id 构建完整回复层级，支持按根拉取整链
2. 新增 external_key 字段——关联外部系统话题（如飞书 thread_id / 微信 group_id），跨渠道同话题消息聚合
3. 飞书 thread_id ↔ external_key 双向映射——lark/impl.rs 自动将飞书 thread_id 存入 external_key，reply_in_thread 出站时取 external_key 作为 thread_id
4. Agent 唤醒注入 reply_to 上下文——awakening.rs 在触发 Agent 执行时携带 reply_to 消息上下文，保持对话连贯性
5. 新增 migration 20260909000004_add_external_key_to_messages.sql
**涉及 RAG 卡**：消息交互与SSE推送卡

---

### 更新摘要（2026-09-11，base ad54411c→HEAD）
**主题**：聊天引用块即时显示 + 宽度以本条消息为上限
**关键变更**：
1. **引用块即时渲染**——chat.rs 消息列表中带 reference_id 的消息，气泡上方即时渲染引用块（被引用的消息摘要），无需 hover 才弹出
2. **宽度以本条消息为上限**——引用块 CSS max-width: 100% + overflow: hidden + text-overflow: ellipsis，不再溢出撑破消息列表布局
3. **前端 chat_side_panel.rs 同步适配**——侧栏消息列表同样遵循引用块即时显示 + 宽度约束
**涉及 RAG 卡**：Canvas HUD 可视化卡 + 消息交互与SSE推送卡