...
### 首次初始化流程（后台任务）
首次初始化由HTTP接口触发，创建一个自包含进度的后台任务，分步执行。步骤数动态计算：`total_steps = 3 + usize::from(chat_model.is_some()) + usize::from(embedding_model.is_some())`

- Step 1：创建组织与超级管理员
- Step 2（可选）：创建对话模型提供商
- Step 3（可选）：创建向量模型提供商
- 最后：同步内置工具到数据库 + 导入预置技能

**前端分两步表单**：Step 1 基础信息（组织名/用户名/密码）；Step 2 模型配置（对话/向量模型均可选，未配置时跳过并提示后果）。

```mermaid
sequenceDiagram
participant FE as "前端页面<br/>reception.rs（两步表单）"
...
FE->>API : 提交初始化请求（chat_model/embedding_model 均为 Option）
...
alt 提供向量模型
T->>T : set_step(2或3, "配置向量模型")
T->>Fin : create_model_provider(embedding)
end
T->>T : set_step(最后, "同步内置工具")
T->>Fin : sync_builtin_tools()
T->>T : set_step(最后, "导入预置技能")
T->>Seed : apply_preset_skills()
T-->>API : 返回结果：org_id + user_id + chat_provider_id(Option) + embedding_provider_id
```
...
- 默认Provider：对话模型提供商**可选**（`chat_model: Option`），未配置时响应中 `chat_provider_id` 为 null；向量模型提供商**可选**，未配置时跳过并提示「语义搜索将不可用，需在系统初始化后手动配置」。
- **新增**：内置工具与技能：初始化过程中会同步内置工具并导入预置技能，确保Agent具备开箱即用的能力。
- **增强**：内置工具同步支持所有权分界，代码定义字段自动更新，运维配置字段保持不变。
- **初始化响应**：`InitializeSystemResponse.chat_provider_id` 为 `Option<String>`，未配置时返回 null，不得返回空字符串。
