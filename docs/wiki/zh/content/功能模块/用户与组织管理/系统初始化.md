...
- 【Batch10 追加】[seed-config-migration.md](docs/design/seed-config-migration.md) — Seed 工具箱 domain 架构；5 套 TEMPLATE_SKILL 编译期嵌入（include_str!）；SeedSnapshot Diff 增量导入；两阶段初始化严格顺序（pkg::init_all → service::init → producer/consumer::init → init_base_data → aop::start）；init_all_base_data 域派发模式

**② 落地计划（Plan）（Batch10 追加）**：
- [Agent管理集成测试.md](docs/plan/Agent管理集成测试.md) — init_full_test_env 启动顺序 6 步与真实启动严格对齐；Consumer::init 只注册订阅者禁止写 DB
