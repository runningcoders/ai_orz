...
- [src/service/dal/organization/mod.rs](src/service/dal/organization/mod.rs)
- [src/service/dal/organization/impl.rs](src/service/dal/organization/impl.rs)
- [src/service/dal/organization/link.rs](src/service/dal/organization/link.rs)
- [src/service/dal/organization/pairing.rs](src/service/dal/organization/pairing.rs)
...
- **2026-09-08 更新摘要**：DAL 层子模块拆分——`dal/organization.rs` → `dal/organization/{mod.rs,impl.rs,link.rs,pairing.rs}`，学习 `agent/`、`lark/` 目录模式，拆为 `OrganizationDal` 总 trait（组织 CRUD + 影子同步 + 联邦委派）+ `OrganizationLinkDal`（links CRUD + 联邦出站 HTTP）+ `OrganizationPairingDal`（配对码 CRUD）。
- **2026-09-10 更新摘要**（base d070a076→HEAD）：签名架构升级——organization_links 表删除 access_token/peer_token_hash 两列，新增 peer_did/peer_verification_key 两列。WS 长连接握手复用每请求签名链路。capabilities 从 link.capabilities JSON 下沉到 federation_contracts 独立表。
- [src/middleware/federation_identity.rs:1-L117](src/middleware/federation_identity.rs#L1-L117) — 联邦身份解析纯函数（HTTP + WS 共用）
- [src/pkg/crypto/did.rs:1-L80](src/pkg/crypto/did.rs#L1-L80) — did:key 生成与解析（配对时签发）
- [src/pkg/crypto/mod.rs:1-L50](src/pkg/crypto/mod.rs#L1-L50) — 签名基建模块
- [src/pkg/nonce.rs:1-L40](src/pkg/nonce.rs#L1-L40) — nonce 防重放（WS 握手跳过，连接级去重）
- [src/models/federation_contract.rs:1-L80](src/models/federation_contract.rs#L1-L80) — 联邦合约数据模型
...
- [migrations/20260909000002_federation_signature_upgrade.sql](migrations/20260909000002_federation_signature_upgrade.sql) — 删除 access_token / peer_token_hash 两列
- [migrations/20260909000003_federation_contracts.sql](migrations/20260909000003_federation_contracts.sql) — federation_contracts 表创建
</cite>
...
- **organization_links 连接契约表**：独立表承载 endpoint + peer_did（did:key 形式）+ peer_verification_key（原始 Ed25519 公钥）+ status + 唯一约束 (local_org_id, peer_org_id)。**2026-09-10 架构升级**：旧版 access_token（明文出站）/ peer_token_hash（SHA256 入站校验）两列已删除，改为 did:key + Ed25519 每请求签名模式。
- **organization_pairing_codes 配对码表**：短时效（10 分钟）单用途握手凭证。仅存 SHA256 code_hash（明文不出库）。consume 原子完成哈希匹配 + 未消费 + 未过期 + 置 consumed_at 四判定。
- **federation_contracts 联邦合约表**（2026-09-10 新增）：与 organization_links 1:N 关系，每条合约记录 contract_type + capabilities（JSON 数组）+ status（Active / Terminated / Expired）+ terminated_at。capabilities 从旧 link.capabilities JSON 字段下沉至此表，支持 terminate 熔断终止。
...
### DAL 层（`dal/organization/` 三模块拆分）

学习 `dal/agent/`、`dal/lark/` 目录模式，Organization DAL 拆为总 trait + 两个子 DAL：

- **`OrganizationDal`（[mod.rs:L1-L177](src/service/dal/organization/mod.rs#L1-L177) + [impl.rs](src/service/dal/organization/impl.rs)）**：总 trait，管组织 CRUD + 影子同步（`upsert_remote_shadow` / `upsert_linked_shadow` / `revoke_link`）+ 联邦委派传输（`send_federated_agent_task` / `resolve_peer_endpoint`）。实现体在 `impl.rs`，持有 `OrganizationDao` + `OrganizationLinkDao` 两个 DAO。
- **`OrganizationLinkDal`（[link.rs:L1-L29](src/service/dal/organization/link.rs#L1-L29)）**：子 DAL，管 `organization_links` 表的 CRUD + 联邦出站 HTTP（组合 `FederationHttpClient`，Domain 不再直捅 HTTP 客户端）。
- **`OrganizationPairingDal`（[pairing.rs:L1-L30](src/service/dal/organization/pairing.rs#L1-L30)）**：子 DAL，管 `organization_pairing_codes` 配对码表（签发、原子 consume、过期清理）。

Domain 层消费方通过 `dal::organization::OrganizationLinkDal::dal()`、`dal::organization::OrganizationPairingDal::dal()` 拿子 DAL 单例，总组织数据访问仍走 `OrganizationDal::dal()`。
...
章节来源
- [org.rs:L1-L1294](src/service/domain/organization/org.rs#L1-L1294)
- [dal/organization/mod.rs + impl.rs](src/service/dal/organization/mod.rs) — OrganizationDal 总 trait + 实现
- [dal/organization/link.rs](src/service/dal/organization/link.rs) — OrganizationLinkDal
- [dal/organization/pairing.rs](src/service/dal/organization/pairing.rs) — OrganizationPairingDal
...
| 日期 | 变更 | 说明 |
|------|------|------|
| 2026-09-05 | 联邦组网 Phase 1+2 全量落地 | scope 三态 + organization_links + pairing_codes + WS 长连接 + /a2a 双模鉴权 |
| 2026-09-08 | DAL 层子模块拆分（`dal/organization.rs` → `dal/organization/mod.rs` + `impl.rs` + `link.rs` + `pairing.rs`） | 学习 `dal/agent/`、`dal/lark/` 目录模式：将单文件 500+ 行拆为总 trait + 实现体 + 两个子 DAL（`OrganizationLinkDal` 管 links CRUD + 联邦出站 HTTP，`OrganizationPairingDal` 管配对码 CRUD）。Domain 层消费方通过 `OrganizationLinkDal::dal()` / `OrganizationPairingDal::dal()` 拿子 DAL 单例，`OrganizationDal::dal()` 保留组织 CRUD + 影子同步 + 联邦委派传输职责 |
| 2026-09-10 | 签名架构升级 + federation_contracts 独立 | organization_links 表删 access_token/peer_token_hash，新增 peer_did/peer_verification_key（3 个 migration）。WS 长连接握手复用每请求签名链路（nonce 握手成功后连接级跳过）。capabilities 从 link.capabilities JSON 下沉到 federation_contracts 独立表，支持 terminate 熔断 |

**拆分动机**：`dal/organization.rs` 随联邦能力叠加膨胀到 500+ 行，links/pairing 两个子领域各自有独立的 DAO 组合（`OrganizationLinkDao` + `FederationHttpClient`、`OrganizationPairingDao`），切为独立子 DAL 后 Domain 层依赖更内聚、测试隔离更干净。同时为后续可能的 User-related 操作进一步移出预留了模块边界。
...
- 🎴 RAG 卡: docs/wiki/knowledge/zh/联邦组网地基：scope 三态 + organization_links + pairing_code + 目录同步 + WS 长连接/联邦组网地基：scope 三态 + organization_links + pairing_code + 目录同步 + WS 长连接.md
...
**主题**：联邦组网签名架构升级 + federation_contracts 合约表独立
**关键变更**：
1. organization_links 表结构变更——删 access_token/peer_token_hash 对称凭证列，新增 peer_did/peer_verification_key（Ed25519 公钥）两列
2. WS 长连接握手复用每请求签名 5 步链路（nonce 握手成功后连接级跳过，避免每帧重复校验）
3. capabilities 从 organization_links.capabilities JSON 字段下沉到独立 federation_contracts 表，支持 1:N 合约 + terminate 熔断
4. 新增 pkg::crypto/mod.rs + did.rs（did:key 配对时签发）+ nonce.rs（防重放）
5. 新增 3 个 migration（20260909000001_identity_keys / 20260909000002_signature_upgrade / 20260909000003_federation_contracts）
**涉及 RAG 卡**：联邦组网地基卡、跨组织业务调用鉴权模型卡、联邦合约授权卡
