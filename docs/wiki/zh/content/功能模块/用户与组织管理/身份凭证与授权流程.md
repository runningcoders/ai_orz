...
- **[协议 SSOT]** 协议常量 / `base_info` 信封 / 请求头档位集中在此（依据官方插件 `@tencent-weixin/openclaw-weixin@2.4.9`）
- `get_login_qrcode(local_token_list)` — **`POST {base}/ilink/bot/get_bot_qrcode?bot_type=3`**，body `{local_token_list}`（本用户已有 bot token，最新在前 ≤10）。`local_token_list` 是重绑的前提；此接口 POST 但**不带 `Authorization`**（扫码前无令牌）
- `poll_qrcode_status(qrcode, verify_code, redirect_host)` — `GET {base}/ilink/bot/get_qrcode_status?qrcode=xxx[&verify_code=yyy]`，只带通用头两件套（`iLink-App-Id` + `iLink-App-ClientVersion`）→ 45s 长轮询（服务端 hold ~35s）。`redirect_host` 经**白名单校验**后才拼成 `https://{host}`（只接受腾讯接入域内的裸主机名）
- `IlinkQrStatusKind` **官方 8 态**：Wait / Scaned / Expired / Confirmed / ScanedButRedirect / NeedVerifyCode / VerifyCodeBlocked / BindedRedirect（字面量 SSOT 在 `common::api::WECHAT_QR_STATUS_*`，前后端共用）
- confirmed 时返回 bot_token（敏感）/ bot_id / user_id（可选）/ base_url（缺省回落 ILINK_DEFAULT_BASE_URL）；`binded_redirect` 是**幂等成功**（该 bot 早已绑过，不签发新凭据、不写库）

**Handler 层（handlers/finance/wechat_integration/）**
- `POST /api/v1/finance/identity/wechat/qrcode` — get_login_qrcode
- `GET /api/v1/finance/identity/wechat/qrcode/status?qrcode=xxx[&verify_code=][&redirect_host=]` — login_status（长轮询，confirmed 时调 Domain 落库）
- `GET /api/v1/finance/identity/wechat/status` — 当前用户 iLink 凭证快照聚合
...
sequenceDiagram
    ...
    User->>FE: 点"扫码绑定微信"
    FE->>Handler: POST /qrcode
    Handler->>Pkg: get_login_qrcode(local_token_list)
    Pkg->>Pkg: POST get_bot_qrcode?bot_type=3
    Pkg-->>Handler: { qrcode, qrcode_img_content }
    Handler-->>FE: 返回二维码内容
    FE->>User: 展示二维码 + 备用授权链接（并开始自动长轮询）

    User->>Pkg: 微信扫码 + 手机确认
    FE->>Handler: GET /qrcode/status?qrcode=xxx（自动循环长轮询）
    Handler->>Pkg: poll_qrcode_status(qrcode, verify_code, redirect_host)
...
两接口在扫码前**无令牌可鉴权**（配置面操作），但请求头档位不同：

- `POST {base}/ilink/bot/get_bot_qrcode?bot_type=3`，body `{ local_token_list: [...] }` → 返回 `qrcode`（轮询标识）+ `qrcode_img_content`（**既是二维码渲染内容，本身也是一条可在手机微信直接打开的授权链接**）。请求头 = 通用两件套 + `AuthorizationType` + `X-WECHAT-UIN`，**不带 `Authorization`**
- `GET {base}/ilink/bot/get_qrcode_status?qrcode=<urlencoded>[&verify_code=<urlencoded>]`，请求头只带通用两件套（`iLink-App-Id` + `iLink-App-ClientVersion`，由我方版本号按 `0x00MMNNPP` 编译期算出）→ 45s 长轮询
- 状态为**官方 8 态**：`wait` / `scaned` / `confirmed` / `expired` / `scaned_but_redirect` / `need_verifycode` / `verify_code_blocked` / `binded_redirect`

**超时宽容**：客户端 45s > 服务端 hold ~35s → 客户端先到超时被视为本轮无事件（返回 Wait），不是网络错误。调用方把超时视为正常状态。

**会话态由前端持有**：`verify_code`（配对码）与 `redirect_host`（IDC 重定向接入点）都是服务端会话态，与 `qrcode` 同理由前端保存并逐轮回传——后端保持无状态。`redirect_host` 在拼成出站 URL 前必须校验为**腾讯接入域内的裸主机名**。

**confirmed 凭据获取**：bot_token / ilink_bot_id 必填（confirmed 响应缺这两个字段 → 校验报错）；ilink_user_id 可选；baseurl 缺省时回落 `ILINK_DEFAULT_BASE_URL`。`binded_redirect` 不携带凭据，语义是"早已绑过本客户端"，按**成功**处理且不写库。
