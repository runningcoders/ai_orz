...
- [src/handlers/system/seed/sync_preset_skills.rs](src/handlers/system/seed/sync_preset_skills.rs)
- [src/handlers/system/seed/sync_preset_agents.rs](src/handlers/system/seed/sync_preset_agents.rs) — 2026-09-12 新增：预置 Agent 一键同步 handler（273 行），三策略 Overwrite/OnlyMissing/RestoreDeleted + 模型绑定排除
- [common/src/api/seed.rs](common/src/api/seed.rs) — 2026-09-12 追加 SyncPresetAgentsRequest/Response + PresetAgentSyncStrategy 枚举
- [src/router.rs](src/router.rs) — 2026-09-12 preset-agents 路由组注册
...
**更新摘要 2026-09-07**：新增预置技能同步接口——`GET /api/v1/system/seed/preset-skills/preview`（预览影响清单：seed vs 技能库逐技能对比 + 已安装副本数量统计）与 `POST /api/v1/system/seed/preset-skills/sync`（后台任务同步）。两种策略：Overwrite（覆盖重置同 ID 技能元数据 + 文件）/ OnlyMissing（仅补缺）。可选 `sync_installed_copies`：把 parent_skill_id 指向预置技能的 Agent 私有副本一并对齐（副本 Draft 状态保持不变）。匹配键是技能 ID（`TEMPLATE_*` / `GIT_BRANCH_WORKFLOW`）。同步走 `pkg::background_task` 通用后台任务，前端轮询 `/system/tasks/{task_id}/progress`。DTO 扩展：`PresetSkillSyncStrategy` / `PreviewPresetSkillsRequest/Response` / `SyncPresetSkillsRequest/Response` 全部在 `common/src/api/seed.rs#L165-L237`。
### 本文关联的三类文档（四类互引闭环）
#### ① Design 决策快照
...
**主题**：默认模板补齐对话模型配置 + 敏感字段回填页面 + diff 校验增强
**关键变更**：
1. **对话模型 context_length 必填校验**——种子导入 diff 校验 + apply-default 默认模板两条路径，对话模型 Provider 必须带 context_length 字段（缺失 400）；default.json 更新为对话模型带 context_length + api_key: PENDING_INPUT 占位符
2. **新增 seed.rs 种子导入前端页**——支持上传 seed 快照 JSON + 导入状态展示 + apply-default 一键初始化
3. **新增 seed_sensitive_fields.rs 敏感字段回填页**——导入完成后引导用户回填 API Key 等敏感字段；彻底修复默认模板 apply 写入路径必然失败的历史 bug
4. **已存在实体免填凭据**——diff 导入时 Existing 状态实体 api_key 从 DB 继承，免除重新填写
**涉及 RAG 卡**：种子配置与系统两阶段初始化卡 + Embedding Provider 生命周期卡

---

### 更新摘要（2026-09-12，base cdc30c46→HEAD）
**主题**：default.json 新增招聘官 Agent + 2 套新预置技能模板 + reception 同名双身份包打通
**关键变更**：
1. **default.json 新增招聘官 Agent 种子**——agents 数组新增 hr_specialist 角色的招聘官 Agent，配合 Agent 招聘三阶段生命周期使用；
2. **TEMPLATE_AGENT_RECRUITMENT 模板**——Agent 招聘技能包（tag=agent_management），embedded.rs EMBEDDED_SKILL_FILES 从 6 套扩展到 8 套；
3. **TEMPLATE_USER_RECEPTION 模板**——用户接待技能包（tag=reception），作为「reception 同名双身份包」既在 Seed 模板存在，又作为 HR onboarding Handler 的默认技能包引用；
4. **reception 双身份打通**——Skill 模板与 Handler 共享 SkillTag::Reception 枚举常量，禁止任何一方硬编码字符串变体；
5. **install_default_skill_packs 顺序扩展**——追加 agent_management / hr_specialist / reception 三个角色技能包到默认安装序列。
**涉及 RAG 卡**：种子配置与系统两阶段初始化卡 + Skill 系统增强卡 + 组织权限与用户偏好卡

---

### 更新摘要（2026-09-12，base c632f4bf→HEAD）
**主题**：预置 Agent 一键同步新接口（三策略 + 模型绑定排除 + Agent 列表页一键无感升级）
**关键变更**：
1. **新增 sync_preset_agents.rs handler（273 行）**——提供三策略：Overwrite（覆盖重置同 ID Agent 元数据和技能包分配）/ OnlyMissing（仅补缺 DB 中缺失的预置 Agent）/ RestoreDeleted（恢复被用户误删的预置 Agent）；API 双端点：GET preview + POST sync；
2. **模型绑定彻底移出同步范围（Ref 67a7c7ec）**——无论选 Overwrite 还是其他策略，用户已配置好的 model_provider_id / model_name / runtime_config **永不被覆盖**；预置 Agent 同步只同步元数据（角色/描述/能力/soul）和技能包分配；
3. **Agent 列表页一键触发（frontend/src/pages/hr/agents.rs）**——新增预置 Agent 同步入口，配合 Loading 组件统一进度指示（Ref 6dbad9b2，不再自定义 spinner）；
4. **DTO 扩展**——common/src/api/seed.rs 追加 `PresetAgentSyncStrategy` 枚举 + `PreviewPresetAgentsRequest/Response` + `SyncPresetAgentsRequest/Response`，前端镜像在 frontend/src/api/seed.rs；
5. **路由注册（src/router.rs）**——preset-agents 路由组：GET preview + POST sync。
**涉及 RAG 卡**：种子配置与系统两阶段初始化卡 + UI Design System 组件设计系统卡