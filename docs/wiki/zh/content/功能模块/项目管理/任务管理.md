...
**本文关联三类文档（四类互引闭环）**
- 【① Design 决策快照】
  - [task_design.md](docs/design/task_design.md) — 任务实体模型 + 四态状态机 + dependencies 数组 DAG 语义
  - [project_design.md](docs/design/project_design.md) — 项目实体一对多任务关联 + owner_agent_id PMO 角色 + last_followup_at
  - [project_management_design.md](docs/design/project_management_design.md) — 任务状态上报 + 实时通知 + 项目巡检补偿三机制
- 【② Plan 落地快照】
  - [项目任务增强.md](docs/plan/项目任务增强.md) — execution_plan/result JSON schema + progress_summary 算法 + TaskGraph DAG
  - [后台任务管理页面与列表清理接口重构.md](docs/plan/后台任务管理页面与列表清理接口重构.md) — query 接口 + 通用 count + PagedResult 统一
- 【④ RAG 原子知识卡】
