...
**① 设计文档（Design）**：
- [项目领域架构设计](docs/archive/design-archive/project_design.md) — Project 一对多任务关联、进度追踪、软删除分层设计
- [项目管理增强设计](docs/archive/design-archive/project_management_design.md) — Project Domain 六能力整合 + 任务 DAG 依赖 + 产物元数据 + 进度实时计算

**② 落地计划（Plan）**：
- [项目任务增强](docs/archive/plan-archive/项目任务增强.md) — TaskGraph DAG 构建 + ArtifactManage + FetchOptions 按需注入模式
