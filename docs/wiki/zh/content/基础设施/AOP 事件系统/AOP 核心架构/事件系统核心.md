...
- 消费阶段：异步模式下 worker 拉取事件；`finish_consumption` 判定 Ack（成功/Discard）或 Nack（Retry），`Consumer::decide_retry` 判定 `RetryDecision`
...
- 频繁失败重试：关注 `decide_retry` 的判定；默认策略按永久错误码与 `DEFAULT_MAX_ATTEMPTS=8` 兜底 `Discard`
