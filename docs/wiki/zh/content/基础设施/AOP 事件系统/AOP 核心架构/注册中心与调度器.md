...
- 成功处理 → queue.ack 后从队列移除；失败 → `on_failed` 返回 `Retry` → queue.nack 重新入队，配合退避避免忙轮；`Discard` → queue.ack（仍回调 on_consumed）。
...
+on_failed(ctx, event, err, attempt) async RetryDecision
...
- 生产者 Producer
  ...
  - 收尾回调：`on_consumed`（成功/Discard 后）、`on_failed`（每次失败 → `RetryDecision`）。
