...
## 更新摘要
**变更内容**
- 兴趣声明由 `interested_events()` / `EventKind` 改为 `subscriptions()` 声明 `task.status_changed` 这一 `EventTopic`
- 删除 `ack` / `nack`；收尾由 `finish_consumption` 统一判定，按 topic 反查生产者回调
...
- 事件订阅：subscriptions() 声明 task.status_changed
...
```mermaid
graph TB
Parse["反序列化事件"] --> CheckStatus{"new_status == Completed?"}
