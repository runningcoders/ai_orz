...
- [src/service/domain/runtime/tool_execution_test.rs](src/service/domain/runtime/tool_execution_test.rs)
...
## 更新摘要
**变更内容**
- 兴趣声明由 `interested_events()` / `EventKind` 改为 `subscriptions()` 声明 `agent.tool.executed` 这一 `EventTopic`
- 删除 `ack` / `nack` 概念；同步消费者收尾由 `finish_consumption` 统一判定（无生产者回调时为零回调的纯通知）
...
D --> E["DailyJsonlWriter.append<br/>写入 {base}/tools/call_trace/YYYYMMDD.jsonl"]
...
- [src/handlers/finance/tool/query_tool_call_entries.rs:1-46](src/handlers/finance/tool/query_tool_call_entries.rs#L1-L46)
...
Cons->>Log : log_call(entry)
Log->>FS : append 到 YYYYMMDD.jsonl
FS-->>Log : 写入完成（不返回行号）
Log-->>Cons : Ok
Cons-->>AOP : 返回 Ok（收尾在 finish_consumption）
```
...
- 关键行为：
  - subscriptions() 声明单一 EventTopic（`agent.tool.executed`）。
...
```mermaid
graph TB
Parse["反序列化为 ToolExecEvent"] --> |成功| GetLogger["获取 ToolCallLogger 单例"]
Parse --> |失败| Err["返回内部错误"]
GetLogger --> Write["log_call(entry)"]
Write --> Done["返回 Ok"]
Err --> End["结束"]
Done --> End
```
...
- 存储布局：{base_data_path}/tools/call_trace/YYYYMMDD.jsonl，每行一个结构化条目。
  （2026-09-19 起不再按 `tool_id` 分目录：`call_id` 即一次调用的唯一身份，`tool_id` 只作为
  entry 字段参与查询过滤；历史数据由 `scripts/migrate_tool_call_trace.sh` 合并迁移。）
...
## 依赖关系分析
- ToolExecLogConsumer 依赖 AOP 框架（ConsumeMode、Consumer、EventTopic）与 ToolCallLogger。
...
- 存储空间管理：
  - 存储路径 {base_data_path}/tools/call_trace/，建议定期归档与压缩历史文件。
- 性能监控指标：
...
**章节来源**
- [src/pkg/tool_tracing/logger.rs:1-253](src/pkg/tool_tracing/logger.rs#L1-L253)
- [src/handlers/system/aop.rs:1-86](src/handlers/system/aop.rs#L1-L86)
- [src/consumer/aop_stats_collector.rs:1-307](src/consumer/aop_stats_collector.rs#L1-L307)
- [src/pkg/stats/tool_call.rs:1-65](src/pkg/stats/tool_call.rs#L1-L65)
...
**图表来源**
- [src/handlers/finance/tool/query_tool_call_entries.rs:1-46](src/handlers/finance/tool/query_tool_call_entries.rs#L1-L46)
- [src/service/domain/runtime/tool_execution_test.rs:745-781](src/service/domain/runtime/tool_execution_test.rs#L745-L781)
...
- 日志未写入：
  ...
  - 查看 tools/call_trace/ 目录下是否有 YYYYMMDD.jsonl 文件。
- 查询结果为空：
