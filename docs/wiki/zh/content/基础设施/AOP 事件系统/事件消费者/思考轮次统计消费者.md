...
## 更新摘要
**变更内容**
- 追加 T2（runtime 诊断三接口 Agent 可调用工具化）RAG 卡互引与对应长文关联
- AOP 生产者-消费者契约重构：兴趣声明改为 `subscriptions()`，删除 `EventKind` / `interested_events()` / `ack` / `nack`；收尾统一由 `finish_consumption` 按 topic 反查生产者（见 `src/pkg/aop/core/consumer.rs` / `registry.rs`）
- §5 exit_reason 归一化章节来源行号因 runtime_tools.rs 扩展导致漂移，降级为无行号范围引用
...
```mermaid
graph TB
Parse["反序列化为 ThinkRoundEvent"] --> Check{"total_tokens > 0 ?"}
Check -- 否 --> Skip["跳过本轮统计"]
...
- ThinkRoundStatsConsumer 依赖：
  - AOP 框架（Consumer trait、EventTopic、ConsumeMode）
