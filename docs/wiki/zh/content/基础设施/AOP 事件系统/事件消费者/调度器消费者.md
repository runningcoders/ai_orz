...
## 架构总览
调度器消费者的端到端流程如下：
- 启动阶段：SystemDomain 初始化时确保两条系统级默认触发器（agent_rest 每天 04:00 cron `0 4 * * *`、project_followup 每 1 小时）。
...
Cons->>Cons : 解析目标 Agent（payload.agent_id 或扫描 Active 短期记忆）
Cons->>Reg : publish(AgentSettleEvent) 逐个派发后立即返回
Note over Cons : 不在本线程沉淀（Sync 消费者跑在 cron poll 线程，长任务会堵住全部定时器）
...
- 动作分发：
  - agent_rest：**只派发不执行**——解析目标 Agent（payload 带 `agent_id` 则只派发该 Agent；缺省则扫描全库 `Active` 短期记忆、按 `agent_id` 去重出列表），逐个 `publish(AgentSettleEvent)` 后立即返回。真正的沉淀由 `agent.awakening` 消费者（`consumer/message.rs`，与 `message.created` **同一个消费者**）执行：两者同队列同 `order_key = agent_id`，所以沉淀在跑时发给该 Agent 的消息压根不出队（不失败、不重试、不刷日志）。**不要把沉淀再拆成独立消费者**——AOP 的 `order_key` 闸门按消费者隔离，拆开后同一 `agent_id` 会落在两条互不知晓的队列里，串行保证静默失效。
    - ⚠️ 不可改回同步调用 `settle_memory::load_and_settle`：本消费者是同步模式、跑在 cron 轮询线程里，一场沉淀是 LLM 往返（实测阻塞轮询数分钟）；且同步路径只能在 Agent 忙时「跳过」，而 Producer 随即将 `next_run_at` 推到下个 cron 点（日触发 = 次日）→ 一次跳过丢一天。详见 [Memory 系统增强 RAG 卡 §4 红线 18/19/20](docs/wiki/knowledge/zh/Memory%20系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+%20agent_rest%20每天%204%20点%20settle%20+%20load_and_settle%20向量去重合并/Memory%20系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+%20agent_rest%20每天%204%20点%20settle%20+%20load_and_settle%20向量去重合并.md)
...
Action --> |agent_rest| DoRest["解析目标 Agent + publish(AgentSettleEvent)（只派发，不沉淀）"]
...
- 初始化阶段确保两条系统级默认触发器（幂等）：agent_rest（每天 04:00，cron `0 4 * * *`）、project_followup（每 1 小时）。
