...
- 动作分发：
  - agent_rest：**只派发不执行**——解析目标 Agent（payload 带 `agent_id` 则只派发该 Agent；缺省则扫描全库 `Active` 短期记忆、按 `agent_id` 去重出列表），逐个 `publish(AgentSettleEvent)` 后立即返回。真正的沉淀由异步消费者 `agent_settle`（`order_key = agent_id`、`concurrency = 2`）执行，Agent 忙时抢占失败 → 队列 nack 重投，等空闲自动补跑。
