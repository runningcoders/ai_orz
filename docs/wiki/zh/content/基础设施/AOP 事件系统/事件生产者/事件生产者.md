...
- `mark_trigger_executed` **已从 tick 移入** `CronTriggerProducer::on_consumed`：消费者报成功后才标记触发器已执行；`executed_at` 取事件 `created_at`（≈ 触发时刻），而非回调时刻（避免 `next_run_at` 被越拖越后）。`CronTriggerConsumer::decide_retry` **恒 `Retry`**（`Discard` 会 ack 且仍回调 `on_consumed` → 反把未执行标成「已执行」）。

错误处理
- 若 Registry 未注册则返回内部错误。
- 事件发布失败由下游消费者在 `finish_consumption` 中经 `decide_retry` 判定重试/丢弃。
...
入站失败适配（P6）：三个入站消费者上报 `Err`，由 `Consumer::decide_retry(err, attempt)` 默认策略按错误码前缀判定 `Discard`（永久）/ `Retry`（否则），`attempt >= DEFAULT_MAX_ATTEMPTS(8)` 兜底 `Discard`。
...
- 实现 `Producer` trait：
  ...
  - `on_failed`：接收 `decision` 并据此更新底层数据（判定权在消费者的 `decide_retry`）；框架不设框架侧 `max_retry`、无死信。
...
- 错误处理：覆写 `decide_retry` 按错误性质判 `Retry` / `Discard`；`on_failed` 只按结论更新底层数据。
...
- 对频繁失败的下游服务在 `decide_retry` 中判 `Discard` 以终止重投，减少雪崩风险。
