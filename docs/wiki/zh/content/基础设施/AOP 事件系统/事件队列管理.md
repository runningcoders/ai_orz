...
- 业务收尾：`on_consumed(ctx, event)`（成功或 Discard 后回调）、`on_failed(ctx, event, err, decision, attempt)`。
...
- 重试策略
  - 异步消费者 on_event 失败 → `finish_consumption` 由 `Consumer::decide_retry` 判定 `RetryDecision`：`Retry` → queue.nack 重投；`Discard` → queue.ack（事件永久移除，无死信）且仍回调 `on_consumed`。
  - 成功处理后 queue.ack，确保事件从 in_progress 移除，并推进同 order_key 的下一个事件。
  - 框架**不设框架侧 max_retry**：终局判定在 `Consumer::decide_retry`（永久错误码表 + `DEFAULT_MAX_ATTEMPTS = 8` 兜底）。
- 幂等性与去重
