...
### 增量 1：Token 消耗时序看板（finance 域归位）
- **新 handler** `GET /api/v1/finance/model-providers/token-stats`：分钟级三线时间序列（tokens_input + tokens_output + call_count），数据源 DuckDB `model_call_events` 表，组织隔离由 DAL 层按 `ctx.organization_id` 注入 filter
- **StatsInterval 枚举扩展**：原有 Hourly/Daily → 新增 Minutely，`collector.rs` 截断函数按分钟 bucket 聚合。⚠️ 实现必须写成 `timestamp - (timestamp % 60000)`，**不可**写成 `(timestamp / 60000) * 60000` —— DuckDB 的 `/` 是浮点除（BIGINT/BIGINT → DOUBLE），后者既失去 bucket 语义、返回值又会被 `as_i64()` 解析成 0（假绿灯）
- **ModelProviderStatsDao 扩展**：新增 `model_call_time_series(ctx, minutes)` + user_id 维度过滤注入
- **批次刷盘预期**：最近 1-2 分钟数据可能未 flush，曲线末端偏低属正常——前端按固定间隔轮询即可

### 增量 2：前端图表组件多场景消费
- LineChart 从「统计仪表盘专属」扩展到聊天侧栏 Agent Tab（320x180 紧凑原生尺寸）
- HudPalette 新增 HUD_SECONDARY(#22d3ee 青蓝) + HUD_TERTIARY(#a78bfa 紫)，支撑三线区分
- 相关变更详见：[多维统计系统.md](docs/wiki/zh/content/项目概述/核心功能特性/多维统计系统/多维统计系统.md) + [图表组件.md](docs/wiki/zh/content/前端应用/组件系统/图表组件/图表组件.md) + [业务组件.md](docs/wiki/zh/content/前端应用/组件系统/业务组件.md)
...
**变更 commit**：9c59ab64（stats-ui 上下文占比缺阈值说明）+ 4a1b381e（周期落盘收口 + 可空 JSON）+ 7469541e（用户页统计兜底 + 时间区间筛选组件）

### 增量 1：周期落盘收口 + 可空 JSON 兼容
- stats 批次刷盘定时器从 `pkg/stats/runtime` 独立调度器收口到 `pkg/stats/default.rs:Stats::open()`——与 DuckDB 连接生命周期绑定，避免多定时器双重 flush 竞争
- `model_call_events.call_summary` / `token_summary` 字段从 NOT NULL 改为 nullable（兼容迁移前的旧数据），查询时 `COALESCE(field, '{}')` 兜底

### 增量 2：用户页统计兜底 + TimeRangePicker 通用时间筛选
- `get_current_user` handler 统计查询返回空结果时，前端 profile 页显示「暂无数据」空态卡片而非 error toast，避免新用户首次访问看到红色报错
- 新增 `frontend/src/components/time_range_picker.rs` 通用时间区间筛选组件——props: start/end + 预设快捷按钮（1h/6h/24h/7d/30d）+ 自定义日期选择器，所有统计看板统一引入（AOP 系统页/ModelProvider Token 时序/用户页统计）