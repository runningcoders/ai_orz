...
## 更新摘要
**变更内容**
- AOP 事件系统小节：移除「Scheduler 定时轮询」描述，`EventKind` 改为 `common::enums::EventTopic`，事件类型标识统一为 `Event::kind() -> EventTopic`。
- 类图更新：`Producer` 改为 `name()/topic()/start(sink)/stop()/on_consumed/on_failed` 形态（删除 `produce()`）；`Consumer` 改为 `subscriptions() -> Vec<Subscription>` + `on_event(ctx, event)` + `decide_retry(err, attempt) -> RetryDecision`（删除 `consume()` 与 `Consumer::ack/nack`）；`Producer::on_failed` 改为**接收** `decision`（终局判定归消费者、数据收尾归生产者）；删除 `Scheduler` 类，`Registry` 增加 `register_producer`/`shutdown_all`，关系改为「按 topic 反查 producer」「start/stop」。
- `性能考量` 中「生产者轮询」改为「生产者自管循环（ProducerLoop）」。
...
## 核心组件
- AOP 事件系统：以 Event/Producer/Consumer/Registry 为核心，提供异步事件分发与调度；生产者用 `ProducerLoop` 自管循环、Registry 统一 `start`/`stop`，业务通过注册消费者参与处理
...
关键类型与职责
- Event/EventTopic：事件数据结构，类型标识统一为 `common::enums::EventTopic`（点分线字符串，如 `message.created`）；经 `Event::kind() -> EventTopic` 取用
- Producer：事件生产者接口（`name()/topic()/start(sink)/stop()/on_consumed/on_failed`）
- Consumer/ConsumeMode：消费者接口与消费模式，经 `subscriptions() -> Vec<Subscription>` 声明兴趣
- Registry：事件注册、分发与收尾，按 topic 反查 producer 回调；`start_all`/`shutdown_all` 统一启停
...
<<enum>>
+as_str() String
+parse(String) Option~EventTopic~
}
class Producer {
+name() String
+topic() EventTopic
+start(sink: EventSink) Result
+stop()
...
Registry --> EventTopic : "按 kind 反查 producer"
Registry --> Producer : "start / stop"
Registry --> Consumer : "分发"
Producer ..> EventSink : "emit 校验 kind == topic"
Consumer ..> Subscription : "声明兴趣"
```

图表来源
- [src/pkg/aop/core/event.rs:1-17](src/pkg/aop/core/event.rs#L1-L17)
- [common/src/enums/event_topic.rs:32-65](common/src/enums/event_topic.rs#L32-L65)
- [src/pkg/aop/core/consumer.rs:22-112](src/pkg/aop/core/consumer.rs#L22-L112)
...
- 统计写入批大小：合理设置 batch_size 平衡延迟与 IO；并配合 flush_interval_secs（默认 30s）周期落盘，避免低频实例攒不满 batch_size 导致看板数据长期滞后
- 日志滚动与保留：按日滚动避免大文件，设置 retention_days 控制磁盘占用
- AOP 调度：异步消费者 worker 与生产者自管循环（ProducerLoop）需匹配负载，避免积压
...
- 统计查询为空 / 近期数据缺失
  ...
  - 检查 batch_size 与 flush_all 调用时机：统计先写内存缓冲，仅在「缓冲攒满 batch_size」或「进程优雅退出」时落盘
  - 若为低频实例（长期攒不满 batch_size），确认周期落盘任务已启动（stats.flush_interval_secs≠0，默认 30s）
- AOP 事件无消费者
