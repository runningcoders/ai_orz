...
- 类图更新：`Producer` 改为 `name()/topic()/start(sink)/stop()/on_consumed/on_failed` 形态（删除 `produce()`）；`Consumer` 改为 `subscriptions() -> Vec<Subscription>` + `on_event(ctx, event)`（删除 `consume()` 与 `Consumer::ack/nack`）；删除 `Scheduler` 类，`Registry` 增加 `register_producer`/`shutdown_all`，关系改为「按 topic 反查 producer」「start/stop」。
...
+on_failed(ctx, event, err) RetryDecision
}
class Consumer {
+name() String
