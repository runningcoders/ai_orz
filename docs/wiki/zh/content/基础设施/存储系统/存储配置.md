...
- [src/pkg/storage/mod.rs:49-122](file://src/pkg/storage/mod.rs#L49-L122)
...
- [src/pkg/storage/mod.rs:36-47](file://src/pkg/storage/mod.rs#L36-L47)
...
- [src/pkg/storage/lance.rs:43-77](file://src/pkg/storage/lance.rs#L43-L77)
...
### 数据目录结构与路径规划
AppConfig 提供了丰富的路径计算能力，便于统一规划数据存放：
- 数据库：ai_orz.db（可通过配置修改文件名）
- 向量库：ai_orz_vector.db（SqliteVss 时使用）
- 向量索引目录：hnsw_index（HNSW 后端）
- 附件目录：attachments（消息附件、Agent 生成文件等）
- 产物目录：artifacts/projects/{project_id}/{artifact_id}
- Agent 数据目录：agents/{agent_id}，记忆子目录 agents/{agent_id}/memory
- 共享技能目录：skills/{skill_id}
- 工具调用追踪与日志：tools/{tool_id}/call_trace 与 tools/{tool_id}/logs

章节来源
- [common/src/config.rs:244-416](file://common/src/config.rs#L244-L416)
...
- [common/src/config.rs:116-143](file://common/src/config.rs#L116-L143)
- [docs/vector_search_architecture.md:339-398](file://docs/vector_search_architecture.md#L339-L398)
...
- [src/config.rs:54-64](file://src/config.rs#L54-L64)
...
- 数据目录规划
  ...
  - 附件与产物：attachments、artifacts/projects/{project_id}/{artifact_id}
  - Agent 数据：agents/{agent_id}，记忆子目录 memory
  - 工具日志与追踪：tools/{tool_id}/logs 与 call_trace
