...
- [tool_registry/doubao_search.rs](src/pkg/tool_registry/doubao_search.rs)
- [tool_registry/lark_cli.rs](src/pkg/tool_registry/lark_cli.rs)
- [generic_token_integration.rs](common/src/api/generic_token_integration.rs)
...
## 更新摘要
**增量更新**（2026-08-24）：
- 新增豆包搜索工具（Doubao Search）接入，支持 web / web_summary / image 三种搜索类型
- 搜索工具凭据机制统一迁移至 GenericToken + platform 二元匹配（Tavily / 豆包）
- 删除 TavilyKey 专用凭据类型，复用 GenericToken + platform 区分不同搜索平台
- 新增通用令牌（GenericToken）Handler 链路（CRUD + set_default + status）
- 前端新增通用 Token 管理区块，按 platform 分 Tab 展示

**增量更新**（2026-08-22，提交 67ff3b57 / f2406bcb）：
- 校验规则矩阵全面下沉 common（前后端零漂移收官）：六规则校验本体（`validate_requirements`，含 binding_allowed / binding_name / mcp_transport_scope / `CredentialEnhancerKind::as_str`）落 [common/src/models/identity_credentials.rs](common/src/models/identity_credentials.rs)——pkg::credential 与前端 `validate_requirements_scoped` 均为薄委托，MCP create / update handler 的 transport→scope 三处重复 match 消除；Builtin config 校验（`validate_builtin_tool_config`）与 HTTP 方法白名单（`is_supported_http_method`）落 [common/src/models/tool.rs](common/src/models/tool.rs)——update_tool / http 工具解析 / 前端表单提交前校验共用，前端 merge 防丢合并改为原样放置 + 合并后单点校验

**增量更新**（2026-08-22，提交 cbf014f7 / 4c978274）：
- 敏感名规则矩阵下沉 common 单点：`is_sensitive_credential_name`（common/src/models/identity_credentials.rs，与 enhancer_supports 同址），后端 `is_sensitive_header` 与前端 `is_sensitive_name` 均改为委托——双端复刻漂移面归零（此前 api-key 连字符规则落地时已被复制错过一次）
- 凭据需求表单 UI 深化：共享模块新增 `injection_value_preview`（注入值形态只读示意：field 直取 > 显式增强器 > 默认增强器 > 类型规范可用值）与 `recommended_binding_name`（Bearer/BasicAuth → authorization、tavily_key → api_key 惯用名建议）；MCP 与 HTTP 两表单均接入预览徽章 + 空注入名建议提示，增强器下拉换语义展示名
...
- 前端：`credential_form` 共享模块（六规则预校验 + 敏感名委托 common 单点）、`CredentialRequirementsTable` 只读组件、MCP 创建 modal 凭据需求编辑器、HTTP 工具表单敏感名即时预检、工具详情配置编辑卡片
- 验收：后端 lib 单测 1046 passed / 集成 23 target 102 passed / 前端 122 passed / clippy 双端零警告
...
## 架构原理与流程
### 配置期（管理面）
MCP Server 与 HTTP 工具的 config 内各挂 `credential_requirements[]` 数组；创建 / 更新 handler 接入 `validate_requirements` 校验（六规则本体在 [common identity_credentials](common/src/models/identity_credentials.rs#L729-L795)，与前端预校验同一实现）：binding↔协议匹配、注入名非空、platform↔kind 匹配、field↔enhancer 互斥、enhancer↔kind supports 矩阵、(kind, platform, 注入点名) 三元组去重；显式选择默认增强器幂等归一（等价于不选，D11）。落库 config 零凭据原文、零 credential_id。
...
### Builtin 更新管道与工厂字段保护
[update_tool.rs](src/handlers/finance/tool/update_tool.rs#L107-L119)：Builtin 更新 diff 式 guard——工厂字段（name / description / protocol / control_mode / parameters_schema / tags）不可改，config / status 放行 + `validate_builtin_config` 轻量校验（本体在 [common tool.rs `validate_builtin_tool_config`](common/src/models/tool.rs#L29-L47)，前端表单提交前同点校验）；config 变化不触发向量重索引。

### 前端共享模块
[credential_form.rs](frontend/src/components/credential_form.rs#L69)：common 单点薄委托门面——`validate_requirements_scoped`（六规则预校验）、`is_sensitive_name`、`binding_name`、`mcp_transport_scope`、`enhancer_to_value` 全部委托 common 同一实现（与后端零漂移）；`injection_value_preview` 注入值形态只读示意（field 直取 > 显式增强器 > 默认增强器 > 类型规范可用值）与 `recommended_binding_name` 惯用注入名建议（Bearer/BasicAuth → authorization、tavily_key → api_key）。[credential_requirements.rs](frontend/src/components/credential_requirements.rs#L39)：`CredentialRequirementsTable` 只读表格组件（MCP 详情 / 工具详情复用）。[create_http_tool.rs](frontend/src/components/create_http_tool.rs#L117)：headers/query 敏感名即时预检（sensitive_keys_hint）+ 方法白名单经 common `is_supported_http_method` 单点判定。[mcp_servers.rs](frontend/src/pages/finance/mcp_servers.rs#L53-L163)：创建 modal 凭据需求动态列表（kind / platform / field / enhancer / binding 联动 + 注入值预览徽章）。[tool_detail.rs](frontend/src/pages/finance/tool_detail.rs#L538)：工具配置编辑卡片（`merge_builtin_config` 防丢合并：表单字段原样放置 + 合并后经 common `validate_builtin_tool_config` 单点校验）+ 凭据需求只读卡片。
...
章节来源
- [tool_execution.rs#L36-L101](src/service/domain/runtime/tool_execution.rs#L36-L101)
- [credential/mod.rs#L22-L237](src/pkg/credential/mod.rs#L22-L237)
...
### 目录结构
```
common/src/models/identity_credentials.rs   ← CredentialRequirement 契约族 + supports 矩阵 + 敏感名 / 六规则 / transport→scope 校验单点
common/src/models/tool.rs                   ← Builtin config 校验 + HTTP 方法白名单单点
src/pkg/credential/                          ← 凭据域纯值加工（零数据访问）
├── mod.rs                                   ← ResolvedCredential + 解密 + 纯函数 + 六规则校验包装（common 单点）
...
frontend/src/components/credential_form.rs   ← common 单点薄委托门面（预校验 / 敏感名 / 注入名 / scope 映射）
```
...
3. 前端即时预检：[create_http_tool.rs#L117](frontend/src/components/create_http_tool.rs#L117) sensitive_keys_hint 与 [credential_form.rs#L199-L203](frontend/src/components/credential_form.rs#L199-L203) is_sensitive_name；规则本体单点在 [is_sensitive_credential_name](common/src/models/identity_credentials.rs#L672-L686)，前后端均委托该实现，无双端漂移面。
...
- **前端表单**：复用 credential_form 共享模块，不复制粘贴校验逻辑；六规则 / 敏感名 / 注入名 / scope 映射 / 方法白名单 / Builtin config 校验全部委托 common 单点（前后端同一实现，规则改动只改 common 一处）。
...
后续扩展路径：OAuth 刷新链路完善（allow_local_network 受控暴露 / refresh_token 轮换 / 刷新失败结构化引导）；generic_token / oauth / user_password 的创建管理 API 与前端凭据区块。~~HTTP 工具凭据 UI 深化~~ 与 ~~前端敏感名规则同源同步机制下沉 common~~ 已于 2026-08-22 完成（提交 4c978274 / cbf014f7：injection_value_preview + recommended_binding_name 接入两表单；is_sensitive_credential_name 单点双端委托）。
