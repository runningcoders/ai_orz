...
- **前端表单**：复用 credential_form 共享模块的六规则预校验，不复制粘贴校验逻辑；敏感名规则改一处即可——单点在 common `is_sensitive_credential_name`，前后端委托调用。
