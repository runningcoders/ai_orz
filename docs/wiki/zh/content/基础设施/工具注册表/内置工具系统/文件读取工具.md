...
**④ RAG 原子知识卡**：
- [用户维度工作区路径改造 + 工具调用安全边界检查：路径逃逸 跨用户访问 相对路径阻断](docs/wiki/knowledge/zh/用户维度工作区路径改造 + 工具调用安全边界检查：路径逃逸 跨用户访问 相对路径阻断/用户维度工作区路径改造 + 工具调用安全边界检查：路径逃逸 跨用户访问 相对路径阻断.md) — §红线 6 file_read mime 白名单（二进制文件强制拒绝）；§红线 7 禁止读符号链接（即使在边界内）

**③ Wiki 关联长文**：
- [工具安全控制.md](docs/wiki/zh/content/基础设施/工具注册表/工具安全控制.md) — 工具安全总览与 resolve_and_validate_path 通用校验
- [本地文件安全与工作区隔离.md](docs/wiki/zh/content/基础设施/存储系统/本地文件安全与工作区隔离.md) — 存储层用户目录拆分 + 工具层边界双重隔离说明
</cite>

## 更新摘要
**2026-08-16 增量更新**：补齐 cite 区四类互引闭环（1 Design+1 Plan 规范占位 + 1 RAG 卡 + 2 关联长文）；关联 T5（工作区路径安全边界）主题；§3 FsReadCoreTool 实现章节来源行号降级（因 6b883e37 改造加入 SecurityBoundaryChecker.enforce 调用、mime 白名单判断等新分支导致 fs_read.rs 行号范围漂移，优先降级为无行号范围引用）。
