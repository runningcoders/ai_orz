...
- 【① Design 决策】[docs/archive/design-archive/generic_builtin_tools_design.md](docs/archive/design-archive/generic_builtin_tools_design.md) — 通用 Builtin 工具 fs_read/fs_write/http_fetch + 安全沙箱最小权限
- 【① Design 决策】[docs/archive/design-archive/handler-tool-registration-macro.md](docs/archive/design-archive/handler-tool-registration-macro.md) — register_handler_tool! 宏把 Handler 函数转神经工具
- 【① Design 决策】[docs/archive/design-archive/builtins_http_tool_design.md](docs/archive/design-archive/builtins_http_tool_design.md) — HTTP Tool Runtime 用户注册式 ToolPo+HttpToolConfig
...
- shell_exec：支持同步执行与后台运行，输出落盘日志，工作目录受限，命令经 `shell_policy` 拦截，环境变量全量继承 + PATH 补全，超时与输出大小限制
...
- 安全
  ...
  - Shell：工作目录限制、命令拦截层（`shell_policy`）、环境变量兼容优先（全量继承 + PATH 补全）；MCP stdio 为凭据隔离走 `env_clear()` 零继承
