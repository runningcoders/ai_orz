...
- **发布打包脚本 `scripts/package.sh`（`make package`）**：本地与 CI 共用单一打包入口，编译（复用 `start.sh build`）后组装发布目录（二进制 + `dist/` + `script/` 运维脚本 + `Makefile` + `README.md`）并产出 tar.gz；release.yml 直接调用本脚本，打包逻辑只有一处。
...
- 发布构建（release.yml）
  ...
  - 构建+打包：直接调用 `scripts/package.sh`（本地 `make package` 同款），内部完成 `start.sh build`（前端 dx build --release → dist/ + 后端 cargo build --release）与发布目录组装（二进制 + `dist/` + `script/` 运维脚本 + `Makefile` + `README.md`），产出 `ai_orz-{tag}-{target}.tar.gz`。
