...
- [.githooks/pre-push](.githooks/pre-push) — 本地 pre-push hook 真门禁（cargo check + cargo clippy 全 workspace）
- [Dockerfile](Dockerfile) — release.yml 多架构镜像基础
- [docker-compose.yml](docker-compose.yml) — compose 多服务编排验证
- [Caddyfile](Caddyfile) — 网关配置验证
- 【🎴 RAG 知识卡】docs/wiki/knowledge/zh/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像.md — release.yml 多架构镜像推送 & Dockerfile 三阶段构建
...
- [Rust Workspace + Dioxus WASM 全栈构建与 CI/CD 流水线](docs/wiki/knowledge/zh/Rust Workspace + Dioxus WASM 全栈构建与 CI_CD 流水线/Rust Workspace + Dioxus WASM 全栈构建与 CI_CD 流水线.md)
- [Rust Workspace + NPM 多语言依赖管理（Cargo.lock / package-lock.json / SQLx 离线构建）](docs/wiki/knowledge/zh/Rust Workspace + NPM 多语言依赖管理（Cargo.lock _ package-lock.json _ SQLx 离线构建）/Rust Workspace + NPM 多语言依赖管理（Cargo.lock _ package-lock.json _ SQLx 离线构建）.md)
</cite>
...
**2026-08-23 增量更新**：新增 `make package` 本地打包命令与 `scripts/package.sh` 打包脚本（release.yml 改为直接复用，打包逻辑单一事实源）；发布包结构升级为「二进制 + dist/ + script/ 运维脚本 + Makefile + README.md（Markdown）」，解压后 `make start` 即可运行。
...
- **发布打包脚本 `scripts/package.sh`（`make package`）**：本地与 CI 共用单一打包入口，编译（复用 `prod.sh build`：只编译，产物留在仓库、不写本机部署根）后组装发布目录（二进制 + `dist/` + `script/` 运维脚本 + `Makefile` + `README.md`）并产出 tar.gz；release.yml 直接调用本脚本，打包逻辑只有一处。
...
- **scripts/package.sh + scripts/release/**：发布物打包脚本与包内容模板（`make package`），本地与 CI 共用；`scripts/release/` 内含 `script/` 运维脚本、`Makefile`、`README.md` 模板。
...
C --> I["调用 scripts/package.sh（make package 同款）"]
I --> J["内部 start.sh build：build_frontend.sh + cargo build --release"]
J --> K["组装发布目录（二进制+dist+script+Makefile+README.md）"]
K --> L["打包 tar.gz"]
L --> M["上传制品/创建 Release"]
```
...
- 发布构建（release.yml）
  ...
  - 构建+打包：直接调用 `scripts/package.sh`（本地 `make package` 同款），内部完成 `prod.sh build`（前端 dx build --release → dist/ + 后端 cargo build --release；只编译，产物留在仓库、不写本机部署根）与发布目录组装（二进制 + `dist/` + `script/` 运维脚本 + `Makefile` + `README.md`），产出 `ai_orz-{tag}-{target}.tar.gz`。部署到本机部署根是另一步 `prod.sh install`（`make install`），打包链路刻意不调用它 —— 否则 CI 会往 runner `$HOME` 白写约 280MB。
...
### 本文关联的文档
- 🎴 RAG 卡: docs/wiki/knowledge/zh/测试与质量工程：1124测试100%通过 + 984后端82%前端 + 87%集成测试19targets + cargo-llvm-cov 38%/45%门槛 + clippy零容忍+Playwright E2E/测试与质量工程：1124测试100%通过 + 984后端82%前端 + 87%集成测试19targets + cargo-llvm-cov 38%2F45%门槛 + clippy零容忍+Playwright E2E.md
...
**主题**：release.yml 多架构 Docker 镜像推送 + 本地 pre-push hook 真门禁
**关键变更**：
1. release.yml 新增多架构镜像推送——amd64/arm64 各自在原生 runner 上构建后合并 manifest，推送到 registry（tag `v*` + `latest`），不用 QEMU（duckdb C++ 源码 + fastembed ONNX Runtime）
2. 新增 .githooks/pre-push 本地 hook——cargo check + cargo clippy --workspace 全 crate 零容忍，push 前拦截低级错误，远端 CI 失败率显著降低
3. Dockerfile 三阶段 + Caddyfile reverse_proxy + docker-compose.yml 两服务成为 release.yml 镜像构建基础
4. CI clippy 升级为 --workspace 全 crate 覆盖，不再仅检查主包
**涉及 RAG 卡**：测试与质量工程卡、容器化与网关部署卡

---

### 本文关联的文档（2026-09-13 增量）
- 🎴 RAG 卡: docs/wiki/knowledge/zh/测试与质量工程：1124测试100%通过 + 984后端82前端 + 87集成测试19targets + cargo-llvm-cov 38%/45%门槛 + clippy零容忍+Playwright E2E/测试与质量工程：1124测试100%通过 + 984后端82前端 + 87集成测试19targets + cargo-llvm-cov 38%/45%门槛 + clippy零容忍+Playwright E2E.md

---

### 更新摘要（2026-09-13，base 7519cacf→HEAD）
**主题**：pre-push 钩子加回 dx check 作为 Dioxus 前端编译检查补充门禁
**关键变更**：
1. .githooks/pre-push 在 clippy + test 之后追加 `cargo check -p frontend --target wasm32-unknown-unknown`（解决前端过 clippy 但 wasm32 编译失败的漏网情况）
2. 修正钩子启动行文案误导（旧写 pre-commit 实际是 pre-push）+ 清理过期注释
**涉及 RAG 卡**：测试与质量工程卡
