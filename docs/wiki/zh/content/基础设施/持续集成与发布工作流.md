...
**② 落地计划（Plan）**：
- [Agent管理集成测试](docs/plan/Agent管理集成测试.md) — Agent 管理域全链路集成测试落地点；集成测试 19 targets 通过 CI 闸门验证

**【Batch10 追加互引】**
#### ① Design 决策快照（Batch10 追加）
- [browser_e2e_test_design.md](docs/design/browser_e2e_test_design.md) — E2E Playbook 双模式；CI 阶段 4（仅本地/手动触发）执行 Playwright E2E；data-testid 选择器保证 CI 稳定
- [seed-config-migration.md](docs/design/seed-config-migration.md) — 两阶段初始化顺序（init_all/init_base_data/aop init）；CI 集成测试阶段必须严格对齐真实启动顺序
...
- [start.sh:130-151](start.sh#L130-L151)
