...
+message_sender_role() MessageRole
...
- 封装方法：
  ...
  - caller_id_or_system()：用于**审计字段** created_by/modified_by（System→"system"）
  - message_sender_id()：用于**消息发送 from_id**。优先 agent_id —— 消息唤醒的消费侧
    rebuild_context 会把被唤醒的 Agent（`message.po.to_id`）写入 agent_id，而 caller_type
    未必是 Agent（System 来源消息 / 后台触发），用 caller_id() 会把真实发送者丢成
    字面量 "system"（不存在的 Agent）
  - message_sender_role()：与 message_sender_id() 配套的 **from_role** 判定 —— agent_id
    有值即落 Agent（避免「Agent ID + System 角色」的错位记录），无 agent_id 回退 caller_role()
  - caller_role()：caller_type 直接映射为 MessageRole（不再直接用于消息 from_role，
    消息场景一律走 message_sender_id / message_sender_role 配对）

### 消息触发的身份分层模型（调度器 / 事件消费者）
系统触发器按「被触达事项的归属」选择发送方身份，身份决定 Final 回复的路由（天然无环）：

| 触发场景 | 发送方身份 | Final 回复去向 |
|---------|-----------|---------------|
| 用户发起的项目 / 任务通知（任务调度、项目巡检） | **归属用户中继**（from_role=User，from_id=root_user_id） | 自动回到该用户（User 分支自然路由） |
| Agent 自发协作 | **发起 Agent**（from_role=Agent） | 回给发起 Agent（Agent 分支） |
| 纯系统指令（无需回复、与用户 / Agent 无关）、无归属用户的 A2A 触达 | **System**（万不得已） | Final 丢弃（无对等回复对象） |

实现要点：
- `consumer/mod.rs::enrich_org_from_project_user`：系统触发链路 ctx 无组织绑定，
  以项目 root_user_id 的组织补齐（组织维度唯一源头是 `UserPo.organization_id`），
  否则消息落库无 organization_id，Agent 工具调用（如 list_messages）报「缺少组织上下文」
- `consumer/message.rs`：消费侧兜底 —— rebuild_context 后 ctx 仍缺组织时，从归属用户补齐；
  Final 路由的 System 兜底分支判定收敛在 `consumer/message.rs::routes_to_system_fallback`
  （单一扩展点：System 来源恒命中；Agent 自触发 from==to 命中防自唤醒循环），
  命中后 Final 丢弃——归属用户需要感知的通知由发送侧以用户身份中继，走 User 分支自然送达
