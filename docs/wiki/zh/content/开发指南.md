...
**② 落地计划（Plan）**：
- [Agent管理集成测试](docs/archive/plan-archive/Agent管理集成测试.md) — Agent 管理域全链路集成测试落地点
...
**② 落地计划（Plan）- Batch9 补充**：
- [Query 接口分页与 List 接口简化重构](docs/archive/plan-archive/Query接口分页与List接口简化重构.md) — 5 实体 list/query 双接口统一模式 + COUNT 与 LIST push_query_filters 强制共享

**【本次 2026-08-16 增量追加互引】**
#### ④ RAG 原子知识卡（本次追加 T8 scripts/Makefile 归置）：
- [scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正](docs/wiki/knowledge/zh/scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正/scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正.md) — §红线 4 日常开发统一 Makefile 入口：make test / make clippy / make fmt / make coverage / make ci-docs-lint
#### ① 设计文档（Design，本次追加占位）：
...
## 更新摘要
**2026-08-16 增量更新**：追加 T8（scripts/ 归置 + Makefile 统一入口 + 60+ 引用修正）RAG 卡互引；cite 区补对应 design/plan 规范占位；§5 详细组件分析开发命令章节来源行号降级（因 7395ce18 Makefile 新增 24 个统一 target 导致命令引用更新，行号范围漂移，优先降级为无行号范围引用）。
