...
### 本文关联的设计文档
- [时间戳处理约定](docs/design/timestamp_convention.md) — created_at/updated_at/indexed_at/expire_at 统一为毫秒级 Unix 时间戳（i64），UTC 时区
...
- 新增时间戳约定引用：所有时间字段（created_at/updated_at/indexed_at/expire_at）统一为毫秒级 Unix 时间戳（i64），UTC 时区
- MemoryTrace 时间戳字段已对齐时间戳规范
- VectorMeta indexed_at/expire_at 使用毫秒级 Unix 时间戳
...
章节来源
- [docs/vector_search_architecture.md:1-159](docs/vector_search_architecture.md#L1-L159)
- [src/pkg/storage/mod.rs:1-212](src/pkg/storage/mod.rs#L1-L212)
...
- Memory 实体族
  ...
  - KnowledgeNodeRelationPo：知识节点关系（source/target/type/weight/status），status=Active/Superseded/Deleted（唯一生效边约束：同键仅一条 Active）
...
- KnowledgeNodeRelationPo：描述知识节点之间的关系（related/contains/depends/prerequisite/similar/causes 等）。**2026-09-18 新增 status 字段**（Active/Superseded/Deleted），表示边自身的版本生命周期——修正关系 = 插入新边 + 旧边降级 Superseded，历史边永久留库供因果链回放。默认查询只取 Active，同 (source, target, relation_type) 仅一条生效（部分唯一索引 uq_knowledge_relation_active_edge 保证）。
...
- 创建
  ...
  - CreateRelations：写关系表（不向量化），默认插入 Active 状态；修正关系 = 新边 Active + 旧边降级 Superseded（唯一索引保证同键仅一条 Active）。
- 更新
  - 仅支持 ShortTerm/KnowledgeNode；更新后重新向量化，失败降级为 warn。
- 删除
  - ShortTerm：软删除（标记 Forgotten）+ 删除向量索引。
  - KnowledgeNode：级联删除关系与引用 + 删除向量索引。
  - Relation：**软删除**（2026-09-18 新增）——标记边为 `Deleted`，行保留支持恢复；仅降级生效边（DAO 层保护）。查询默认只取 Active，Deleted/Superseded 边在列表视图不出现。
- 沉淀
...
图表来源
- [src/service/dal/memory.rs:377-475](src/service/dal/memory.rs#L377-L475)
- [src/models/memory.rs:342-366](src/models/memory.rs#L342-L366)
...
- [src/models/vector.rs:94-136](src/models/vector.rs#L94-L136)
...
章节来源
- [src/service/dal/memory.rs:518-652](src/service/dal/memory.rs#L518-L652)
...
- [src/pkg/storage/mod.rs:20-35](src/pkg/storage/mod.rs#L20-L35)
- [migrations/20260712000000_memory_fts5.sql:16-29](migrations/20260712000000_memory_fts5.sql#L16-L29)
- [migrations/20260505000000_vector_metadata.sql:1-16](migrations/20260505000000_vector_metadata.sql#L1-L16)
...
章节来源
- [src/pkg/storage/hnsw.rs:20-34](src/pkg/storage/hnsw.rs#L20-L34)
...
- [src/models/vector.rs:69-92](src/models/vector.rs#L69-L92)
