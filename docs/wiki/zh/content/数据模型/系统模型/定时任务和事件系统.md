...
**② 落地计划（Plan）**：
- [唤醒上下文与睡眠约束](docs/archive/plan-archive/唤醒上下文与睡眠约束.md) — Agent 睡眠沉淀与唤醒上下文落地点
...
## 更新摘要
**变更内容**
- 事件主题统一为 `EventTopic`（`common::enums::event_topic.rs`，点分线字符串格式），旧变体（`Message` / `TaskChange` / `CronTrigger` / `Custom(u16)`）已废弃
- `Event::topic()` → `Event::kind()`；`src/models/event.rs` 已删除，事件定义位于 `src/models/events/`
- 订阅由 `interested_events()` 改为 `subscriptions()`；`CronTriggerProducer` 改用 `ProducerLoop` + `EventSink`，`mark_trigger_executed` 移入 `on_consumed`（P3）
- 收尾统一为 `finish_consumption`，按 topic 反查生产者 `on_consumed` / `on_failed`（`RetryDecision`）
- 修正既有非法 mermaid（`graph TB` → `graph TB`、`stateDiagram-v2` → `graph TB`）
...
- [common/src/enums/event_topic.rs:1-244](common/src/enums/event_topic.rs#L1-L244)
...
- [frontend/src/pages/system/triggers.rs:394-421](frontend/src/pages/system/triggers.rs#L394-L421)
...
- CronTriggerProducer：周期轮询到期触发器，生成事件并通过 `EventSink` 发布；`mark_trigger_executed` 已移至 `on_consumed`，由消费终结回调推进执行标记（见 P3 修复）。
...
章节来源
- [src/models/cron_trigger.rs:10-66](src/models/cron_trigger.rs#L10-L66)
- [common/src/enums/cron_trigger.rs:9-47](common/src/enums/cron_trigger.rs#L9-L47)
- [migrations/20260711000000_cron_triggers.sql:4-24](migrations/20260711000000_cron_triggers.sql#L4-L24)

### 执行计划管理与重试机制
- 执行计划：生产者每 60 秒轮询一次，查询 next_run_at <= now 且启用的触发器，批量上限 100。
- 执行标记：`mark_trigger_executed` 已移至 `CronTriggerProducer::on_consumed`：消费成功且事件终结时，取事件 `created_at` 推进 `last_run_at` 与 `next_run_at`（避免被消费耗时拖后）；`decide_retry` 恒 `Retry`。
...
### Event 实体：类型定义、元数据与状态流转
  - 事件主题：统一为 `EventTopic`（`common::enums::event_topic.rs`），点分线字符串格式（如 `message.created` / `cron.trigger` / `lark.inbound.message` 等 16 个变体），旧变体 `Message` / `TaskChange` / `CronTrigger` / `Custom(u16)` 已废弃。
  - 事件引用：event_id、order_key、priority、created_at；用于堆排序与队列存储。
  - 事件基础能力：`Event` trait 提供 `kind()`（返回 `EventTopic`）、id、order_key、priority、created_at 等元数据访问，`to_event_ref` 由队列使用。
...
章节来源
- [frontend/src/pages/system/triggers.rs:394-421](frontend/src/pages/system/triggers.rs#L394-L421)
- [src/handlers/system/cron_trigger/resume_cron_trigger.rs:1-22](src/handlers/system/cron_trigger/resume_cron_trigger.rs#L1-L22)
- [migrations/20260711000000_cron_triggers.sql:11-13](migrations/20260711000000_cron_triggers.sql#L11-L13)
...
- [common/src/enums/event_topic.rs:1-244](common/src/enums/event_topic.rs#L1-L244)
- [src/pkg/aop/core/event.rs:4-17](src/pkg/aop/core/event.rs#L4-L17)
- [src/producer/cron_trigger.rs:55-58](src/producer/cron_trigger.rs#L55-L58)
- [src/pkg/aop/queue/mod.rs:77-106](src/pkg/aop/queue/mod.rs#L77-L106)
...
章节来源
- [tests/integration/system_cron_triggers_test.rs:102-129](tests/integration/system_cron_triggers_test.rs#L102-L129)
- [frontend/src/pages/system/triggers.rs:394-421](frontend/src/pages/system/triggers.rs#L394-L421)

---

### 更新摘要（2026-09-12，base c632f4bf→HEAD）
**主题**：CronTriggerConsumer agent_rest handler 新增系统级全局执行模式
**关键变更**：
1. **handle_agent_rest 全局模式（src/consumer/scheduler.rs）**——trigger payload 的 `organization_id` 字段为 None/空字符串/不存在时，**禁止**直接 return 跳过，必须查询 `SELECT id FROM organizations WHERE status=1` 所有活跃组织，逐一构造 RequestContext 调用 load_and_settle；
2. **向后兼容**——payload 带 org_id 时走原组织级路径（特定 org 的 Agent 沉淀）；不带 org_id 时走全局遍历（所有活跃组织的 Agent 沉淀）；
3. **修复的历史 bug**——旧版硬编码 org_id 导致多组织部署时 agent_rest cron 一直在跑但 90% 沉淀请求被跳过，生产环境日志有大量 warn 但用户感知不到。
**涉及 RAG 卡**：Memory 系统增强与休息沉淀卡