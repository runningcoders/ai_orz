...
## 更新摘要
**变更内容**
- 事件基础契约同步：`EventKind` 删除，统一为 `EventTopic`（`common::enums::EventTopic`）；`Event::topic()` → `Event::kind()`
- `src/models/event.rs`（单数）已删除，事件定义位于 `src/models/events/`（复数）
- 修正既有非法 mermaid（`graph TB` → `graph TB`、`erDiagram` → `classDiagram`）
...
- Event：定义事件总线契约，主题统一为 `EventTopic`（`common::enums::EventTopic`，点分线字符串格式），含优先级、排序键与创建时间，确保有序与可追踪的异步处理。
