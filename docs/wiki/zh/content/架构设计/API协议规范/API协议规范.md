...
### 本文关联的三类文档（四类互引闭环）
#### ① Design 决策快照
- [entity_list_query_search_design.md](docs/archive/design-archive/entity_list_query_search_design.md) — list / query / search 三接口职责二分 + 七层架构 + push_query_filters 复用 WHERE + PagedResult<T> T map 全链路
...
#### ② Plan 落地快照
- [批量查询与通用Query接口增强重构.md](docs/archive/plan-archive/批量查询与通用Query接口增强重构.md) — query 核心接口 / PagedResult<T> 全链路 / COUNT 与 LIST WHERE 100% 共享
- [Query接口分页与List接口简化重构.md](docs/archive/plan-archive/Query接口分页与List接口简化重构.md) — 分页 PaginationParams 统一 + list 简化为 query 语法糖 + MAX_PAGE_SIZE=100
#### ④ RAG 原子知识卡
...
## 更新摘要
**所做更改**
- 更新了分页与计数规范的引用，从AGENTS.md迁移到专门的pagination_and_count_convention.md文档
- 增强了分页基础设施的详细说明，包括PagedResult和PaginationParams的使用模式
- 更新了错误模型章节，反映最新的ErrorCode定义
- 优化了架构总览图，更清晰地展示分页数据流
- 简化了部分章节内容，保持文档的精简性
...
**重要更新**：分页与计数规范现已迁移至专门的[分页与计数规范文档](docs/design/pagination_and_count_convention.md)，提供更详细的实现模式和最佳实践指导。
...
- **增强的分页系统**：PaginationParams + PagedResult<T> 作为统一分页契约，支持query/list双模式接口设计。
...
- **分页数据流**：Handler → Domain → DAL → DAO 全链路传递 PaginationParams，使用 PagedResult::map() 进行类型转换。
...
H->>D : "调用业务逻辑 (含PaginationParams)"
D-->>H : "PagedResult<T>"
H-->>C : "ApiResponse<PagedResult<T>>"
```
...
- **分页双模式**：
  - query接口（POST）：完整查询条件 + pagination，返回 PagedResult<T>
  - list接口（GET）：只接受分页参数，内部固定默认过滤和排序，返回 PagedResult<T>
...
**关键实现模式**：
- DAO层必须抽取 push_query_filters 函数，COUNT 和 LIST 查询复用同一套 WHERE 条件
...
- **增强的分页限制**：统一使用 limit/offset，避免全量拉取；对大数据集建议服务端限制最大页大小。
- 错误快速失败：认证失败尽早返回，减少下游处理成本。
- 协议收敛：所有DTO集中在 common，新增接口需同步更新前后端引用，编译期发现不一致。
- **分页性能优化**：通过 push_query_filters 复用WHERE条件，避免count和list查询的重复逻辑。
...
### 请求/响应约定
- 统一响应：ApiResponse<T>
- **增强的分页**：PaginationParams + PagedResult<T>，支持query/list双模式
