...
- 重试计数：`EventRef.attempt` 累计消费次数并注入封套；框架不设 `max_retry`、无死信，决策权下沉生产者 `RetryDecision`
...
- Worker 流程：
  ...
  - 结果交给 `finish_consumption`：反查 producer 回调 `on_consumed` / `on_failed` → `RetryDecision` 决定 `Ack` / `Nack`
...
- 无限重投（Nack 风暴）
  - 消费返回 `Err` 且无 producer 时，`delivery_of` 兜底 `Nack` → `attempt` 持续自增、无限重投；入站生产者 `on_failed` 应经 `inbound_retry::decide`（`MAX_ATTEMPTS=8`）收敛为 `Discard`
- Discard 静默
