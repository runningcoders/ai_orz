...
- 重试语义：队列侧 `EventRef.attempt` 累计次数，`DeliveryOutcome { Ack, Nack }` 驱动 ack/nack；框架不设 `max_retry`、无死信，判定权下沉消费者（`decide_retry`） `RetryDecision { Retry, Discard }`
...
- 无限重投（Nack 风暴）：消费返回 `Err` 时由 `Consumer::decide_retry` 判定（永久错误码 / `attempt >= DEFAULT_MAX_ATTEMPTS(8)` → `Discard`），**与有无 producer 无关**；producer 只做数据收尾，不参与判定。
...
- 重试与死信：框架**不设框架侧 `max_retry`、无死信队列**——重试次数由 `EventRef.attempt` 体现，是否继续由消费者的 `decide_retry` 判定；若需要死信能力，应在生产者侧自行实现（如 `Discard` 分支写入独立存储）。
- 一致性保障：
