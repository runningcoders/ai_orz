...
### 数据存储策略、查询优化与访问控制
- 存储策略：
  - JSONL：按日分片，路径 {base_data_path}/tools/call_trace/{YYYYMMDD}.jsonl。
    （2026-09-19 起不再按 `tool_id` 分目录：`call_id` 是一次调用的唯一身份，`tool_id` 只是
    entry 字段；拉平后未带 `tool_id` 的查询不再退化成全量目录枚举。）
