...
- [src/consumer/message.rs#L78-L82](src/consumer/message.rs#L78-L82) (on_event 签名适配：(ctx: RequestContext, event)；注释说明 ctx 已由框架还原)
- [src/consumer/message.rs#L621-L661](src/consumer/message.rs#L621-L661) (rebuild_context 以框架 ctx 为基底追加字段，保留 log_id 贯穿)
- [src/consumer/message.rs#L337-L492](src/consumer/message.rs#L337-L492)
...
- [src/pkg/aop/core/registry.rs#L229-L233](src/pkg/aop/core/registry.rs#L229-L233) (carried_ctx 还原 helper)
- [src/pkg/aop/core/consumer.rs](src/pkg/aop/core/consumer.rs)
- [src/pkg/aop/core/consumer.rs#L44-L48](src/pkg/aop/core/consumer.rs#L44-L48) (Consumer.on_event 签名变更：新增 ctx: RequestContext)
- [src/consumer/aop_stats_hook.rs#L36-L120](src/consumer/aop_stats_hook.rs#L36-L120) (链路可观测日志 + log_id_of)
...
**更新摘要 2026-08-29**：MessageConsumer.handle_agent_message 新增 Framework 层主动回复——按入口消息 from_role 三路分发（User → send_to_user / Agent → send_to_agent / System → 跳过）；Agent 从此不需要自己调用 send_message 工具回复当前对话用户。

**更新摘要 2026-08-31**：Consumer.on_event 签名变更 + MessageConsumer context 贯穿
- on_event 签名更新为 `(ctx: RequestContext, event: serde_json::Value)`，ctx 由 Registry 从事件顶层 context_carrier 字段通过 `carried_ctx()` 还原，保留原始 log_id
- rebuild_context 方法（L621-L661）以框架传来的 ctx 为基底，用 `ctx.to_builder()` 追加 organization_id / caller_type / user_id / project_id / task_id / agent_id 等业务字段
- 关键修复：MessageConsumer 在此变更之前会自行 `RequestContext::new_system()` 重建 ctx，导致消费侧丢失原始 log_id，整条 AOP 事件链路断链。变更后改用框架还原的 ctx 基底，链路贯通
</cite>

## 更新摘要
**变更内容**
- 消费者订阅声明由 `interested_events()` 改为 `subscriptions() -> Vec<Subscription>`（`Subscription { kind: EventTopic, ordered, notify_producer }`）；`Consumer` trait 删除 `ack` / `nack` / `source`，收尾统一在 `Registry::finish_consumption` 按 topic 反查 producer 回调 `on_consumed` / `on_failed`
- 事件类型标识由 `EventKind` 统一为 `common::enums::EventTopic`（点分线字符串，16 变体）；`Event::topic()` 改为 `Event::kind() -> EventTopic`
- `MessageConsumer`（订阅 `agent.awakening`）的订阅抽成关联函数 `declarations()`，并发度 4（全仓唯一覆写并发度的消费者）；`message.created` 的业务收尾（`messages.status → Processed`）搬到 `impl Producer for MessageDalImpl::on_consumed`，`Pending`（启动恢复依据）搬到同对象 `on_failed`
- 本文类图随之更新：`Consumer` 去 `ack` / `nack`、增 `subscriptions()`；`Registry` 增 `register_producer` / `finish_consumption`，去 `ack` / `nack`
