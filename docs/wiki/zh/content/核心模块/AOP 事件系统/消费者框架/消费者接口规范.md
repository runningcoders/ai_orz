...
- 消费者 trait 删除 `ack` / `nack` / `source`；收尾统一在 `Registry::finish_consumption` 单出口，按 topic 反查 producer：`Ok → on_consumed → Ack`，`Err → decide_retry → RetryDecision{Retry(Nack)/Discard(Ack)}`。
- `RetryDecision { Retry, Discard }`：框架不设 `max_retry`、无死信，重试判定权下沉消费者（`decide_retry`）；`Discard` 仍回调 `on_consumed` 并打 `on_consume_discarded` 埋点（不计入失败率）。
...
## 架构总览
AOP 框架解耦生产与消费：Producer 经 EventSink 投递，Consumer 专注业务编排。Registry 统一分发事件，根据 consume_mode 选择同步或异步路径；异步路径通过 EventQueue 缓冲并由多 worker 拉取消费。消费结果进入 `finish_consumption` 单一出口，按 topic 反查已注册 producer 回调 `on_consumed`/`on_failed`，由 `decide_retry` 判定的 `RetryDecision` 决定 Ack（移除事件）或 Nack（退避重投），从而在不暴露 Consumer::ack/nack 的前提下保证可靠性。
...
- 异步模式：
  ...
  - on_event 失败：`Consumer::decide_retry` 判定 `RetryDecision`；`Retry` → 回调 `on_failed` 后走 queue.nack 退避重投，`Discard` → 回调 `on_failed` + `on_consumed` 后走 queue.ack 并打 `on_consume_discarded` 埋点（不计入失败率）。
  - dequeue 失败：sleep(error_retry_sleep_ms)。
  - 队列为空：sleep(empty_queue_sleep_ms)。
- 框架不设框架侧 max_retry、无死信：终局（重试 / 放弃）由消费者的 `decide_retry` 判定（永久错误码 + `DEFAULT_MAX_ATTEMPTS = 8`）。
- 业务侧需区分临时错误（返回 Err 以触发 Retry）与永久错误（由 `decide_retry` 落到 Discard），避免无限重投。
...
- 常见问题定位：
  ...
  - 无限重投：消费失败的收敛由 `Consumer::decide_retry` 负责（与 producer 无关）；`notify_producer=true` 的 topic 若未 `register_producer`，后果只是**没有数据收尾**（状态不翻转 / 游标不推进）。
...
## 结论
AOP 消费者接口通过统一的 Consumer 抽象与 Registry 调度，实现了高内聚、低耦合的事件驱动架构。消费收尾已收敛到 finish_consumption 单一出口，按 topic 反查 producer 完成 on_consumed/on_failed 回调，配合 RetryDecision 把重试判定权下沉消费者（`decide_retry`）；选择合适的消费模式、合理的并发与退避策略、严格的收尾语义与完善的监控集成，是构建稳定高效消费者的关键。
