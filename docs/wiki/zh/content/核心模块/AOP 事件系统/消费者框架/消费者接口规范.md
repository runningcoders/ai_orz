...
1. **签名更新**：`async fn on_event(&self, ctx: RequestContext, event: serde_json::Value) -> Result<()>` —— Sync 路径（`registry.rs#L177-L179`）和 Async worker 路径（`registry.rs#L385-L387`）均使用 `carried_ctx(&event_json)` 还原 ctx 后调用。

2. **carried_ctx 辅助方法**（`registry.rs#L229-L233`）：从事件 JSON 顶层提取 `context_carrier` 字段，通过 `ContextCarrier::from_json → into_context` 重建 RequestContext；缺失或解析失败时回退为 `RequestContext::new_system()`。
...
图表来源
- [src/pkg/aop/core/registry.rs:1-120](src/pkg/aop/core/registry.rs#L1-L120)
...
- [src/consumer/task_event_consumer.rs:38-138](src/consumer/task_event_consumer.rs#L38-L138)
- [src/consumer/tool_exec_log_consumer.rs:26-53](src/consumer/tool_exec_log_consumer.rs#L26-L53)
...
章节来源
- [src/pkg/aop/core/registry.rs:156-206](src/pkg/aop/core/registry.rs#L156-L206)
...
章节来源
- [src/consumer/scheduler.rs:39-95](src/consumer/scheduler.rs#L39-L95)
- [src/consumer/scheduler.rs:133-194](src/consumer/scheduler.rs#L133-L194)
