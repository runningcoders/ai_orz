...
- `RetryDecision { Retry, Discard }`：框架不设 `max_retry`、无死信队列，重试决策权下沉到生产者。
...
## 架构总览
事件从生产者发出后，Registry 根据事件类型路由到对应消费者；同步消费者立即处理，异步消费者入队并由 worker 循环拉取。消费结果统一进入 `finish_consumption` 单一出口：成功经 producer 回调 `on_consumed` 后 `Ack`，失败经 `on_failed` 得到 `RetryDecision`——`Retry` 走 `Nack` 退避重投，`Discard` 走 `Ack` 但仍回调 `on_consumed` 并打 `on_consume_discarded` 埋点（不计入失败率）。队列保证同 order_key 的顺序性与单活消息，避免重复处理导致的竞争。
...
- 无限重投（retry loop）：
  - 若某 topic 没有注册对应 producer，消费失败会走 `delivery_of` 兜底直接 `Nack`，导致该事件**无限重投**——需确认 `subscriptions()` 中 `notify_producer` 为 true 的 topic 都已 `register_producer`。
  - 检查 `on_failed` 返回的 `RetryDecision`：临时错误应返回 `Retry`，永久错误应通过 `inbound_retry::decide` 等落到 `Discard` 而非一味 `Retry`。
- 丢弃静默（discard-silent）：
...
## 结论
该消费者框架通过统一的 Consumer 抽象与 Registry 调度，实现了高内聚、低耦合的事件驱动架构。消费收尾已收敛到 `finish_consumption` 单一出口，按 topic 反查 producer 完成回调，配合 `RetryDecision { Retry, Discard }` 把重试决策权下沉生产者；内置消费者覆盖了消息、定时、Agent 循环、工具执行与统计等关键场景；内存队列提供了顺序、优先级与重试能力；指标钩子与前端仪表盘形成闭环监控。遵循同步/异步模式的选择策略与并发/退避配置，可在保证可靠性的同时获得良好性能。
