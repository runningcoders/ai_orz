...
- [src/pkg/aop/core/consumer.rs](src/pkg/aop/core/consumer.rs) — 消费者终局判定 `decide_retry` / `DEFAULT_MAX_ATTEMPTS`
</cite>
...
- `RetryDecision { Retry, Discard }`：框架不设 `max_retry`、无死信，重试判定权下沉消费者（`decide_retry`）
...
## 核心组件
- 生产者抽象 Producer：定义 `name()` / `topic()` / `on_consumed(ctx, event)` / `on_failed(ctx, event, err, decision, attempt)` / `start(sink)` / `stop()`。不再有 `register()` / `poll()` / `poll_interval_secs`。
...
- 收尾反查：消费者处理结束统一走 `finish_consumption`，按 topic 反查 `producer_for(kind)` → 回调 `on_consumed` / `on_failed`；重试或丢弃由 `decide_retry` 判定，结论传给 `on_failed`。
...
- 收尾：`mark_trigger_executed` 已移入 `CronTriggerProducer::on_consumed`；`CronTriggerConsumer::decide_retry` 恒返回 `Retry`（Discard 会 ack 且仍回调 `on_consumed` → 反把未执行标成「已执行」）。消费者 `CronTriggerConsumer` 订阅 `cron.trigger` 且声明 `notify_producer()`。
...
- 收尾：`on_consumed` 完成业务状态落库（如 `MessageDalImpl` 将 `messages.status` 置 `Processed`、`advance_inbound_cursor` 推进游标）；`on_failed` 经 `Consumer::decide_retry` 收敛（`MAX_ATTEMPTS=8`，永久错误码 → `Discard`，否则 `Retry`）。
...
## 错误处理、重试与失败恢复
- 生产者侧：
  - CronTriggerProducer：`registry` 未注册返回错误；无触发任务直接返回；`mark_trigger_executed` 失败记录日志。`decide_retry` 恒 `Retry`。
  - A2aPollingProducer：`claim` 中远端 `list_agents` 失败记录警告并跳过；`poll_agent` 失败由 `A2aPollConsumer` 侧经 `decide_retry` 默认策略判定。
  - DAL 生产者：`on_failed` 按 `decision` 更新底层数据（`Retry` → 置回 `Pending`；`Discard` → 不动）；判定由 `Consumer::decide_retry`（永久错误码 + `attempt >= 8`）给出。
...
- 消费者侧（框架）：
  ...
  - 无 producer 的 `Err`：`decide_retry` 照常判定，只是没有数据收尾回调。
...
- 实现 Producer trait：
  ...
  - `on_consumed` / `on_failed`：收尾回调；`RetryDecision { Retry, Discard }` 由消费者 `decide_retry` 判定，`on_failed` 接收结论更新底层数据。
- 注册与启动：
...
## 结论
AOP 生产者机制通过统一的 `Producer` 抽象与 Registry 编排，实现了循环与回调两种模式的灵活组合，内置定时触发、A2A 轮询与 DAL 入站生产者覆盖了典型的生产场景。`ProducerLoop` 自管可中断循环取代了旧的 Registry 轮询调度，`EventSink` 强制 1 producer : 1 topic，`RetryDecision` 将重试判定权下沉消费者（`decide_retry`），配合 `finish_consumption` 单点收尾，系统具备良好的可观测性与可维护性。扩展自定义生产者时，遵循分层与接口约定即可无缝集成。
