...
- [metrics_hook.rs#L18-L61](src/pkg/aop/core/metrics_hook.rs#L18-L61) (AopEventMeta 新增 context_carrier: Option<ContextCarrier> 字段)
- [registry.rs](src/pkg/aop/core/registry.rs)
- [registry.rs#L148-L156](src/pkg/aop/core/registry.rs#L148-L156) (publish 注入 ctx.to_carrier() 到 context_carrier)
- [registry.rs#L229-L233](src/pkg/aop/core/registry.rs#L229-L233) (carried_ctx 还原 helper)
- [aop_stats_collector.rs](src/consumer/aop_stats_collector.rs)
- [aop_stats_hook.rs](src/consumer/aop_stats_hook.rs)
- [aop_stats_hook.rs#L36-L120](src/consumer/aop_stats_hook.rs#L36-L120) (四回调追加 [aop] log_id 链路日志 + log_id_of 辅助函数)
...
## 更新摘要 2026-08-31：AopEventMeta 扩展 + AopStatsHook 链路可观测日志

**核心变更**：AopMetricsHook 体系新增 `context_carrier` 字段承载链路上下文，并在业务实现 AopStatsHook 中追加 log_id 链路日志。

1. **AopEventMeta 扩展**（`metrics_hook.rs#L27`）：新增 `pub context_carrier: Option<ContextCarrier>` 字段，`from_json` 方法同步从事件顶层提取并反序列化。该字段在 `AopStatsHook.log_id_of(meta)` 中被用来提取链路 log_id。

2. **Registry publish 注入**（`registry.rs#L148-L156`）：publish 统一元字段注入块之后，追加 `ctx.to_carrier()` 调用并序列化到事件 JSON 顶层 `context_carrier` 字段，确保 AopEventMeta.from_json 能同步解析到。

3. **AopStatsHook 链路可观测日志**（`aop_stats_hook.rs#L36-L120`）：四回调均追加 `sys_info!("[aop] log_id={} event={} ...", log_id_of(meta), meta.event_kind, ...)` 格式日志：
   - `on_publish`：`log_id={} event={} -> {} (mode={})` — 记录事件进入 AOP
   - `on_consume_start`：`log_id={} event={} <- {} consume start` — 记录开始消费
   - `on_consume_success`：`log_id={} event={} <- {} success (duration_ms={})` — 记录消费完成耗时
   - `on_consume_failure`：`log_id={} event={} <- {} failed (duration_ms={})` — 记录消费失败耗时

4. **log_id_of 辅助函数**（`aop_stats_hook.rs#L115-L120`）：从 `meta.context_carrier` 提取 log_id，缺失时回退 "unknown"。

**排查增益**：整条 AOP 事件链路（publish → consume_start → consume_success/failed）的 sys_info! 日志现在可以通过同一个 log_id 串联，定位问题时只需 grep log_id 即可看到完整事件流转轨迹。
