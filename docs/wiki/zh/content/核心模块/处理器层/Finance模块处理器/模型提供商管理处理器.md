...
### 本文关联的设计文档
- [系统初始化模型配置策略调整](docs/design/system_init_model_provider_strategy.md) — Embedding Provider 动态初始化、条件性模型创建、自动向量重建触发
- [用户身份凭证独立表落地](docs/plan/用户身份凭证独立表落地.md) — 统一凭证 CRUD、Tavily/Lark/GitHub 凭证迁移
</cite>

## 更新摘要
**变更内容（2026-08-19 增量更新）**
- 新增 Embedding Provider Handler 完整引用：switch_embedding、rebuild_vectors_task、rebuild_progress
- 新增凭证 Handler 引用：identity_credential 统一 Domain CRUD、Tavily 集成凭证 Handler
- 新增设计文档引用：用户身份凭证独立表落地
- 补充 Embedding Provider 切换与向量重建的处理器链路描述
- 更新前端页面引用：identity_tavily.rs 凭证管理页面
...
- [common/src/enums/provider.rs:1-154](common/src/enums/provider.rs#L1-L154)
- [src/models/brain.rs:1-86](src/models/brain.rs#L1-L86)
- [src/service/domain/finance/identity_credential.rs:1-454](src/service/domain/finance/identity_credential.rs#L1-L454) — 凭证统一 CRUD Domain
- [common/src/api/tavily_integration.rs:1-107](common/src/api/tavily_integration.rs#L1-L107) — Tavily 凭证 DTO
...
### 凭证集成（Tavily / Identity Credential）
- 统一凭证 Domain
  - `identity_credential_manage()` 提供类型无关的 create/update/delete/set_default CRUD，底层使用 user_credentials 独立表。
  - CredentialDetail 枚举支持 LarkApp、GithubPAT、TavilyKey 等变体，新增类型无需修改 Domain 层。
- Tavily 凭证 Handler
  - create_credential：手动录入 Tavily API key（加密落库，永不回显）
  - update_credential：更新凭证信息（key 留空保留原值）
  - delete_credential：删除凭证
  - set_default_credential：设置默认凭证（多条 key 时 tavily_search 工具身份优先）
  - get_status：聚合返回个人凭证快照（key 尾号 + 默认标记）+ 实例共享 key 配置状态
- 路由：`/api/v1/finance/identity/tavily/*`
- 前端页面：`identity_tavily.rs` 嵌入身份凭证聚合页（identity.rs）

章节来源
- [src/service/domain/finance/identity_credential.rs:1-454](src/service/domain/finance/identity_credential.rs#L1-L454)
- [src/handlers/finance/tavily_integration/create_credential.rs:1-35](src/handlers/finance/tavily_integration/create_credential.rs#L1-L35)
- [src/handlers/finance/tavily_integration/update_credential.rs:1-36](src/handlers/finance/tavily_integration/update_credential.rs#L1-L36)
- [src/handlers/finance/tavily_integration/delete_credential.rs:1-25](src/handlers/finance/tavily_integration/delete_credential.rs#L1-L25)
- [src/handlers/finance/tavily_integration/set_default_credential.rs:1-32](src/handlers/finance/tavily_integration/set_default_credential.rs#L1-L32)
- [src/handlers/finance/tavily_integration/get_status.rs:1-23](src/handlers/finance/tavily_integration/get_status.rs#L1-L23)
