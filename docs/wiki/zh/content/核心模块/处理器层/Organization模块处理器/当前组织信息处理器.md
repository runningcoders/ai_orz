...
- [migrations/20260830000000_add_org_config.sql](migrations/20260830000000_add_org_config.sql)
...
本次变更新增**组织级可扩展配置系统**，并在当前组织信息处理器中集成配置读写：

**Handler 层 DTO 扩展**：`OrganizationInfoResponse` 新增 `config: OrganizationConfig` 字段返回；`UpdateCurrentOrganizationRequest` 新增 `config: Option<OrganizationConfig>`，超级管理员可通过 PUT /api/v1/organization/me 修改组织级开关（如 `enable_message_vector`）。

**DAO 层 ORG_CONFIG_CACHE**：`src/service/dao/organization/sqlite.rs` 新增全局 `LazyLock<Mutex<HashMap<org_id, OrganizationConfig>>>` 缓存，`get_org_config` 读穿 + `set_org_config` 写穿双模式——Handler 读取组织信息时的配置查询走 DAO 缓存，避免每条消息落库回查 DB。

**DAL + Domain 透传**：`OrganizationDal` / `OrganizationManage` trait 新增 `get_org_config` / `update_org_config`，impl 层薄封装透传 DAO。

**下游消息 DAL 消费**：消息落库时经 DAO 缓存检查 `enable_message_vector`，默认 false 跳过向量索引构建（FTS 搜索不受影响）。

> ⚠️ 原「缓存策略与实时更新」小节中"当前实现未引入应用级缓存"的描述已过时——DAO 层现已引入 `ORG_CONFIG_CACHE` 全局配置缓存，其他组织数据（基本信息）仍走 DB 直读。
