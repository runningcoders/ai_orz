...
```
resolve_api_key(ctx)
  → 1. 用户凭证库（TavilyCredentialResolver）→ 命中 → UserCredential
  → 2. 共享 config（ai_orz.toml [tavily].api_key）→ 命中 → SharedConfig
  → 3. 皆缺 → api_key_missing 结构化引导
```
...
章节来源
- [tavily_search.rs:83-95](src/pkg/tool_registry/tavily_search.rs#L83-L95)
...
| tavily_search | key | `TavilyKeyProbe`（双轨：共享 config + 用户凭证库） |
...
tool_readiness.rs
  ...
  └── tavily_search.rs ← 复用 api_key_missing_json()
```
...
### Troubleshooting 6：Tavily 搜索报 "api_key_missing"

**排查步骤**：
1. 检查 `ai_orz.toml` `[tavily].api_key` 是否已配置共享 key
2. 或检查用户是否在"设置 → 身份凭证"绑定了个人 Tavily key
3. 确认 `TavilyCredentialResolver` 是否已在 `service::init` 中注册
4. 源码位置：[tavily_search.rs:83-95](src/pkg/tool_registry/tavily_search.rs#L83-L95)
