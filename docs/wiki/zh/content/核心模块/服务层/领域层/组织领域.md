...
- [domain/mod.rs](src/service/domain/organization/mod.rs)
- [org.rs](src/service/domain/organization/org.rs)
- [user.rs（Domain）](src/service/domain/organization/user.rs)
- [organization.rs（DAL）](src/service/dal/organization.rs)
- [organization/mod.rs（DAO）](src/service/dao/organization/mod.rs)
...
- [organization.rs（DAL 实现）](src/service/dal/organization.rs)
...
本次变更新增**组织级可扩展配置系统**，为组织领域提供持久化的维度开关能力：

**新增 DB 层**：`ALTER TABLE organizations ADD COLUMN config TEXT NOT NULL DEFAULT '{}'`，采用 JSON 文本承载可扩展配置，后续新增配置项无需改表结构。

**新增 DTO 层**：`OrganizationConfig { enable_message_vector: bool }`，所有字段带 `#[serde(default)]` 默认值保证向后兼容；Handler 层 `OrganizationInfoResponse` / `UpdateCurrentOrganizationRequest` / `UpdateOrganizationRequest` 同步扩展 config 字段。

**DAO 层缓存**：新增 `ORG_CONFIG_CACHE` 全局 `LazyLock<Mutex<HashMap<org_id, OrganizationConfig>>>` 缓存，`get_org_config` 采用读穿（命中直接返回，未命中回退 DB 并回填），`set_org_config` 采用写穿（DB 落盘后同步刷新）；`read_org_config_from_db` 对空/非法 JSON 回退默认配置。

**DAL + Domain 层透传**：`OrganizationDal` / `OrganizationManage` trait 新增 `get_org_config` / `update_org_config`，impl 层透传调用，保持分层薄封装。

**下游消费点**：消息 DAL 落库时检查 `org_config.enable_message_vector`（经 DAO 缓存读取），默认 false 跳过向量索引构建，避免每条消息回查 DB。
...
- [domain/mod.rs:1-200](src/service/domain/organization/mod.rs#L1-L200)
- [organization.rs（DAL）:1-126](src/service/dal/organization.rs#L1-L126)
- [organization/mod.rs（DAO）:1-40](src/service/dao/organization/mod.rs#L1-L40)
- [organization.rs:1-62](src/models/organization.rs#L1-L62)
- [user.rs:1-98](src/models/user.rs#L1-L98)
- [organization.rs（枚举）:1-101](common/src/enums/organization.rs#L1-L101)
- [user.rs（枚举）:1-212](common/src/enums/user.rs#L1-L212)
...
图表来源
- [initialize_system.rs:134-224](src/handlers/organization/initialize_system.rs#L134-L224)
- [org.rs:53-90](src/service/domain/organization/org.rs#L53-L90)
- [organization.rs（DAL）:93-95](src/service/dal/organization.rs#L93-L95)
- [organization/mod.rs（DAO）:14-33](src/service/dao/organization/mod.rs#L14-L33)
...
章节来源
- [org.rs:1-134](src/service/domain/organization/org.rs#L1-L134)
- [organization.rs（DAL）:1-126](src/service/dal/organization.rs#L1-L126)
- [organization/mod.rs（DAO）:1-40](src/service/dao/organization/mod.rs#L1-L40)
...
- [user.rs（Domain）:85-117](src/service/domain/organization/user.rs#L85-L117)

章节来源
- [user.rs（Domain）:1-124](src/service/domain/organization/user.rs#L1-L124)
- [jwt_auth.rs:1-156](src/middleware/jwt_auth.rs#L1-L156)
...
章节来源
- [jwt_auth.rs:56-84](src/middleware/jwt_auth.rs#L56-L84)
- [user.rs:13-26](src/models/user.rs#L13-L26)
- [organization.rs:13-27](src/models/organization.rs#L13-L27)
...
章节来源
- [CODE_WIKI.md:130-161](docs/CODE_WIKI.md#L130-L161)
...
- [require_role.rs:20-38](src/middleware/require_role.rs#L20-L38)
- [user.rs（Domain）:85-117](src/service/domain/organization/user.rs#L85-L117)
- [initialize_system.rs:251-284](src/handlers/organization/initialize_system.rs#L251-L284)
...
章节来源
- [organization_design.md:73-102](docs/organization_design.md#L73-L102)