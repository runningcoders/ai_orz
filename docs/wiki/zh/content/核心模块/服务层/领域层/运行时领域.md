...
- [src/pkg/policy/builtin.rs#L1-L361](src/pkg/policy/builtin.rs#L1-L361)
- [src/pkg/policy/tests.rs](src/pkg/policy/tests.rs)
- [src/pkg/tool_registry/shell_policy.rs](src/pkg/tool_registry/shell_policy.rs)
...
- [src/service/domain/runtime/abort_summary.rs](src/service/domain/runtime/abort_summary.rs#L1-L505)

**更新摘要 2026-09-04**：新增 `abort_summary.rs`（完全不依赖 LLM 的规则化兜底总结），为 think 循环异常退出（429/5xx/超时/用户取消）提供保底记忆落库路径——避免循环因 LLM 不通而蒸发已完成进度；think_loop 进度快照位置调整为「每轮工具执行完后立即落袋」，为兜底摘要提供数据源；`attach_abort_notice` 把进度概览挂到 Error.field.extra.abort_notice 传递给消费者，消息失败通知里告知用户「工作没白做」。

**更新摘要 2026-09-07**：策略引擎迭代——新增 `PolicyAction` 三值枚举（Deny/Confirm/Audit）+ trait `action()` 默认 None 向后兼容 + PolicyGroup 自动上浮首个命中策略的动作；think_loop 全策略化重构：`build_policy_for_scene` 统一 8 策略 OR 组（含 FinalAnswerPolicy 正常完成裁决 + ConsecutiveLlmErrorsPolicy 错误路径裁决），Final 和工具调用统一经 `policy_exit`；错误路径（brain_dal think 失败）也过策略，预算内立即重试。Shell 拦截层（`shell_policy.rs`）落地通用 Policy trait——ShellRulePolicy + 5 条静态规则表（scope 越界 Confirm / 身份边界越界 Confirm / 破坏性命令 Confirm / git push-reset-clean Confirm / git commit Audit）。废弃 mixed.rs（PolicyMixed 硬软分层）和 TokenCostPolicy（替换为 TokenBudgetPolicy）。
...
- 【② Plan 落地快照】
  ...
  - （2026-09-04 清理：原 superpowers/plans/ 已归档，相关 policy_engine plan 尚未落地到 plan-archive，待 doc-maintainer 跟进）
- 【④ RAG 原子知识卡】
  - [策略引擎：Policy trait + PolicyGroup 嵌套组合 + policy_set! 宏声明式写法 + PolicyAction 动作上浮 + Shell 拦截层](docs/wiki/knowledge/zh/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法.md) — 策略引擎框架硬约束
  ...
  - [思考循环异常退出兜底总结与错误上报结构化](docs/wiki/knowledge/zh/思考循环异常退出兜底总结与错误上报结构化/思考循环异常退出兜底总结与错误上报结构化.md) — abort_summary.rs 纯规则化兜底摘要 + 进度快照 + attach_abort_notice 错误上报（2026-09-04 新增）
  - 【Batch4 新增 2 张】
  ...
  - 【2026-09-07 新增】
  - [策略引擎：Policy trait + PolicyGroup 嵌套组合 + policy_set! 宏声明式写法 + PolicyAction 动作上浮 + Shell 拦截层 + PolicyAction 动作上浮 + Shell 拦截层](docs/wiki/knowledge/zh/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法%20+%20PolicyAction%20动作上浮%20+%20Shell%20拦截层/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法%20+%20PolicyAction%20动作上浮%20+%20Shell%20拦截层.md) — PolicyAction 三值枚举 + think_loop 全策略化 + Shell 拦截层落地
  - [Shell 工具全链路：shell_exec 内置执行 + shell_policy 策略拦截 + shell_tool 声明式注册](docs/wiki/knowledge/zh/Shell%20工具全链路：shell_tool%20注册%20+%20shell_policy%20拦截%20+%20shell_exec%20执行/Shell%20工具全链路：shell_tool%20注册%20+%20shell_policy%20拦截%20+%20shell_exec%20执行.md) — shell_exec 与 shell_tool 双执行通道同策略管线
- 【③ Wiki 关联长文】
...
- [策略引擎：Policy trait + PolicyGroup 嵌套组合 + policy_set! 宏声明式写法 + PolicyAction 动作上浮 + Shell 拦截层](docs/wiki/knowledge/zh/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法/策略引擎：Policy%20trait%20+%20PolicyGroup%20嵌套组合%20+%20policy_set!%20宏声明式写法.md) — §红线 2 禁止 awakening 中手工 if/else 判断策略；§红线 6 PolicyMixed hard_hit 必须强制退出
