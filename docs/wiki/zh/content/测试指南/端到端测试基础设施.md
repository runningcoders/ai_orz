...
- [docs/archive/design-archive/browser_e2e_test_design.md](docs/archive/design-archive/browser_e2e_test_design.md)
</cite>
...
H["E2E 配置与脚本<br/>tests/e2e/README.md"]
end
A --> D
B --> D
C --> A
...
- **关键测试文件**：
  ...
  - `tests/integration/model_provider_embedding_test.rs`：Embedding Provider 生命周期集成测试（创建/禁用/切换/重建/上下文长度）
- **E2E 入口**：
  - `make e2e`：Playwright E2E 本地运行入口（`cd tests/e2e && npx playwright test`）
  - `tests/e2e/playwright.config.ts`：Playwright 配置（`outputDir: ./test-results`，失败视频自动保存）
- **服务层初始化**：
