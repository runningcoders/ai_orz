...
- [src/config.rs:90-L140](src/config.rs#L90-L140) — server.trust_proxy 配置项定义
- [src/middleware/proxy.rs:1-L60](src/middleware/proxy.rs#L1-L60) — 反向代理头解析（X-Forwarded-*）
- [src/middleware/sse_headers.rs:1-L40](src/middleware/sse_headers.rs#L1-L40) — SSE 防缓冲（Cache-Control/no-transform）
- [配置系统：嵌入默认 TOML + 运行时加载 + 环境变量覆盖 + 前端编译期注入](docs/wiki/knowledge/zh/配置系统：嵌入默认 TOML + 运行时加载 + 环境变量覆盖 + 前端编译期注入/配置系统：嵌入默认 TOML + 运行时加载 + 环境变量覆盖 + 前端编译期注入.md)
- 【🔧 Plan 落地】docs/plan/容器化与网关部署方案.md — Dockerfile 三阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像
- 【🎴 RAG 知识卡】docs/wiki/knowledge/zh/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像.md
</cite>
...
**2026-08-23 增量更新**：环境变量新增 `AI_ORZ_LISTEN_ADDR`（监听地址，直接对齐 `server.listen_addr`，取代旧 `SERVER_PORT` 端口提示）与 `SECRET_KEY`（敏感数据加密密钥）；新增**首次初始化固化**机制——首次生成配置时把 `AI_ORZ_LISTEN_ADDR`/`JWT_SECRET`/`JWT_EXPIRY_HOURS`/`FRONTEND_DIST_DIR`/`SECRET_KEY` 的环境变量偏好定向写回配置文件（保留模板注释）；JWT 回退链改为 env → `jwt.secret` → 内置默认。
...
- 监听地址：AI_ORZ_LISTEN_ADDR 直接覆盖 server.listen_addr（host:port，与配置字段对齐）。
- 安全密钥：SECRET_KEY 覆盖 security.secret_key（数据库敏感字段加密）。
- JWT 相关：
  - JWT_SECRET：签名密钥（生产环境必须设置；回退链 env → jwt.secret → 内置默认）
  - JWT_EXPIRY_HOURS：默认过期小时数
- 前端静态目录：FRONTEND_DIST_DIR 可覆盖配置中的 dist_dir。
- **首启固化**：以上映射到配置字段的环境变量在首次初始化时写回 .ai_orz/ai_orz.toml（定向替换、保留注释），后续环境变量仅本次运行生效、不再改写文件。
...
### Docker 容器化配置
仓库已提供官方容器化入口：`Dockerfile` + `docker-compose.yml` + `Caddyfile` + `.env.deploy.example`。

- **镜像分层**（[Dockerfile](Dockerfile)）：
  - `frontend-builder`：Node（Tailwind 编译）+ wasm32 + dioxus-cli，复用 `scripts/build_frontend.sh` 产出 dist/
  - `backend-builder`：`cargo build --release --locked`（`SQLX_OFFLINE=true`，`.sqlx/` 已入库，纯离线编译）
  - `runtime`：debian-slim + git/bash/curl/procps/tzdata，非 root 用户（uid 10001）
- **多架构**：amd64 / arm64 各自在原生 runner 上构建后合并 manifest，不用 QEMU（duckdb C++ 源码编译 + fastembed 下载 ONNX Runtime 二进制，模拟构建慢且易失败）；tag 触发推 `<版本号>` + `latest`。
- **运行参数**：
  - 环境变量：`AI_ORZ_BASE_PATH`（镜像内默认 `/data`）、`AI_ORZ_LISTEN_ADDR`、`AI_ORZ_PUBLIC_BASE_URL`、`AI_ORZ_TRUST_PROXY`、`AI_ORZ_COOKIE_SECURE`、`SECRET_KEY`、`JWT_SECRET`
  - 挂载卷：只需挂载一个数据根目录（命名卷 `ai_orz_data`），SQLite / 向量库 / 工作区 / 日志 / 配置全在其下；改 bind mount 时宿主目录需 `chown 10001:10001`
  - 端口：compose 只绑 `127.0.0.1:3000`，公网入口由网关反代
- **内置网关（默认 Caddy）**：`docker-compose.yml` 里的 `caddy` 服务属于 `gateway` profile，配置在仓库根目录 [Caddyfile](Caddyfile)。

  - 生产启动：`cp .env.deploy.example .env`（填 `SECRET_KEY` / `JWT_SECRET` / `AI_ORZ_DOMAIN`）后 `docker compose --profile gateway up -d`
  - 无域名的本机试用：`docker compose up -d`，直连 `127.0.0.1:3000`
  - 证书存在 `caddy_data` 卷，**必须持久化**，丢失会重复触发 Let's Encrypt 速率限制
  - 裸机部署用同一份 `Caddyfile` + systemd drop-in 注入环境变量，见部署方案文档 §2.7
- **置于网关之后**：必须设 `AI_ORZ_TRUST_PROXY=true`（否则日志里客户端 IP 全是网关 IP），并设 `AI_ORZ_PUBLIC_BASE_URL` 为网关域名（联邦 peer 自报地址）。
- **不含 Node/Python**：MCP stdio 需要时可基于官方镜像自建（扩展示例见部署方案文档）。

完整拓扑、Caddy/Nginx/Traefik 参考配置与联邦适配见 [容器化与网关部署方案](docs/plan/容器化与网关部署方案.md)。

章节来源
- [Dockerfile](Dockerfile)
- [docker-compose.yml](docker-compose.yml)
- [Caddyfile](Caddyfile)
- [src/config.rs:90-140](src/config.rs#L90-L140)
...
- AI_ORZ_LISTEN_ADDR：监听地址（覆盖 server.listen_addr，首启固化）
- SECRET_KEY：敏感数据加密密钥（覆盖 security.secret_key，首启固化）
- JWT_SECRET：JWT 签名密钥（生产必须；回退链 env → jwt.secret → 内置默认，首启固化）
...
### 本文关联的文档
- 🔧 Plan: docs/plan/容器化与网关部署方案.md
- 🎴 RAG 卡: docs/wiki/knowledge/zh/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像/容器化与网关部署：Dockerfile 多阶段 + Caddyfile reverse_proxy + release.yml 多架构镜像.md
- 🎴 RAG 卡: docs/wiki/knowledge/zh/配置系统：嵌入默认 TOML + 运行时加载 + 环境变量覆盖 + 前端编译期注入/配置系统：嵌入默认 TOML + 运行时加载 + 环境变量覆盖 + 前端编译期注入.md

---

### 更新摘要（2026-09-10，base d070a076→HEAD）
**主题**：容器化部署链路完善 + trust_proxy 安全前提明确 + SSE headers / proxy middleware 新增
**关键变更**：
1. Dockerfile 三阶段（backend Rust builder → runtime slim + frontend Dioxus WASM 注入）+ Caddyfile reverse_proxy + docker-compose.yml 两服务（后端 + Caddy 网关 profile）+ .dockerignore + .env.deploy.example
2. server.trust_proxy 安全前提——config.server.trust_proxy = false 时 proxy_middleware 忽略所有 X-Forwarded-* 头；仅 Caddy/Nginx reverse_proxy 后才设为 true（防止 Header 注入）
3. 新增 middleware/proxy.rs（反向代理头解析）+ middleware/sse_headers.rs（SSE 防缓冲 Cache-Control/no-transform）
4. release.yml 多架构镜像（amd64/arm64）各自在原生 runner 上构建后合并 manifest
5. 环境变量清单新增 AI_ORZ_TRUST_PROXY / AI_ORZ_PUBLIC_BASE_URL / AI_ORZ_COOKIE_SECURE
**涉及 RAG 卡**：容器化与网关部署卡、配置系统卡