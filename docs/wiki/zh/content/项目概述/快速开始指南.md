...
**④ RAG 原子知识卡**：
- [scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正](docs/wiki/knowledge/zh/scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正/scripts 归置 + Makefile 统一入口 + 60+ 引用全量修正.md) — §红线 2 新用户一律用 `make run` / `make build`，不直接执行 scripts/ 下脚本；§红线 3 所有常用命令必须有 Makefile target 暴露
</cite>

## 更新摘要
**2026-08-16 增量更新**：补齐 cite 区四类互引闭环（1 Design+1 Plan 规范占位 + 1 RAG 卡）；关联 T8（scripts/ 归置 + Makefile 统一入口）主题；快速开始启动命令统一更新为 `make run`（取代旧版裸 `./start.sh`）；§3 核心组件、§5 详细组件分析章节来源 start.sh/Makefile 行号降级（因 7395ce18 scripts/ 归置新增 Makefile 24 个 target 导致行号漂移，优先降级为无行号范围引用）。
