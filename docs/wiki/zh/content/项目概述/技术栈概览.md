...
- [browser_e2e_test_design.md](docs/archive/design-archive/browser_e2e_test_design.md) — dioxus-playwright 选型；E2E 本地独立启动；3 大类页面覆盖
- [seed-config-migration.md](docs/archive/design-archive/seed-config-migration.md) — 5 套 TEMPLATE_SKILL 编译期嵌入；两阶段 init/aop 严格分离；SeedSnapshot Diff 增量导入
#### ② Plan 落地快照（Batch10 追加 1 张）
- [Agent管理集成测试.md](docs/archive/plan-archive/Agent管理集成测试.md) — 19 targets 集成测试；向量降级 fallback 保证 CI 稳定
