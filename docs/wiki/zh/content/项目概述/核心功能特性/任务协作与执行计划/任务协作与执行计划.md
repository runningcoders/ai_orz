...
- [src/service/domain/project/task_graph.rs](src/service/domain/project/task_graph.rs)
...
- [migrations/20260806000001_execution_plan_result.sql](migrations/20260806000001_execution_plan_result.sql)
...
## 更新摘要
**所做更改**
- 新增任务依赖图可视化功能，支持Mermaid图表展示
- 增强Markdown渲染能力，支持execution_plan和execution_result的富文本显示
- 扩展前端组件库，新增MermaidDiagram组件
- 完善项目详情页面的任务依赖关系可视化
...
**最新更新**：新增任务依赖图的Mermaid可视化功能，支持在项目详情中直观展示任务间的依赖关系和执行流向。
...
TG["任务依赖图构建器<br/>domain/project/task_graph"]
end
subgraph "数据访问层"
...
- [src/service/domain/project/task_graph.rs:1-67](src/service/domain/project/task_graph.rs#L1-L67)
...
- 领域服务（Domain）
  ...
  - TaskGraph构建器：基于dependencies字段构建DAG并渲染为Mermaid字符串。
- 数据访问层（DAL）
...
- 前端组件
  - MarkdownRenderer：通用Markdown渲染组件，支持表格、删除线、任务列表等扩展语法。
  - MermaidDiagram：独立Mermaid图表渲染组件，用于展示任务依赖关系。
...
- [src/service/dal/task.rs:1-800](src/service/dal/task.rs#L1-L800)
- [src/consumer/task_event_consumer.rs:1-138](src/consumer/task_event_consumer.rs#L1-L138)
- [frontend/src/components/markdown.rs:1-210](frontend/src/components/markdown.rs#L1-L210)
...
- 可视化展示：项目详情页面支持Mermaid图表展示任务依赖关系。
...
**图表来源**
- [src/service/domain/project/task_graph.rs:17-54](src/service/domain/project/task_graph.rs#L17-L54)
...
- 前端展示
  - 项目详情和任务详情页面支持Markdown格式的执行计划和结果展示。
  - 使用MarkdownRenderer组件进行富文本渲染，支持表格、任务列表等语法。
...
**图表来源**
- [migrations/20260806000001_execution_plan_result.sql:1-9](migrations/20260806000001_execution_plan_result.sql#L1-L9)
- [src/service/domain/project/task.rs:227-279](src/service/domain/project/task.rs#L227-L279)

**章节来源**
- [migrations/20260806000001_execution_plan_result.sql:1-9](migrations/20260806000001_execution_plan_result.sql#L1-L9)
- [src/service/domain/project/task.rs:227-279](src/service/domain/project/task.rs#L227-L279)
- [src/models/task.rs:59-62](src/models/task.rs#L59-L62)
...
- [src/service/dal/task.rs:690-800](src/service/dal/task.rs#L690-L800)

### 任务依赖图可视化（新增功能）
- 前端集成
  - 项目详情页面新增"任务依赖图"区块，使用MermaidDiagram组件渲染任务依赖关系。
  - 支持按需加载（with_task_graph参数），避免不必要的计算开销。
- Markdown渲染增强
  - MarkdownRenderer组件支持Mermaid代码块渲染，自动检测language-mermaid代码块。
  - 通过window.__renderMermaid全局函数实现Mermaid图表的动态渲染。
- 后端支持
  - GetProjectResponse新增task_graph字段，支持按需返回任务依赖图。
  - 任务依赖图基于dependencies字段构建，支持跨项目依赖的external节点标记。

```mermaid
flowchart LR
Frontend["前端项目详情页"] --> API["GetProjectRequest<br/>with_task_graph=true"]
API --> Backend["后端领域服务"]
Backend --> GraphBuilder["任务依赖图构建器"]
GraphBuilder --> Mermaid["Mermaid字符串"]
Mermaid --> Frontend
Frontend --> MermaidComponent["MermaidDiagram组件"]
MermaidComponent --> SVG["SVG图表渲染"]
```

**图表来源**
- [frontend/src/pages/project/project_detail.rs:443-451](frontend/src/pages/project/project_detail.rs#L443-L451)
- [frontend/src/components/markdown.rs:92-115](frontend/src/components/markdown.rs#L92-L115)
- [common/src/api/project.rs:138-140](common/src/api/project.rs#L138-L140)
...
- 图表渲染优化
  - Mermaid图表采用按需加载模式，通过with_task_graph参数控制是否生成。
  - Markdown渲染使用use_memo缓存HTML，避免重复解析。
  - Mermaid图表渲染延迟30ms确保DOM完全挂载后再执行。

**章节来源**
- [src/service/dal/task.rs:366-571](src/service/dal/task.rs#L366-L571)
- [src/service/dal/task.rs:723-800](src/service/dal/task.rs#L723-L800)
- [src/consumer/task_event_consumer.rs:52-136](src/consumer/task_event_consumer.rs#L52-L136)
- [frontend/src/components/markdown.rs:67-76](frontend/src/components/markdown.rs#L67-L76)
...
- Mermaid图表不显示
  - 检查with_task_graph参数是否正确传递；确认Mermaid.js库是否成功加载；验证DOM元素ID是否正确生成。
- Markdown渲染异常
  - 检查content是否为空；确认pulldown-cmark配置是否正确；验证dangerous_inner_html注入是否安全。
...
**最新更新亮点**：新增的任务依赖图可视化功能显著提升了用户体验，通过Mermaid图表直观展示任务间的依赖关系和执行流向，帮助用户更好地理解项目结构和任务执行顺序。
...
**项目详情API增强**：
- 获取项目详情
  - 查询参数：with_task_graph（可选，默认false）- 是否返回任务依赖图
  - 响应体：GetProjectResponse（包含task_graph字段）
