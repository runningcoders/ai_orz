...
**本文关联三类文档（四类互引闭环）**
- 【① Design 决策快照】
  - [memory_system_enhancement_design.md](docs/design/memory_system_enhancement_design.md) — 记忆系统三阶段增强（工具拆分 + 定时沉淀 + 搜索增强）总设计
  - [memory_search_enhancement_design.md](docs/design/memory_search_enhancement_design.md) — 三位一体搜索（FTS5 + 向量 + 图谱关系）
- 【② Plan 落地快照（真实定稿）】
  - [图谱遍历查询优化.md](docs/plan/图谱遍历查询优化.md) — 图谱遍历 IN 分块 400 + DFS 栈批量预取（记忆系统图谱侧性能优化专项）
  - [知识图谱推荐起点与组件复用重构.md](docs/plan/知识图谱推荐起点与组件复用重构.md) — 推荐起点 + KnowledgeGraph 组件两端复用
...
#### ① Design 决策快照
- [memory_system_enhancement_design.md](docs/design/memory_system_enhancement_design.md) — 四层记忆系统增强三阶段：工具拆分（save_short/long_term）+ 定时框架（ensure_system_cron_triggers 每天 4 点）+ 休息沉淀（Resting 小憩 / 睡眠）
#### ② Plan 落地快照
- [唤醒上下文与睡眠约束.md](docs/plan/唤醒上下文与睡眠约束.md) — Settle 场景双层工具过滤只保留 neural/memory，Agent 沉淀绝不发消息
#### ④ RAG 原子知识卡
