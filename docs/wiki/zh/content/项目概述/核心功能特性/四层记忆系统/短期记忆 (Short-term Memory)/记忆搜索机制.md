...
**本文关联三类文档（四类互引闭环）**
- 【① Design 决策快照】
  - [memory_search_enhancement_design.md](docs/design/memory_search_enhancement_design.md) — §1 关键决策表（FTS5 选型 + 死代码清理 + 搜索/查询分离）
  - [vector_search_architecture.md](docs/design/vector_search_architecture.md) — VectorStore 抽象 + 多后端 + Vectorizable trait 统一索引入口（本长文 §3-5 的底层）
- 【② Plan 落地快照（真实定稿 2 张）】
  - [图谱遍历查询优化.md](docs/plan/图谱遍历查询优化.md) — §6 图谱遍历 BFS/DFS + IN 400 分块优化真实落地
  - [知识图谱推荐起点与组件复用重构.md](docs/plan/知识图谱推荐起点与组件复用重构.md) — recommend_seed_nodes + 前端组件拆分真实定稿
