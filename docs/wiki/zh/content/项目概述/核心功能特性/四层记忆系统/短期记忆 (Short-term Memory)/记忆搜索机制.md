...
- [sqlite.rs](src/service/dao/memory/sqlite.rs)
- [vector.rs](src/service/dao/memory/vector.rs)

**本文关联三类文档（四类互引闭环）**
- 【① Design 决策快照】
  - [memory_search_enhancement_design.md](docs/archive/design-archive/memory_search_enhancement_design.md) — §1 关键决策表（FTS5 选型 + 死代码清理 + 搜索/查询分离）
  - [vector_search_architecture.md](docs/archive/design-archive/vector_search_architecture.md) — VectorStore 抽象 + 多后端 + Vectorizable trait 统一索引入口（本长文 §3-5 的底层）
- 【② Plan 落地快照（真实定稿 2 张）】
  - [图谱遍历查询优化.md](docs/archive/plan-archive/图谱遍历查询优化.md) — §6 图谱遍历 BFS/DFS + IN 400 分块优化真实落地
  - [知识图谱推荐起点与组件复用重构.md](docs/archive/plan-archive/知识图谱推荐起点与组件复用重构.md) — recommend_seed_nodes + 前端组件拆分真实定稿
- 【④ RAG 原子知识卡（Batch5 新增 2 张 + 存量 1 张 + 关联 1 张）】
  - [知识图谱 traverse：BFS levels 深度返回 + DFS 栈批量预取 edge_cache + IN 列表 400 分块防 999 溢出](docs/wiki/knowledge/zh/知识图谱%20traverse：BFS%20levels%20深度返回%20+%20DFS%20栈批量预取%20edge_cache%20+%20IN%20列表%20400%20分块防%20999%20溢出/知识图谱%20traverse：BFS%20levels%20深度返回%20+%20DFS%20栈批量预取%20edge_cache%20+%20IN%20列表%20400%20分块防%20999%20溢出.md) — 本长文 §6 图谱遍历底层实现细节
  - [recommend_seed_nodes 种子节点推荐：三因子打分 0.45 连通度 0.35 内容丰富度 0.2 分享权重 + KnowledgeGraph 组件两端复用](docs/wiki/knowledge/zh/recommend_seed_nodes%20种子节点推荐：三因子打分%200.45%20连通度%200.35%20内容丰富度%200.2%20分享权重%20+%20KnowledgeGraph%20组件两端复用/recommend_seed_nodes%20种子节点推荐：三因子打分%200.45%20连通度%200.35%20内容丰富度%200.2%20分享权重%20+%20KnowledgeGraph%20组件两端复用.md) — 图谱起步入口与本长文的「图谱扩展用户视角」互补
  - 【存量总览卡】[记忆领域搜索增强：FTS5 tags 语义过滤 + knowledge graph traverse 图谱遍历 + recommend_seed_nodes 种子节点推荐](docs/wiki/knowledge/zh/记忆领域搜索增强：FTS5%20tags%20语义过滤%20+%20knowledge%20graph%20traverse%20图谱遍历%20+%20recommend_seed_nodes%20种子节点推荐/记忆领域搜索增强：FTS5%20tags%20语义过滤%20+%20knowledge%20graph%20traverse%20图谱遍历%20+%20recommend_seed_nodes%20种子节点推荐.md) — 三位一体搜索总览（tags 过滤 + 图谱扩展 + 推荐起点）
  - 【关联】[三位一体混合搜索：FTS5 关键词 + 向量语义 + 合并排序（6 DAO 统一 search 模式 + 向量失败降级）](docs/wiki/knowledge/zh/三位一体混合搜索：FTS5%20关键词%20+%20向量语义%20+%20合并排序（6%20DAO%20统一%20search%20模式%20+%20向量失败降级）/三位一体混合搜索：FTS5%20关键词%20+%20向量语义%20+%20合并排序（6%20DAO%20统一%20search%20模式%20+%20向量失败降级）.md) — 记忆域搜索是整个 6 DAO 混合搜索标准的子集
- 【③ Wiki 关联长文】
  - [四层记忆系统.md](docs/wiki/zh/content/项目概述/核心功能特性/四层记忆系统/四层记忆系统.md) — 四层认知模型总入口（本长文 §简介 的上游）
  - [短期记忆索引.md](docs/wiki/zh/content/项目概述/核心功能特性/四层记忆系统/短期记忆%20(Short-term%20Memory)/短期记忆索引.md) — 短期记忆 FTS5 索引构建（FTS5 搜索的底层数据来源）
  - [记忆系统架构.md](docs/wiki/zh/content/架构设计/记忆系统架构.md) — §搜索扩展层完整架构
...
图表来源
- [fts5.rs:1-45](src/pkg/storage/fts5.rs#L1-L45)
- [vector.rs:18-74](src/pkg/storage/vector.rs#L18-L74)
- [hnsw.rs:149-166](src/pkg/storage/hnsw.rs#L149-L166)
- [sqlite.rs:383-484](src/service/dao/memory/sqlite.rs#L383-L484)
- [vector.rs:36-147](src/service/dao/memory/vector.rs#L36-L147)
- [memory.rs:188-276](src/service/dal/memory.rs#L188-L276)

章节来源
- [vector_search_architecture.md:11-55](docs/vector_search_architecture.md#L11-L55)
...
- [memory.rs:188-276](src/service/dal/memory.rs#L188-L276)
- [sqlite.rs:383-484](src/service/dao/memory/sqlite.rs#L383-L484)
- [vector.rs:67-93](src/service/dao/memory/vector.rs#L67-L93)
...
- [vector.rs:18-74](src/pkg/storage/vector.rs#L18-L74)
- [hnsw.rs:149-166](src/pkg/storage/hnsw.rs#L149-L166)
- [hnsw.rs:432-616](src/pkg/storage/hnsw.rs#L432-L616)
...
- [sqlite.rs:383-484](src/service/dao/memory/sqlite.rs#L383-L484)
- [hnsw.rs:493-543](src/pkg/storage/hnsw.rs#L493-L543)
- [vector.rs:126-173](src/pkg/storage/vector.rs#L126-L173)
...
章节来源
- [vector_search_architecture.md:46-55](docs/vector_search_architecture.md#L46-L55)
...
- [memory.rs:654-799](src/service/dal/memory.rs#L654-L799)
- [sqlite.rs:383-484](src/service/dao/memory/sqlite.rs#L383-L484)
- [hnsw.rs:191-218](src/pkg/storage/hnsw.rs#L191-L218)
- [vector.rs:245-262](src/pkg/storage/vector.rs#L245-L262)
