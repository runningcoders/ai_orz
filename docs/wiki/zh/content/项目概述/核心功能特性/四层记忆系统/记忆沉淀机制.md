...
## 项目结构
记忆沉淀涉及四层协作：
- 生产者层：CronTriggerProducer 每分钟扫描到期触发器，发布 cron.trigger 事件（发布后才 `mark_trigger_executed` 推进 `next_run_at`）
...
2. **新增 `AgentSettleEvent`（src/models/events/agent_settle.rs），并把它并入 `agent.awakening` 消费者**——不新建独立消费者是**关键决策**：AOP 的 `order_key` 闸门按消费者隔离（`registry.queues: HashMap<consumer_name, EventQueue>`），拆成两个消费者会让同一个 `agent_id` 落在两条互不知晓的队列里，串行保证静默失效。合并后两类事件同队列同 `order_key = agent_id`：沉淀在跑时消息压根不出队（不失败、不重试、不刷日志），`concurrency = 4`；`resource_not_found` → ack 作废
   > 初版曾用 `NackMode::Defer` + `set_idle` 主动唤醒兜底抢占失败，已**整体回滚**（抢占失败本就不该发生；`Consumer::ack/nack` 只保留了 `source` 参数）
...
**涉及 RAG 卡**：Memory 系统增强与休息沉淀卡（§4 红线 18/19/20/21/22 已重写，其中 21 锁定「沉淀必须与 message 同属一个消费者」、22 锁定 ack/nack 按 source 分流）
