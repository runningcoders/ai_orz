...
- [agent.rs (dal/agent/mod.rs)](src/service/dal/agent/mod.rs)
- [compaction.rs](src/service/domain/runtime/compaction.rs)
...
**更新摘要 2026-08-29**：summary.rs 重构为 compaction.rs（上下文压缩），settle_memory.rs 增量 +473 行批量沉淀（自适应上下文预算、按轮次分桶、PENDING_BUDGET_RATIO 常量）。
</cite>
...
## 简介
本技术文档围绕“记忆沉淀机制”展开，聚焦 agent_rest 定时任务如何每天凌晨 4 点（cron `0 4 * * *`）触发一次，将短期记忆转化为长期知识，并构建可检索、可遍历的知识图谱。文档覆盖：
...
## 项目结构
记忆沉淀涉及四层协作：
- 生产者层：CronTriggerProducer 经 `ProducerLoop` 每 60 秒轮询到期触发器并发布 cron.trigger 事件；`mark_trigger_executed` 现由 `on_consumed` 在消费终结时回调（取事件 `created_at` 推进 `next_run_at`）
- 消费者层：CronTriggerConsumer 消费事件，按 action 路由业务处理；`agent_rest` **只解析目标 Agent 并派发 `AgentSettleEvent` 后立即返回**，不在消费者线程里沉淀
- 队列层：`agent.settle.requested` 与 `message.created` 由**同一个**消费者 `agent.awakening` 消费（`order_key = agent_id`、`concurrency = 4`）—— 同 Agent 的沉淀与消息在队列层就串行
- 领域层：MemoryDAL 编排短期记忆聚合、向量化、知识节点创建与引用建立

```mermaid
graph TB
A["CronTriggerProducer<br/>每分钟扫描到期触发器"] --> B["AOP 事件中心<br/>cron.trigger"]
B --> C["CronTriggerConsumer<br/>action=agent_rest（只派发）"]
C --> F["AOP 队列<br/>agent.settle.requested（order_key=agent_id）"]
F --> G["agent.awakening 消费者<br/>与 message.created 同队列同 order_key（串行）"]
G --> D["MemoryDAL<br/>settle_short_term_to_long_term"]
D --> E["DAO/Vector Store<br/>写入知识节点 + 向量索引"]
```

> ⚠️ 中间这两个节点（C → F → G）不是可选的编排细节：**去掉它们就会在 Agent 正忙时静默丢一整天**（旧实现即如此）。另外 **G 必须与 `message.created` 是同一个消费者**：AOP 的 `order_key` 闸门按消费者隔离，拆成两个消费者时同一个 `agent_id` 落在两条互不知晓的队列里，串行保证会静默失效（只能靠抢占失败重试兜底，日志与失败指标被刷爆）。审计/回归见 [Memory 系统增强 RAG 卡 §4 红线 18/19/20/21/22](docs/wiki/knowledge/zh/Memory 系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+ agent_rest 每天 4 点 settle + load_and_settle 向量去重合并/Memory 系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+ agent_rest 每天 4 点 settle + load_and_settle 向量去重合并.md)。
...
- CronTriggerConsumer：同步消费者，解析 payload.action 并分发；`agent_rest` 仅解析目标 Agent 并 `publish(AgentSettleEvent)`，**不在此线程执行沉淀**（Sync 消费者跑在 cron 轮询线程，长任务会堵住所有定时器）
- agent.awakening 消费者（`src/consumer/message.rs`）：异步消费者，同时订阅 `message.created` 与 `agent.settle.requested`；后者走 `try_set_resting` 抢占 + `BusyGuard` 兜底释放执行独占沉淀（抢不到属防御分支，上抛冲突由队列重投）。两类事件共用一条 `order_key = agent_id` 的队列，所以沉淀在跑时发给同 Agent 的消息不会出队
...
Check --> |否| Create["创建 Cron 触发器<br/>cron_expression='0 4 * * *'（每天 04:00）<br/>payload={action:'agent_rest', extra:{settle_limit:10}}"]
Check --> |是| Skip["跳过创建（幂等）"]
Create --> NextRun["设置 next_run_at = 计算下次 04:00"]
Skip --> NextRun
...
- 重要性标记：对值得其他 Agent 优先参考的通用方法论 / 模式 / 概念添加 published 标签（只影响推荐起点排序，**不**影响可见性）
...
- [agent.rs:1185-1204](src/service/dal/agent/mod.rsL1204)
...
- 触发器执行：`mark_trigger_executed` 已由 `CronTriggerProducer::on_consumed` 在消费终结时回调推进 last_run_at/next_run_at；消费者解析失败返回错误
...
- 重要性标注：published 标签标记「值得其他 Agent 优先参考」的高价值节点，只影响推荐起点排序；知识节点在蜂巢内本就全局可见，**不存在**「不写 published = 私有节点」
...
- CronTriggerConsumer 依赖 handler 层 `settle_memory::{settle_agent_exclusive, load_and_settle}`：前者给定时队列链路（抢占式），后者给神经工具链路；生产者与消费者之间经 `AgentSettleEvent` 排队解耦
...
C["CronTriggerConsumer"] --> AS["agent.awakening 消费者<br/>handle_settle_request"]
AS --> LS["settle_agent_exclusive"]
LS --> MDAL["MemoryDAL"]
MDAL --> MDAO["MemoryDao"]
...
- 可配置的沉淀策略（settle_limit、向量距离阈值）与节点重要性标注（published）
...
## 附录
- 系统默认触发器：agent_rest（每天 04:00，cron `0 4 * * *`）、project_followup（每 1 小时）
...
**主题**：agent_rest 定时沉淀支持系统级全局执行，修复「从未成功」历史 bug
**关键变更**：
1. **handle_agent_rest 新增全局模式（src/consumer/scheduler.rs）**——旧版只对 trigger payload 硬编码的特定 org_id 执行沉淀，多组织部署时大部分组织的沉淀永远不触发；重构后 payload 无 organization_id 时遍历所有活跃组织（`SELECT id FROM organizations WHERE status=1`）逐一执行沉淀；
2. **向后兼容**——旧 trigger payload 带 org_id 时仍走组织级路径，新系统级 trigger 不带 org_id 时走全局遍历；
3. **load_and_settle 支持 organization_id=None（src/handlers/hr/agent/settle_memory.rs）**——全局模式下 organization_id 从 DB 活跃组织列表动态获取。
**涉及 RAG 卡**：Memory 系统增强与休息沉淀卡 + 定时任务和事件系统卡（长文）

> ⚠️ **本条已被 2026-09-16 取代**：`handle_agent_rest` 不再按组织遍历（`load_and_settle` 也已无 `organization_id` 参数），改为扫描全库 `Active` 短期记忆去重出目标 Agent 后逐个派发 —— 见下方 2026-09-16 更新摘要。本段仅保留为历史脉络。
...
### 更新摘要（2026-09-16，base 5ac0a99b→HEAD）
**主题**：agent_rest 沉淀队列化——修复「项目巡检跑了、睡眠沉淀没跑，触发器显示最近执行 = 启动时刻」的静默丢一天
**关键变更**：
1. **`handle_agent_rest` 改为只派发（src/consumer/scheduler.rs）**——解析目标 Agent（payload.agent_id 或扫描全库 `Active` 短期记忆去重）后逐个 `publish(AgentSettleEvent)` 并立即返回；不再在 cron 轮询线程里同步跑沉淀（旧实现一场沉淀阻塞轮询数分钟，且 Agent 忙时只 `Ok(0)` 跳过，而 Producer 随后已把 `next_run_at` 推到次日 → 丢一天）
2. **新增 `AgentSettleEvent`（src/models/events/agent_settle.rs），并把它并入 `agent.awakening` 消费者**——不新建独立消费者是**关键决策**：AOP 的 `order_key` 闸门按消费者隔离（`registry.queues: HashMap<consumer_name, EventQueue>`），拆成两个消费者会让同一个 `agent_id` 落在两条互不知晓的队列里，串行保证静默失效。合并后两类事件同队列同 `order_key = agent_id`：沉淀在跑时消息压根不出队（不失败、不重试、不刷日志），`concurrency = 4`；`resource_not_found` → ack 作废
   > 初版曾用 `NackMode::Defer` + `set_idle` 主动唤醒兜底抢占失败，已**整体回滚**（抢占失败本就不该发生；收尾统一由 `Registry::finish_consumption` 按 topic 反查生产者回调，不再有 `Consumer::ack/nack/source`）
3. **`settle_memory.rs` 拆出 `settle_body` / `settle_agent_exclusive`（独占式，`try_set_resting` 抢占 + `BusyGuard` 兜底）**；`load_and_settle` 保留为神经工具入口，维持「Agent 天然 Busy → return 0」语义
4. **`AgentRuntimeStateManager::try_set_resting`**——与 `try_set_busy` 同构的原子抢占（Idle → Resting 才成功），禁止拆成「先查询、后设状态」
5. **顺带修复存量解析坑**：`CronTriggerPayload.extra` 曾是 `#[serde(flatten)]`，会把 `extra` 键名多包一层导致 `agent_id`/`settle_limit` 静默降级 None（指定单 Agent 退化成全局扫描、条数上限被忽略，且不报错）；已改为具名字段 + `action_params()` 空值归一
**涉及 RAG 卡**：Memory 系统增强与休息沉淀卡（§4 红线 18/19/20/21/22 已重写，其中 21 锁定「沉淀必须与 message 同属一个消费者」、22 锁定收尾按 topic 归属反查生产者回调，无 source 分流）
**本次为设计快照级变更**：接口细节以 `docs/design/runtime_design.md` §25.15 与代码为准
