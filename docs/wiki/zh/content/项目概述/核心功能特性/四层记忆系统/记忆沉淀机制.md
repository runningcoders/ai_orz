...
- 队列层：AgentSettleConsumer 异步消费 `agent.settle.requested`（`order_key = agent_id`、`concurrency = 2`），用 `try_set_resting` 原子抢占；抢不到 → `Err(conflict)` → 队列 nack 重投，等 Agent 空闲自动补跑
...
F --> G["AgentSettleConsumer<br/>try_set_resting 抢占，失败即 nack 重投"]
G --> D["MemoryDAL<br/>settle_short_term_to_long_term"]
D --> E["DAO/Vector Store<br/>写入知识节点 + 向量索引"]
```

> ⚠️ 中间这两个节点（C → F → G）不是可选的编排细节：**去掉它们就会在 Agent 正忙时静默丢一整天**（旧实现即如此）。审计/回归见 [Memory 系统增强 RAG 卡 §4 红线 18/19/20](docs/wiki/knowledge/zh/Memory 系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+ agent_rest 每天 4 点 settle + load_and_settle 向量去重合并/Memory 系统增强与休息沉淀：四层记忆（Core／Working／Short／Long）+ agent_rest 每天 4 点 settle + load_and_settle 向量去重合并.md)。
...
- AgentSettleConsumer：异步消费者，独占式执行单个 Agent 的沉淀（`try_set_resting` 抢占 + `BusyGuard` 兜底释放），忙时上抛冲突由队列重投
...
C["CronTriggerConsumer"] --> AS["AgentSettleConsumer"]
...
2. **新增 `AgentSettleConsumer`（src/consumer/agent_settle.rs）+ `AgentSettleEvent`（src/models/events/agent_settle.rs）**——Async 消费者，`order_key = agent_id`、`concurrency = 2`；忙 → `Err(conflict)` → 队列 nack 重投（30s 退避）→ 空闲自动补跑；`resource_not_found` → ack 作废
...
**涉及 RAG 卡**：Memory 系统增强与休息沉淀卡（§4 红线 18/19/20 已重写）
