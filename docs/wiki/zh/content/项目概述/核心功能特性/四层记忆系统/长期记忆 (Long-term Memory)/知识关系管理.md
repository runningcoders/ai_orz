...
- [memory_test.rs](src/service/dal/memory_test.rs)
...
**图表来源**
- [save_long_term_memory.rs:75-109](src/handlers/hr/agent/save_long_term_memory.rs#L75-L109)
- [memory.rs:372-390](src/service/dal/memory.rs#L372-L390)
- [sqlite.rs:1095-1125](src/service/dao/memory/sqlite.rs#L1095-L1125)
- [knowledge_graph.rs:55-108](frontend/src/pages/hr/knowledge_graph.rs#L55-L108)
...
## 核心组件
- 关系类型词表：`KnowledgeRelationType` 定义 15 个规范关系词 + `custom` 兜底，**只用于展示期映射**（命中 → 中文短标签；未命中 → 原样透出）；落库字段 `relation_type` 存的是**写入方标注的原文**。
...
- [sqlite.rs:1095-1125](src/service/dao/memory/sqlite.rs#L1095-L1125)
- [sqlite.rs:1127-1283](src/service/dao/memory/sqlite.rs#L1127-L1283)
...
### 关系模型与约束
- 关系类型：`relation_type` 落库存**写入方标注的原文**（自由字符串，写入侧不做任何归一化）；`KnowledgeRelationType` 只是**展示期词表** —— 命中 → 中文短标签，未命中 → 原样透出（**绝不**替换成「自定义」）。`From<String>` 只作归类用途，**禁用在写入路径**（词表外的值会被塌成 `Custom`、原文永久丢失）。
...
KnowledgeNodeRelationPo ..> KnowledgeRelationType : "展示期映射（不落库）"
```
...
- [memory.rs:372-390](src/service/dal/memory.rs#L372-L390)
- [sqlite.rs:1095-1125](src/service/dao/memory/sqlite.rs#L1095-L1125)
- [sqlite.rs:1285-1328](src/service/dao/memory/sqlite.rs#L1285-L1328)
- [memory_test.rs:1260-1284](src/service/dal/memory_test.rs#L1260-L1284)
...
**图表来源**
- [memory.rs:891-995](src/service/dal/memory.rs#L891-L995)

**章节来源**
- [memory.rs:805-889](src/service/dal/memory.rs#L805-L889)
- [memory.rs:891-995](src/service/dal/memory.rs#L891-L995)
...
**图表来源**
- [sqlite.rs:1287-1306](src/service/dao/memory/sqlite.rs#L1287-L1306)
- [sqlite.rs:131-179](src/service/dao/memory/sqlite.rs#L131-L179)

**章节来源**
- [sqlite.rs:25-28](src/service/dao/memory/sqlite.rs#L25-L28)
- [sqlite.rs:1287-1306](src/service/dao/memory/sqlite.rs#L1287-L1306)
- [memory.rs:997-1028](src/service/dal/memory.rs#L997-L1028)
...
**章节来源**
- [sqlite.rs:1095-1125](src/service/dao/memory/sqlite.rs#L1095-L1125)
- [sqlite.rs:1302-1328](src/service/dao/memory/sqlite.rs#L1302-L1328)
...
**章节来源**
- [sqlite.rs:1231-1283](src/service/dao/memory/sqlite.rs#L1231-L1283)
- [memory.rs:805-889](src/service/dal/memory.rs#L805-L889)
- [memory.rs:891-995](src/service/dal/memory.rs#L891-L995)
- [sqlite.rs:1287-1306](src/service/dao/memory/sqlite.rs#L1287-L1306)
- [20260724000000_knowledge_node_tags.sql:24-44](migrations/20260724000000_knowledge_node_tags.sql#L24-L44)
- [20260731000001_knowledge_node_is_published.sql:1-12](migrations/20260731000001_knowledge_node_is_published.sql#L1-L12)
...
- [memory_test.rs:2121-2167](src/service/dal/memory_test.rs#L2121-L2167)

## 结论
本方案通过明确的关系类型词表（**原文落库 + 展示期映射**）、有向边模型与严格的更新/删除约束，结合高效的批量查询与可控的遍历算法，实现了稳定可靠的知识关系管理与图谱分析能力。**最新的性能优化进一步提升了系统在大规模图谱场景下的表现**：DFS遍历的N+1查询问题得到解决，SQLite IN子句分块处理避免了参数溢出风险。配合 FTS5 与向量索引，系统在检索与可视化方面具备良好性能与可扩展性。开发者可基于现有 API 快速构建知识图谱应用，并在需要时扩展关系类型与遍历策略。
...
#### ① Design 决策快照
- [memory_search_enhancement_design.md](docs/archive/design-archive/memory_search_enhancement_design.md) — 搜索增强决策 3 关系搜索 JOIN 方式 + 决策 4 向量阈值参数化 0.8 默认
#### ② Plan 落地快照
- [知识图谱推荐起点与组件复用重构.md](docs/archive/plan-archive/知识图谱推荐起点与组件复用重构.md) — recommend_seed_nodes 冷启动起步入口 + KnowledgeGraph 子组件两端复用
#### ④ RAG 原子知识卡
- [记忆搜索增强三合一：FTS5 tags 语义过滤 + 图谱 traverse BFS／DFS 遍历 + recommend_seed_nodes 三因子推荐](docs/wiki/knowledge/zh/记忆搜索增强三合一：FTS5%20tags%20语义过滤%20+%20图谱%20traverse%20BFS%2FDFS%20遍历%20+%20recommend_seed_nodes%20三因子推荐/记忆搜索增强三合一：FTS5%20tags%20语义过滤%20+%20图谱%20traverse%20BFS%2FDFS%20遍历%20+%20recommend_seed_nodes%20三因子推荐.md) — §3 三合一总数据流 + §4.1 红线 7 agent_id=None 全局推荐必须 include_shared=true
