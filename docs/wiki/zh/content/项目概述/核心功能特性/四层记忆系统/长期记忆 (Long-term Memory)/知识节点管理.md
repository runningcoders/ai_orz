...
## 简介
本技术文档围绕“知识节点管理”展开，聚焦于长期知识图谱中的知识节点（LongTermKnowledgeNodePo）的数据结构设计、创建流程（含从短期记忆到知识提炼的 AI 处理与质量评估）、更新策略（内容修订、版本控制与变更历史）、CRUD 操作接口、以及共享模型（蜂巢共享、重要性标记与归属筛选）。文档严格遵循四层单向调用规范：Adapter（HTTP Handler / AOP Producer）→ Domain → DAL → DAO，PO 仅在 DAO/DAL 内部使用，Domain 对外暴露业务实体。
...
- KnowledgeNodeRelationPo：知识节点关系 PO，描述源节点与目标节点的关系类型（**原文**）、关联强度与时间戳。
...
- 遍历：基于种子节点进行广度优先或深度优先遍历，支持以高价值节点作为桥梁发现跨 Agent 的知识网络。
- 更新：支持内容修订、标签更新（含 published），自动重新向量化；删除采用软删除（标记 Forgotten）。
- 沉淀：将未沉淀的短期记忆总结为知识节点，并标记原短期记忆为 Settled。
- 蜂巢共享：所有知识节点对所有 Agent 可见（无需标记）；`published` 是重要性标记，`is_published` 冗余字段供索引/排序使用。
...
- LongTermKnowledgeNodePo：
  ...
  - tags：JSON 数组字符串，支持标签过滤与全文索引；包含 "published" 时 is_published 为 true（重要性标记）。
  - status：Active/Settled/Forgotten，控制生命周期状态。
  - is_published：冗余字段，由 tags 派生，供索引与推荐起点排序使用（**不控制可见性**）。
...
**2026-09-17 拍板：蜂巢共享**——知识节点是**全体 Agent 的共享资产**，任何 Agent 都能读到全部节点，保证「知识不重复不遗漏」。可见性**不再**由 `published` 决定。

- **`published` = 重要性 / 影响力标记**：`tags` 包含 `"published"` 时冗余字段 `is_published=true`。唯一用途是 `recommend_seed_nodes` 在连接度持平时作决胜项，**不改变可见性、不进业务标签**。
- **归属只是筛选条件**：`agent_id` 仅在调用方**显式传入**时才作为过滤谓词；不传 / 空串 = 不过滤（蜂巢全域）。**禁止**在 handler 里回退 `ctx.agent_id()` 去收窄知识节点查询（会让 Agent 调用静默变成「只看自己」）。
- **已删除的旧机制**：`include_shared` 查询参数、`TraverseScope` 遍历作用域均随该语义变更移除；`is_published` 冗余列保留（索引 / 排序用）。
- **边与节点同批返回**：遍历 / 搜索返回的每条关系边，其两端节点必定同在本次结果中（端点缺失的边在 DAL 单点闸 `drop_dangling_relations` 丢弃，前端另有一道兜底不画）。
...
- 遍历为空：检查 seed_node_ids 是否存在，max_depth/max_breadth 是否合理，节点是否都处于 Active（Forgotten 默认不检索）。
...
## 结论
知识节点管理通过清晰的分层架构与丰富的功能（混合搜索、图谱遍历、短期记忆沉淀、蜂巢共享）实现了高效、可扩展的知识图谱能力。开发者可通过标准 API 进行 CRUD 操作，并利用标签与状态控制节点生命周期；可见性对全体 Agent 一视同仁（蜂巢共享），`published` 仅用于标注高价值节点。建议在生产环境中结合监控与日志，持续优化向量化与搜索性能。
