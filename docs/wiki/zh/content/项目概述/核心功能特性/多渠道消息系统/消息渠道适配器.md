...
- [src/service/domain/message/inbound.rs](src/service/domain/message/inbound.rs)
...
- 入站适配与生产者：Consumer（无状态，Async 模式）从 AOP 事件管道取消息，经 MessageDomain 入站门面 `adapt_inbound(ctx, InboundSource::Xxx)` 转换为 AdaptedMessage，再通过 AdapterRegistry 回调投递到 producer 路由链。
...
- 适配桥接：
  - Consumer 无状态化：LarkInboundConsumer / WechatInboundConsumer 均 derive Default，不持有任何渠道 DAL 引用。
  - DAO 读循环只 publish（Async 模式），协议转换 / 渠道查找 / 消息投递全部在 AOP worker 线程执行，慢业务不阻塞 WS / 长轮询收帧。
  - Consumer 统一走 MessageDomain 入站门面：`message_domain::domain().inbound().adapt_inbound(ctx, InboundSource::Xxx(event))`，InboundSource 枚举收敛 Lark / Wechat 等所有渠道变体。
  - 投递回调统一经 `registry().current_callback()` 取 AdapterRegistry 登记的 producer 回调，解耦 consumer 与 producer。

```mermaid
sequenceDiagram
participant FE as "飞书平台"
participant WS as "ws.rs (DAO读循环)"
participant Pipe as "AOP事件管道"
participant Consumer as "LarkInboundConsumer (无状态)"
participant Domain as "MessageDomain 入站门面<br/>(InboundSource枚举 + MessageInboundAdapt trait)"
participant DAL as "LarkMessageChannelDal"
participant Registry as "AdapterRegistry<br/>current_callback()"
FE-->>WS : 建立WS连接(获取端点)
WS->>WS : 定时发送ping(30s)
FE-->>WS : event(im.message.receive_v1)
WS->>Pipe : publish lark.inbound.message (Async模式，入队即返回)
Pipe->>Consumer : on_event(ctx, LarkInboundEvent)
Consumer->>Domain : domain().inbound().adapt_inbound(ctx, InboundSource::Lark(event))
Domain->>DAL : adapt_lark(ctx, app_id, event)
DAL->>DAL : 协议转换 / 渠道定位 / 用户映射
DAL-->>Domain : AdaptedMessage
Domain-->>Consumer : AdaptedMessage
Consumer->>Registry : registry().current_callback()
Registry-->>Consumer : Option<Callback>
Consumer->>Registry : callback.on_message(AdaptedMessage)
Note over Registry : producer路由档位链 → send_to_agent
```
...
- [src/service/domain/message/inbound.rs:25-66](src/service/domain/message/inbound.rs#L25-L66)
- [src/consumer/lark_inbound.rs:43-73](src/consumer/lark_inbound.rs#L43-L73)
- [src/consumer/wechat_inbound.rs:42-80](src/consumer/wechat_inbound.rs#L42-L80)
...
### 历史演进：Consumer 简化 + MessageDomain 入站门面
- **早期架构**：Consumer 持有各渠道 DAL 弱引用，DAO 读循环直接调用 DAL 的 adapt 方法并同步投递到 producer，慢业务阻塞 WS / 长轮询收帧。
- **当前架构**（Consumer 简化迭代）：
  - DAO 读循环只 publish（Async 模式，入队即返回），消费全部在 AOP worker 线程执行。
  - Consumer 无状态化：LarkInboundConsumer / WechatInboundConsumer 均 `derive(Default)`，不持有任何 DAL 引用。
  - MessageDomain 新增入站门面层：`InboundSource` 枚举统一收敛 Lark / Wechat 等渠道入站变体，`MessageInboundAdapt` trait 暴露 `adapt_inbound(ctx, InboundSource)` 入口。
  - Consumer 调用链简化为两步：① `message_domain::domain().inbound().adapt_inbound(...)` 获取 AdaptedMessage；② `registry().current_callback()` 取回调并投递。
  - 新增渠道只需加 InboundSource variant + 在 MessageInboundAdapt 的 match 分支接入对应 DAL adapt 方法，Consumer 侧零改动。

章节来源
- [src/service/domain/message/inbound.rs:1-66](src/service/domain/message/inbound.rs#L1-L66)
- [src/consumer/lark_inbound.rs:19-73](src/consumer/lark_inbound.rs#L19-L73)
- [src/consumer/wechat_inbound.rs:18-80](src/consumer/wechat_inbound.rs#L18-L80)

## 依赖关系分析
- DAL 对 DAO 单向依赖，DAO 之间相互独立。
- 飞书模块内部分为 http/token/ws/event/error，职责清晰。
- Consumer 无状态：LarkInboundConsumer / WechatInboundConsumer 均 `derive(Default)`，不持有 DAL 引用；协议转换统一经 `message_domain::domain().inbound().adapt_inbound(ctx, InboundSource::Xxx)` 门面层收敛，投递回调统一经 `registry().current_callback()` 解耦。
