...
- [token_stats.rs](src/handlers/finance/model_provider/token_stats.rs)
- [model_provider.rs (domain)](src/service/domain/finance/model_provider.rs)
- [model_provider.rs (dal)](src/service/dal/model_provider.rs)
- [runtime_stats.rs (tool)](src/handlers/finance/tool/runtime_stats.rs) — 组织级工具运行时统计 handler（2026-09-14 新增）
- [number.rs](frontend/src/utils/number.rs) — 大数格式化工具（千分位 + compact 缩写）
- [workspace.rs](frontend/src/pages/workspace.rs) — Workspace 顶栏数字统计卡消费端
- [src/handlers/system/workspace_metrics.rs](src/handlers/system/workspace_metrics.rs) — 工作台顶栏聚合端点，复用 DuckDB 统计域（model_call_time_series / tool_call_stats）做窗口读数
- [frontend/src/pages/workspace.rs](frontend/src/pages/workspace.rs) — 工作台顶栏改为单一聚合快照消费，旧 token_stats/tool_stats/runtime-list 前端封装已清理
...
- 【① Design 决策快照】
  ...
  - [workspace_topbar_metrics_design.md](docs/design/workspace_topbar_metrics_design.md) — 工作台顶栏从 5s/30s 多轮询收敛为 30s 单端点快照（DuckDB 批次刷盘语义保留）
- 【② Plan 落地快照】
...
- 【④ RAG 原子知识卡】
  ...
  - [工作台顶栏聚合指标：system handler 跨域编排 + 前端 30s 单轮询](docs/wiki/knowledge/zh/工作台顶栏聚合指标：system%20handler%20跨域编排%20+%20前端%2030s%20单轮询/工作台顶栏聚合指标：system%20handler%20跨域编排%20+%20前端%2030s%20单轮询.md) — DuckDB 批次刷盘语义在顶栏的消费端说明
- 【③ Wiki 关联长文】
...
- 统计引擎 Stats
  - 管理多张统计表，自动批处理与 flush（`batch_size` 阈值 + 周期落盘 + 退出兜底三重保障）。
...
- 初始化与落盘时机（策略由 `StatsConfig` 驱动，全部在 `Stats` 模块内闭环）
  - `Stats::open_and_init(path, &StatsConfig)`：生产唯一入口，由 `Storage::new()` 调用，内含 `open()` + `initialize_default()` + 启动周期落盘；`Stats::open()` 保持纯构造（只开连接、不启后台任务），供测试复用。
  - record() 将事件推入缓冲，达到 `batch_size`（默认 100）时 flush 到 DuckDB。
  - `flush_interval_secs`（默认 30s，0=关闭）驱动周期落盘任务：低频实例长期攒不满 `batch_size` 时兜底，避免 DuckDB 停在旧快照导致看板查不到近期数据；任务只持 `Weak<Stats>`，库释放即自行退出。
  - 进程优雅退出时 flush_all() 做最后一次落盘。
  - flush_type_id() 保证锁顺序 tables → conn，避免死锁。
- 字段可空约定
  - 事件的 tags / metrics 均为可选；`default_events` 的 JSON 列在字段缺失时写 NULL，不写空串（空串不是合法 JSON 文档，DuckDB 会拒绝，进而让整批落盘失败）。
  - 专用表（model_call_events 等）的缺失字段同样以 NULL 落库，过滤条件对空值不匹配。
- 通用聚合与时序查询
  - query_aggregation()：支持 equals/range 过滤、group by、count/sum/avg 聚合。
  - query_time_series()：按 Minutely/Hourly/Daily 分组，计算 tokens_input/output/call_count。粒度只决定桶宽，**桶数上限由调用侧的策略护栏统一收敛**（见下条）。
  - 粒度护栏：`common::models::clamp_interval_to_span(interval, span_ms)` 保证桶数 ≤ `STATS_MAX_BUCKETS`（512），超限则沿 Minutely → Hourly → Daily 单向回退；窗口缺失（`span_ms <= 0`）时不干预。收敛点在 `ModelProviderStatsDao::do_query`（全项目时序查询的唯一入口），而非各 handler 白名单 —— 只有那里同时拿得到「最终生效的粒度」与「兜底后的窗口」（`time_range=None` 会被兜成最近 7 天）。
...
- 可视化
  ...
  - Workspace 顶栏统计卡（2026-09-14 升级）：从折线/环形图表可视化切换为**数字读数模式**——四张统计卡并列展示「工具总数 / 今日调用次数 / 成功率 / 平均耗时」，配套 `frontend/src/utils/number.rs` 大数格式化工具（千分位 + compact 缩写 1.5K / 2.3M），消费后端组织级接口 `GET /api/v1/finance/tools/runtime-stats`（DuckDB tool_call_events 按 organization_id 自动过滤聚合，多组织隔离）。
- 报表生成
...
**变更 commit**：1245f3f8（Token 消耗看板后端）+ 3f6872c8（聊天侧栏运行统计）

### 增量 1：Token 消耗看板新维度
- ModelProvider 维度统计新增独立 handler `GET /api/v1/finance/model-providers/token-stats`，分钟级三线时间序列（input/output/total tokens + call_count），finance 域归位（从 system handler 独立出来）
- StatsInterval 枚举从 Hourly/Daily 扩展为 Minutely/Hourly/Daily，collector.rs 支持分钟 bucket 截断查询
- ModelProviderStatsDao 新增 user_id 维度过滤注入，支撑个人信息页 + 聊天侧栏的 Agent 维度统计按需查询
- 批次刷盘预期约束：最近 1-2 分钟数据可能未 flush，前端曲线末端偏低属正常

### 增量 2：前端消费扩展
- 统计数据消费从「统计仪表盘页面」扩展到「个人信息页（UserStatsPanel）」+「聊天侧栏 Agent Tab（AgentStatsPanelCompact）」
- 三线趋势图（LineChart）默认尺寸 600x200；聊天侧栏窄容器用 320x180 原生尺寸避免 CSS 缩放
- HudPalette 扩展 HUD_SECONDARY + HUD_TERTIARY 两色，支撑三线对比色区分（冷/暖/紫）

### 增量 3：新增文件清单
| 路径 | 角色 |
|------|------|
| src/handlers/finance/model_provider/token_stats.rs | Token 消耗时序新 handler |
| src/service/domain/finance/model_provider.rs | Domain 新方法 model_call_time_series + get_model_call_stats_for_user |
| common/src/models/stats.rs | StatsInterval.Minutely + TimeSeriesPoint |

---

### 本文关联的文档
- 🎴 RAG 卡: docs/wiki/knowledge/zh/统计查询 API 与前端仪表盘：DuckDB 5 维表查询 + RuntimeStats 内存滑动聚合 + StatsHandler REST API + 前端 Line/Donut/Gauge 展示/统计查询 API 与前端仪表盘：DuckDB 5 维表查询 + RuntimeStats 内存滑动聚合 + StatsHandler REST API + 前端 Line/Donut/Gauge 展示.md

---

### 更新摘要（2026-09-11，base ad54411c→HEAD）
**主题**：周期落盘收口 + 可空 JSON 兼容 + 用户页统计兜底 + TimeRangePicker 通用时间筛选
**关键变更**：
1. **周期落盘收口到 Stats::open()**——src/pkg/stats/default.rs 启动批次刷盘定时器，与 DuckDB 连接生命周期绑定
2. **可空 JSON 兼容**——model_call_events.call_summary/token_summary nullable + COALESCE 兜底旧数据
3. **用户页统计兜底**——空结果时前端显示空态卡片而非 error toast
4. **TimeRangePicker 通用组件**——所有统计看板统一引入时间区间筛选，预设 1h/6h/24h/7d/30d
**涉及 RAG 卡**：统计查询 API 与前端仪表盘卡 + Canvas HUD 可视化卡

---

### 更新摘要（2026-09-14）

Workspace 顶栏统计图升级为**数字读数模式**——从折线/环形图表可视化切换为数字统计卡（工具总数 / 今日调用次数 / 成功率 / 平均耗时），配套 `frontend/src/utils/number.rs` 大数格式化工具（千分位 + compact 缩写 1.5K / 2.3M）。后端新增组织级工具运行时统计接口 `GET /api/v1/finance/tools/runtime-stats`（DuckDB tool_call_events 按 organization_id 过滤聚合）。

- 工作台顶栏改为单一聚合端点消费，不再直接拉 token-stats / tools/runtime-stats / runtime-list；30s 单轮询覆盖所有顶栏数字