...
- [Cron 任务调度与系统启动顺序：5 字段 cron 解析 + ensure_system_cron_triggers 2 条注入 + CronTriggerProducer 60s 轮询 + 两阶段 init/init_base_data/aop 严格分离](docs/wiki/knowledge/zh/Cron 任务调度与系统启动顺序：5字段cron解析 + ensure_system_cron_triggers 2条注入 + CronTriggerConsumer 事件 + 两阶段init/aop严格分离/Cron 任务调度与系统启动顺序：5字段cron解析 + ensure_system_cron_triggers 2条注入 + CronTriggerConsumer 事件 + 两阶段init/aop严格分离.md)
</cite>

## 更新摘要
**变更内容**
- Cron 生产者-消费者契约同步：`CronTriggerProducer` 改用 `Producer::start(EventSink)` 自管 `ProducerLoop`（每 60s `loop_ctl.sleep`），删除 `poll` / `poll_interval_secs`
- P3 修复：`mark_trigger_executed` 从 `tick()` 移入 `CronTriggerProducer::on_consumed`（取事件 `created_at` 推进 `next_run_at`）；`decide_retry` 恒 `Retry`
- 消费者订阅声明改为 `subscriptions()`；收尾由 `finish_consumption` 反查生产者回调
- 类图与架构时序图已对齐新 trait（`topic()` / `on_consumed` / `on_failed` / `subscriptions()`）
...
- 生产者与消费者
  - CronTriggerProducer：每 60 秒经 `ProducerLoop` 轮询一次，查询到期触发器并发布 cron.trigger 事件；`mark_trigger_executed` 已移至 `on_consumed`，由消费终结回调推进 `next_run_at`。
...
- Producer 经 `ProducerLoop` 周期扫描到期触发器并发布事件；执行标记由 `on_consumed` 推进。
...
K->>K : "执行业务逻辑"
E->>P : "finish_consumption → on_consumed"
P->>D : "mark_trigger_executed(id, created_at)"
D->>L : "mark_executed(...)"
...
- 流程：
  ...
  - （tick 内只 `emit`；`mark_trigger_executed` 由 `on_consumed` 在消费终结时回调，取事件 `created_at` 推进 `next_run_at`）
