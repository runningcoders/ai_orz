...
### 本文关联的三类文档（四类互引闭环）
#### ① Design 决策快照
- [entity_list_query_search_design.md](docs/design/entity_list_query_search_design.md) — search 接口定义（关键词 + 向量语义 + 图谱关系三位一混合搜索）+ MAX_SEARCH_RESULTS=20 上限
- [memory_search_enhancement_design.md](docs/design/memory_search_enhancement_design.md) — 三阶段增强：tags 语义过滤 + SearchMatchInfo 扩展 + 向量阈值参数化
#### ② Plan 落地快照
- [批量查询与通用Query接口增强重构.md](docs/plan/批量查询与通用Query接口增强重构.md) — memory domain 的 query/list/search/count 透传改造
- [Query接口分页与List接口简化重构.md](docs/plan/Query接口分页与List接口简化重构.md) — knowledge_graph 搜索列表分页 PaginationParams 统一
- [知识图谱推荐起点与组件复用重构.md](docs/plan/知识图谱推荐起点与组件复用重构.md) — recommend_seed_nodes 三因子推荐 + KnowledgeGraph 两端复用（Batch11 精确对应本 Wiki 上游落地快照）
