//! HTTP 工具子表单（由 `create_tool` 容器按协议渲染）
//!
//! 对齐后端 `HttpToolConfig` 结构：HTTP 配置（method/url/模板/超时/SSRF 白名单）、
//! 凭据需求动态列表（HttpTool scope：Header/Query binding）。
//! 跨协议共享字段（name/description/tags/参数 Schema）由容器持有，见 `ToolBasicsState`。
//!
...
/// HTTP 工具协议特定表单状态（全部为文本输入，提交时统一解析校验）
...
/// HTTP 工具子表单（协议特定字段渲染，状态由容器持有）
#[component]
pub fn HttpToolSubForm(mut form: Signal<HttpToolFormState>) -> Element {
    // 敏感名即时提示（输入期预检）
...
mod tests {
    use super::*;
    use crate::components::create_tool::ToolBasicsState;
    use common::models::CredentialEnhancerKind;

    fn base_basics() -> ToolBasicsState {
        ToolBasicsState {
    ...
    fn base_http() -> HttpToolFormState {
        HttpToolFormState {
    ...
    fn build_request_requires_name_and_url() {
        let mut basics = base_basics();
        basics.name = String::new();
        assert!(build_http_create_request(&basics, &base_http()).is_err());

        let mut form = base_http();
        form.url = String::new();
        assert!(build_http_create_request(&base_basics(), &form).is_err());
    }
    ...
    fn build_request_parses_full_config() {
        let mut basics = base_basics();
        basics.tags = "weather, query".to_string();
        basics.parameters_schema = r#"{"type":"object"}"#.to_string();
        let mut form = base_http();
        ...
        let req = build_http_create_request(&basics, &form).expect("valid form should build");
    ...
    fn build_request_includes_credential_requirements() {
        ...
        let req = build_http_create_request(&base_basics(), &form)
            .expect("valid requirements should build");
