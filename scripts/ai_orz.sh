...
构建
  build              全量 release 构建（前端 dist/ + 后端二进制），产物留在仓库内
  build-fe           仅前端 release 构建并复制到 dist/

生产（仓库自部署）
  prod               构建 + 搬运到部署根 + 后台启动 release 服务（连跑两次 = 幂等重启）
  install            仅把构建产物搬运到部署根（prod 已内含；用于 build → install → start）
...
# 生产模式全景：构建 → 搬运到部署根 → 清理残留（DuckDB 文件锁 / 端口占用）→ 后台启动
# 三段分工见 scripts/prod.sh 头部「编译 / 搬运 / 启动」：build 不写部署根，install 只搬运。
cmd_prod() {
    "$SCRIPTS_DIR/prod.sh" build
    "$SCRIPTS_DIR/prod.sh" install
