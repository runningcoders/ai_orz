...
pub async fn upload_attachment(
...
) -> Result<(StatusCode, Json<ApiResponse<UploadAttachmentResponse>>), AppError> {
    let user_id = ctx.uid();
    if user_id.is_empty() {
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|err| AppError::BadRequest(format!("解析上传表单失败: {}", err)))?
    {
        ...
        if field_name == "purpose" {
            purpose = field
                .text()
                .await
                .map_err(|err| AppError::BadRequest(format!("解析 purpose 失败: {}", err)))?
                .trim()
                .to_string();
            continue;
        }
    ...
    let mut upload =
        file_upload.ok_or_else(|| AppError::BadRequest("缺少 file 字段".to_string()))?;
