...
#[register_handler_tool(
    id = "delete_agent",
    name = "Remove Agent",
    description = "Delete an agent by id (marked as Deleted, after which it no longer appears in list/query/search results). Fails with InvalidRequest if the agent is Onboarded/PendingOffboard (complete the offboarding flow first). Fails with NotFound if the agent does not exist.",
