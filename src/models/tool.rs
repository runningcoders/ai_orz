...
/// Rig 适配层 - 将我们的 Tool trait 转换为 Rig 的 ToolDyn trait
/// 
...
impl RigToolAdapter {
    pub fn new(ctx: RequestContext, inner: Box<dyn Tool>) -> Self {
        Self { ctx, inner }
    }
}
...
pub type FullTool = ToolEntity;

/// ToolEntity - complete tool entity with PO and boxed trait object
///
...
impl Clone for ToolEntity {
    fn clone(&self) -> Self {
        unreachable!("ToolEntity cannot be cloned due to dyn Trait object. Use Arc<ToolEntity> for sharing.");
    }
}
