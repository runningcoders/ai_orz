...
impl StatTable<TaskEvent> for TaskStatTable {
    ...
    fn create_table(&self, conn: &mut Connection) -> Result<()> {
        ...
        conn.execute(
            "ALTER TABLE task_events ADD COLUMN IF NOT EXISTS caller_organization_id VARCHAR",
            [],
        )
        .map_err(|e| {
            common::error::Error::internal(format!("Failed to migrate task_events table: {}", e))
        })?;
        Ok(())
    }
