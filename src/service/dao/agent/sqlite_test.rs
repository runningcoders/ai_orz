...
async fn test_insert_and_find_by_id(pool: SqlitePool) {
    ...
    let result = agent_dao.insert(new_ctx("admin", pool.clone()), &agent_po).await;
    assert!(result.is_ok());
    
    // 验证插入成功（使用 DAO 接口查询，不是直接 SQL）
    let found = agent_dao.find_by_id(new_ctx("admin", pool), &agent_po.id).await.unwrap();
...
async fn test_find_all(pool: SqlitePool) {
    ...
    for i in 0..2 {
        let agent_po2 = create_test_agent(&format!("Agent{}", i), &format!("provider-{}", i), "admin");
        let _ = agent_dao.insert(new_ctx("admin", pool.clone()), &agent_po2).await;
    }
...
async fn test_update(pool: SqlitePool) {
    ...
    let _ = agent_dao.insert(new_ctx("admin", pool.clone()), &agent_po).await;
    
    let found = agent_dao.find_by_id(new_ctx("admin", pool.clone()), &agent_po.id.clone()).await.unwrap().unwrap();
    let mut updated = found;
    updated.name = "UpdatedAgent".to_string();
    let result = agent_dao.update(new_ctx("editor", pool.clone()), &updated).await;
    assert!(result.is_ok());
    let found_after_update = agent_dao.find_by_id(new_ctx("admin", pool), &updated.id).await.unwrap().unwrap();
...
async fn test_soft_delete(pool: SqlitePool) {
    ...
    assert!(agent_dao.delete(new_ctx("admin", pool.clone()), &agent_po).await.is_ok());
    let found_after_delete = agent_dao.find_by_id(new_ctx("admin", pool), &agent_po.id).await.unwrap();
...
async fn test_find_all_excludes_deleted(pool: SqlitePool) {
    ...
    let _ = agent_dao.insert(new_ctx("admin", pool.clone()), &agent_po1).await;
    let _ = agent_dao.insert(new_ctx("admin", pool.clone()), &agent_po2).await;
    let _ = agent_dao.delete(new_ctx("admin", pool.clone()), &agent_po2).await;
...
async fn test_find_not_exists(pool: SqlitePool) {
    ...
    let found_none = agent_dao.find_by_id(new_ctx("admin", pool), "not-exist-id").await.unwrap();
