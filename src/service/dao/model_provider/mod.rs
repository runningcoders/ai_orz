...
pub trait ModelProviderStatsDao: Send + Sync {
    ...
    async fn query_tool_calls(&self, ctx: RequestContext, query: ModelProviderStatsQuery) -> Result<Vec<JsonValue>>;
    ...
    async fn sum_tool_calls(&self, ctx: RequestContext, query: ModelProviderStatsQuery) -> Result<u64> {
        let mut query = query;
        query.group_by = vec![];
    ...
    async fn query_tool_call_time_series(&self, ctx: RequestContext, mut query: ModelProviderStatsQuery) -> Result<Vec<TimeSeriesPoint>> {
        if query.interval.is_none() {
            query.interval = Some(StatsInterval::Daily);
        }
        let rows = self.query_tool_calls(ctx, query).await?;
