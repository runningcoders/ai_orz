...
async fn test_insert_and_find_by_id(pool: SqlitePool) {
    ...
    org_dao.insert(new_ctx("test-user", pool.clone()), &org).await.unwrap();
    
    let found: Option<OrganizationPo> = org_dao.find_by_id(new_ctx("test-user", pool.clone()), &org_id).await.unwrap();
...
async fn test_find_all(pool: SqlitePool) {
    ...
    org_dao.insert(new_ctx("test-user", pool.clone()), &org1).await.unwrap();
...
async fn test_update(pool: SqlitePool) {
    ...
    org_dao.update(new_ctx("test-user", pool.clone()), &org).await.unwrap();
...
async fn test_soft_delete(pool: SqlitePool) {
    ...
    // 执行软删除
    org_dao.delete(new_ctx("test-user", pool.clone()), &org_id).await.unwrap();
