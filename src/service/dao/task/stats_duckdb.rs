...
static TASK_STATS_DAO: OnceLock<Arc<dyn TaskStatsDao>> = OnceLock::new();

/// 创建一个全新的 Task Stats DAO 实例（用于测试）
pub fn stats_new() -> Arc<dyn TaskStatsDao> {
    Arc::new(TaskStatsDaoDuckDbImpl)
}

/// 获取 TaskStatsDao 单例
pub fn stats_dao() -> Arc<dyn TaskStatsDao> {
...
/// Task 统计 DAO DuckDB 实现（空结构体，每次从 ctx.stats() 获取连接）
