...
记忆系统模拟了人类从「人格底色→工作记忆→短期经验→长期知识」的四层认知结构：你的 Soul（人格/能力声明）是先天底色，Trace（会话原始记录）像你当下正在经历的事，**短期记忆**是你主动把重点写在便签上，**长期知识图谱**是你睡觉时把零散经验整理成结构化的知识网络（整理动作是个人化的，但沉淀出的节点进入蜂巢共享）。跨 Agent 的**蜂巢共享**模拟了人类文明的「群体智慧」——长期知识节点在蜂巢内全局可见，每个个体把自己的经验开放出来，其他人可以在这个基础上吸收、修正、迭代（`published` 标签标记其中值得优先参考的高价值节点）。
...
**何时保存**（有价值再记，不要每句都记）：用户提出重要需求 / 约束 / 偏好；用户明确要求「学习 / 记住」指定知识（见下节「指令式学习」）；做出关键决策或方案选择；发现反复遇到的坑、可复用模式；阶段性完成需总结成果。
...
用户给一份文档 / 链接 / 规范 / 经验，要求学习理解并记住时，**先消化再记，按类型分流**：

1. **消化提炼**：读完用自己的话总结核心要点与适用场景，不照抄原文；内容复杂或影响面大时，先向用户复述要点确认理解，再入记忆
2. **按类型分流**：
   - **通用知识 / 事实** → `save_short_term_memory`（tags 注明主题），后续沉淀为知识节点
   - **用户偏好**（用户明确告知「记住我喜欢…」）→ 走「用户偏好沉淀」流程，观察依据写「用户明确告知」，权威性高于观察推断
   - **关于我自己的行为规则**（「以后你要…」「你的风格要…」）→ 这是**自我进化**信号，何时沉淀、走哪条进化路径（记忆节点 → 技能副本 → 新技能 → Soul）统一参见**自我进化技能**（进化知识的唯一权威来源），此处只记工具用法：涉及更新 Soul 时用 `update_agent(id = 我自己的 agent_id, ...)`——**只能改自己，params.id 必须与我匹配**；名字 / 角色 / 模型 / 运行时参数这类身份路由字段不要尝试修改，会被忽略，应由管理员调整
...
5. 评估节点价值 → 对值得其他 Agent 优先参考的通用方法论 / 模式 / 概念加 `published`（重要性标记，非可见性开关）
6. `update_memory` 把已处理的短期记忆标 `Settled`（不再重复沉淀）

**该沉淀 / 不该沉淀**：✅ 可复用模式 / 抽象经验 / 核心概念 / 对旧认知的修正 / 反复出现的规律 / 用户偏好（见专节）/ 指令式学习的知识要点（消化后）；❌ 一次性对话或操作步骤 / 临时 ID 或查询结果 / 未提炼出模式的单点案例 / 不关联更大认知的纯事实。
...
**所有知识节点对所有 Agent 可见**——不存在「只有我能看见」的长期知识。你沉淀的每个节点，其他 Agent 都能通过 `search_memory` / `query_memory` 检索到；同理你也能检索到他们的。所以：
...
**层级是节点维度的**：第 0 层就是起点节点自己，每沿一条关系边走到一个新节点 = +1 层；`traversal_depth=0` 只返回起点节点本身。**起点节点一定在结果里**（不会缺中心节点），且返回的每条边两端节点都会一起返回（不会出现「A → ?」这种半条边）。
