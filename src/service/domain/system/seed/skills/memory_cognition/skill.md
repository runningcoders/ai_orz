...
**用途**：触发你进入沉淀工作模式（类似人脑的睡眠整理记忆）。与 `awaken`（唤醒响应外部消息）对称，`settle_memory` 调用 `sleep_and_settle`（沉睡整理内部记忆），你用已有工具自主完成沉淀。

**参数**：
- `limit` — 处理的短期记忆数量（可选，默认 10）

**行为**：
1. 系统查询你未沉淀的短期记忆（Active 状态），生成编号摘要
2. 唤醒大脑进入 **Settle 场景**：系统过滤工具集，只保留 `neural` / `memory` 标签的工具和技能注册到大脑，消息类工具不可见
3. 拼装沉淀场景 prompt（用户画像 + 场景上下文 + 待沉淀记忆摘要 + 沉淀约束 + 任务指引），保留你的身份认知和当前场景上下文
4. 调用 `sleep_and_settle`（与 `awaken` 对称的沉睡方法），直接进入沉淀思考
5. 你在沉淀思考中用已有记忆类工具自主完成沉淀

**沉淀工作模式的约束（重要）**：
- **不要发送消息**：睡觉是对自身知识的沉淀积累，不应依赖外部信息。发送消息会触发消息流程，把自己异步唤醒，破坏内循环
- **不要调用消息类工具**（`send_message` / `send_task_assignment_message` 等）——沉淀场景下这些工具已被系统过滤，不可见
- **只使用记忆类工具**：`search_memory` / `save_long_term_memory` / `update_memory` / `query_memory`
- 这是一个**内循环**：你与自己的记忆对话，不是与外部世界交互

**沉淀场景保留的上下文**：
- **用户画像**：认知是具身的，不知道自己是谁就不能形成有效认知
- **项目 / 任务上下文**（如有）：沉淀出的经验自带场景标签，便于场景化复用
- **历史短期记忆**：作为思考素材参与沉淀
